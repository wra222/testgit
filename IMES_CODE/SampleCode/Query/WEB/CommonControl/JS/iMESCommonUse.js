                    ...
                    case ("MB"):
                        {
                            ...
                            if ((sn.length == 11) && (sn.substr(4, 1) == "M")) {
                                return sn.substring(0, 10);
                            }
                            else if ((sn.length == 10) && (sn.substr(4, 1) == "M")) {
                                return sn.substring(0, 10);
                            }
                            else if ((sn.length == 11) && (sn.substr(5, 1) == "M")) {
...
function getMBSno(input) {
    ...
    if (pattMBSno3.exec(input)) return input.substring(0, 10);
    else return "";
}
...
function isCustSN(input) {
    ...
    pattCustSN = /^CN.{8}$/;
    if (pattCustSN.exec(input)) return true;
    else return false;
}
...
function invokeEDITSFunc(WEBEDITSAddr, EDITSFunctionName, Parameters) {
    ...
    data = data + '<soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">'; //固定
    data = data + '<soap:Body>';
