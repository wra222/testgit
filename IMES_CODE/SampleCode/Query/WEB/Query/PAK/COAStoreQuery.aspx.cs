...
public partial class Query_PAK_COAStoreQuery: IMESQueryBasePage
{
   ...
   // IMES.Query.Interface.QueryIntf.IFamily Family = ServiceAgent.getInstance().GetObjectByName<IMES.Query.Interface.QueryIntf.IFamily>(WebConstant.IFamilyBObject);
    ...
    IPAK_COAStoreQuery PAK_COAStoreQuery = ServiceAgent.getInstance().GetObjectByName<IPAK_COAStoreQuery>(WebConstant.IPAK_COAStoreQuery);

    public string[] GvQueryColumnName = { "IECPN", "Qty" };
    public int[] GvQueryColumnNameWidth = { 60, 60};
    ...
    protected void btnQuery_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //int sum = 0;
            bool flag_COA = true;
            if (rdbutCOA.SelectedValue == "COA Win7")
            {
                flag_COA = true;
            }
            else if (rdbutCOA.SelectedValue == "COA Win8")
            {
                flag_COA = false;
            }

            dt = PAK_COAStoreQuery.GetCOAStoreQueryResult(DBConnection,flag_COA);
