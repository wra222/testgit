...
{
    /// <summary>
    /// Summary description for OutSAInLineInfo
    /// </summary>
    ...
    public class OutSAInLineInfo : System.Web.Services.WebService
    {
        private static ILog logger = LogManager.GetLogger(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType);
        [WebMethod]
        public OutSAInLine[] GetOutSAInLineInfo()
        {
            string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
            DateTime now = DateTime.Now;
            string TmpPeriodDate = null;
            IList<OutSAInLine> ret = new List<OutSAInLine>();
            ...
            try
            {
                ...
                IList<OutSAInLine> OutSAInLineList = SqlHelper.ExecuteReader<OutSAInLine>(dbConnectStr, CommandType.Text, sqlText1);

                for (int i = 0; i <= OutSAInLineList.Count - 1; ++i)
                {
                    ret.Add(new OutSAInLine
                    {
                        Period_Date = OutSAInLineList[i].Period_Date,
                        Station = OutSAInLineList[i].Station,
                        PdLine = OutSAInLineList[i].PdLine,
                        OutQty = OutSAInLineList[i].OutQty,
                        WIPQty = OutSAInLineList[i].WIPQty
                    });
                }
                return ret.ToArray();
