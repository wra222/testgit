...
<%@ Page Language="C#"  MasterPageFile="~/MasterPage.master"  ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="KBCTCheck.aspx.cs" Inherits="FA_KBCTCheck" Title="Untitled Page" %>
                ...
                else
                {
                    if (newProcessFlag == true)
                    {
                        ...
                        else 
                        {
                            ...
                            WebServiceKBCTCheck.checkProdId(currentProdid, line, "<%=UserId%>", station, "<%=Customer%>", "<%=CheckModel%>", onCheckSucceed, onCheckFail);
                        }
                    ...
                    else
                    {
                        if (kbctFirstFlag == false) {
                            ...
                            WebServiceKBCTCheck.checkAndSave(currentProdid, kbct, line, "<%=UserId%>", station, "<%=Customer%>", onSucceed, onFail);
                        }
                        else {
                            beginWaitingCoverDiv();
