...
<div style="z-index: 0; width:95%" class="iMes_div_center">
	...
	<tr>
	    <td align="left">
	        <asp:Label ID="lblctQty" runat="server" CssClass="iMes_label_13pt"></asp:Label>
	    </td>
	    <td colspan="2" align="left" >
            <asp:UpdatePanel ID="UpdatePanel4" runat="server" UpdateMode="Conditional">
                <ContentTemplate>
                 <input type="text" id="txtctQty" style="ime-mode:disabled;width:100%" class="iMes_textbox_input_Yellow"
	    ...
	    <td align="center">
	        <asp:Label ID="lblctQtyTip" runat="server" iMes_textbox_input_Yellow></asp:Label>
	    </td>
...
var mesNoInputctQty = '<%=this.GetLocalResourceObject(Pre + "_mesNoInputctQty").ToString()%>';
...
function print() {
    try {
        ...
        ctqty = document.getElementById("<%=txtctQty.ClientID%>").value.trim();
        var qty = document.getElementById("<%=txtQty.ClientID%>").value.trim();
        var config = document.getElementById("<%=txtConfig.ClientID%>").value.trim();
        ...
        if (!errorFlag) {
           //调用web service提供的打印接口
            ...
            WebServiceIECLabelPrint.print(dataCode, config, rev,ctqty, qty, "", editor, stationId, customer, lstPrintItem, onSucceed, onFail);
        }
    } catch (e) {
        alert(e.description);
    }
...
function onSucceed(result) {
    try {
        ...
        if (result == null) {
			...
			//Vincent add this logical
			//pass all CT in sp
			var ct=ctLst.join();						
            var qty =ctLst.length/ctqty;
			
			for(var j=0;j<printLst.length;++j)
			{
				var port=printLst[j].PrinterPort;
				var templateName = printLst[j].TemplateName; 
				var labelType =printLst[j].LabelType;
				...
				keyCollection[3] = "@Qty";
				valueCollection[3] = generateArray(qty);
					
				keyCollection[4] = "@CQty";
				valueCollection[4] = generateArray(ctqty);
				
				keyCollection[5] = "@Port";
				valueCollection[5] = generateArray(port);	
				
				keyCollection[6] = "@Template";
				valueCollection[6] = generateArray(templateName);			
					
				setPrintParam(printLst, labelType, keyCollection, valueCollection);
				printLabels(printLst, false);
            }
			/*
            for (var i = 0; i < ctLst.length;i=j) {
			 var ct=ctLst[i];
			  for(var j=i+1;j<=i+(ctqty-1);j++)
			  {
                ct+=','+ctLst[j];}
