...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="FATestStation.aspx.cs" Inherits="FA_FATestStation" Title="无标题页" %>
        ...
        if (!errorFlag) 
        {
            ...
            else if (document.getElementById("<%=txtProdId.ClientID%>").innerText == "") 
            {
                ...
                else 
                {
                    ...
                    WebServiceFATestStation.inputProdId(getPdLineCmbValue(),getTestStationCmbValue(),SubStringSN(inputData,"ProdId"),document.getElementById("<%=useridHidden.ClientID%>").value, document.getElementById("<%=customerHidden.ClientID%>").value,onProdIdSucceed,onProdIdFail);
