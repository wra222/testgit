...
namespace IMES.Station.Implementation
{
    ...
    public class _CombineCOAandDN : MarshalByRefObject, ICombineCOAandDN
    {
		...
		private IList<Delivery> getDnByModel(string model)
		{
			IList<Delivery> deliveryList = new List<Delivery>();
			if ("PC" == model.Substring(0, 2))
				deliveryList = currentRepository.GetDeliveryListByModel(model, "PC", 12, "00");
			else if (CommonImpl.GetInstance().CheckModelByProcReg(model, true))
				deliveryList = currentRepository.GetDeliveryListByModel(model, model.Substring(0, 2), model.Length, "00");
			return deliveryList;
		}
        ...
        public IList<S_RowData_COAandDN> GetDNListQuick(string model, string pono)
        {
            ...
            try
            {
                ...
                if (pono == "")
                {
					...
					IList<Delivery> deliveryList = getDnByModel(model);
                    
                    //Vincent 2015-02-27 過濾綁訂PoDN
                    IList<string> bindPoNoList = moRep.GetBindPoNoByModel(model);
       ...
       /*
        ...
        // mantis 1658
        private bool ChkInDelivery(string model)
        {
            if (string.IsNullOrEmpty(model))
                return false;
            string sql = "select top 1 DeliveryNo from Delivery where Model=@Model";
            SqlParameter[] param = new SqlParameter[] {
                new SqlParameter("@Model", model)
            };
            DataTable dt = SqlHelper.ExecuteDataFill(SqlHelper.ConnectionString_PAK, CommandType.Text, sql, param);
            if ((dt != null) && (dt.Rows.Count > 0))
                return true;
            return false;
        }

        private bool ChkInCartonLoc(string ProductID)
        {
            if (string.IsNullOrEmpty(ProductID))
                return false;
            string sql = "select top 1 AttrValue from ProductAttr where AttrName='CartonLocation' and ProductID=@ProductID and AttrValue!=''";
            SqlParameter[] param = new SqlParameter[] {
        ...
        private void RollbackBsamLoc(BSamLocation loc, string editor)
        {
            if (null != loc)
            {
                try
                {
                    bsamRepository.RollbackMoveInLoc(loc.LocationId, 1, editor);
                }
                catch (Exception e)
                {
        ...
        public S_RowData_Product GetProductOnly(string customerSN)
        {
            ...
            try
            {
                ...
                bool isJ = CheckJapanByPart(temp.Model.Trim());
                ret.isJapan = isJ ? "Y" : "N";
