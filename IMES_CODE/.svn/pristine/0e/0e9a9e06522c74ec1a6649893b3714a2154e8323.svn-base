...
namespace IMES.Station.Implementation
{
    ...
    public class _PoData : MarshalByRefObject, IPoData
    {
        ...
        private S_PLDNLine ParseDNInfo173(string line)
        {
            ...
            if (fields.Length>143)
                ret.HDeliveryInfo.Add("ShipToParty", fields[143].Trim());
            if (fields.Length > 144)
                ret.HDeliveryInfo.Add("GroupId", fields[144].Trim());
            if (fields.Length > 145)
                ret.HDeliveryInfo.Add("Priority", fields[145].Trim());
        ...
        private PakDotPakComnInfo ParseOBDNInfo(string line)
        {
            ...
            ret.sold_to_name = ret.reseller_name;
            ret.container_id = fields[154].Trim();
            if (fields.Length > 155)
            {
                ret.incoterm = fields[155].Trim();
            }
            //for Uploadlog ID
            ret.logID = 0;
            return ret;
        ...
        private PakDashPakComnInfo CopyPakComnInfo(PakDotPakComnInfo input)
        {
            ...
            ret.container_id =input.container_id;
            ret.sold_to_name = input.sold_to_name;
            ret.incoterm = input.incoterm;
        ...
        private string GetPrimaryModel(string str, IList<string> strList)
        {
            ...
            //fixed bug infinit loop bug, don't need care about second delimiter value 
            // Spec: IEC Model/ Customer PN 
        ...
        private string GetUCCorBoxIdFor173(S_PLDNLine dn)
        {
            ...
            if (valueList[0] == "ICC")//重慶
            {
                ...
                //if (regId == "QAF") prefix = "CH39-";
                //else if (regId == "QNU") prefix = "D790-";
                //else if (regId == "QNL") prefix = "LAIQ-";
                //else if (regId == "QCN") prefix = "CD45-";
                //else if (regId == "QNE") prefix = "63D7-";
                //else if (regId == "QLA") prefix = "LDIQ-";
            }
            else
            {
        ...
        private IList<DNForUI> UploadData173(IList<string> dnLines, IList<string> pnLines, string editor)
        {
            ...
            try
            {
                ...
                foreach (string tmp in dnLines)
                {
                    ...
                    regId = (string)ele.HDeliveryInfo["RegId"];
                   ...
                   // if ((string)ele.HDeliveryInfo["RegId"] == "SCN")
                    //if (regId=="CN")
                    if (ActivityCommonImpl.Instance.CheckDomesticDN(regId))
                   {
                        continue;
                    }
                  ...
                  //  if ((string)ele.HDeliveryInfo["RegId"] == "SUC")
                    if (regId == "UC")
                    {
                      //  ele.HDeliveryInfo["RegId"] = "SNU";
                        ele.HDeliveryInfo["RegId"] = ele.HDeliveryInfo["RegId"].ToString().Replace("UC", "NU");
                    ...
                    bCarrier = new[] { "~BNAF~", "~DZNA~" }.Any(tmp.Contains);
                    bShRegId = new[] { "~SCA~", "~SNA~", "~SAF~" }.Any(tmp.Contains);
                    bCqRegId = new[] { "~QCA~", "~QNA~", "~QAF~" }.Any(tmp.Contains);
                    // if ((tmp.Contains("~BNAF~") || tmp.Contains("~DZNA~")) && (tmp.Contains("~SCA~") || tmp.Contains("~SNA~") || tmp.Contains("~SAF~")))
                    if(bCarrier &&(bShRegId||bCqRegId))
                    {
                ...
                foreach (string tmp in pnLines)
                {
                    ...
                    if (tmpDlv == null && ele.UCC == "" && !ele.PalletNo.StartsWith("NA"))
                    {
                        ...
                        if (tmpRegId != null && tmpRegId.Length == 3)
                        { tmpRegId = tmpRegId.Substring(1, 2); }
                        else
                        { tmpRegId = ""; }
                    //    if (tmpRegId == "SCU" || tmpRegId == "SNU")
                           if (tmpRegId == "CU" || tmpRegId == "NU")
                          {
                ...
                hDnList.Clear();
                //Vincent 2013-08-30 Get ProductCategory
                IPartRepository ipartRep = RepositoryFactory.GetInstance().GetRepository<IPartRepository, IPart>();
                IList<ConstValueInfo> prodCategory = ipartRep.GetConstValueInfoList(new ConstValueInfo() { type = "ProcReg" });
                ...
                foreach (DictionaryEntry de in tmpHT)
                {
                    ...
                    //Vincent 2013-08-30 Check setting ProductCategory
                    var existSKUProduct =( from item in prodCategory
                                          where Regex.IsMatch(ele.Model, item.value)
                    ...
                    if(existSKUProduct.Count>0 && !(((string)ele.HDeliveryInfo["PartNo"]).StartsWith(ele.Model + "/")))
                    {
                        continue;
                    }
        ...
        private IList<DNForUI> UploadDataPLNormal(IList<string> dnLines, IList<string> pnLines, string editor)
        {
            ...
            try
            {
                ...
                foreach (string tmp in dnLines)
                {
                    ...
                    //Vincent 2013-08-30 Check setting ProductCategory
                    var existSKUProduct = (from item in prodCategory
                                                            where ele.Model.Length>11 && 
                                                                       Regex.IsMatch(ele.Model.Substring(0,12), item.value) && 
                                                                       item.description.Trim().ToUpper()=="Y"
                                                            select item.name).ToList();

                    //<UC:a>(1)只抓取Delivery.Model前2位=PC的记录
                    //if (!ele.Model.StartsWith("PC"))
                    if (existSKUProduct.Count==0)
                    {
                        continue;
                    }
                ...
                foreach (string tmp in pnLines)
                {
                    ...
                    if (ele.PalletType.StartsWith("ZC") && !ele.PalletNo.StartsWith("BA") && !ele.PalletNo.StartsWith("NA"))
                    {
                        ...
                        bCarrier = (carrier == "BNAF" || carrier == "DZNA" || carrier == "NPN9" || carrier == "EXDO");
                        bShRegId = (regid == "SNA" || regid == "SAF" || regid == "SCA");
                        bCqRegId = (regid == "QNA" || regid == "QAF" || regid == "QCA");
        ...
        private IList<DNForUI> UploadDataPLDomestic(IList<string> dnLines, IList<string> pnLines, string editor)
        {
            ...
            try
            {
                ...
                foreach (string tmp in dnLines)
                {
                    ...
                    if (existSKUProduct.Count > 0 && !ele.Model.Contains("/"))
                    {
                        continue;
                    }
               ...
               //     if ((string)ele.HDeliveryInfo["RegId"] != "SCN")
                  //  if (regId!="CN")
                    ...
                    */
                    ...
                    hDnList.Add(ele.DeliveryNo, ele);
                    bCarrier = tmp.Contains("~EXDO~") || tmp.Contains("~BNAF~") || tmp.Contains("~DZNA~");
                    bShRegId = tmp.Contains("~SCA~") || tmp.Contains("~SNA~") || tmp.Contains("~SAF~");
                    bCqRegId = tmp.Contains("~QCA~") || tmp.Contains("~QNA~") || tmp.Contains("~QAF~");
        ...
        private IList<DNForUI> UploadDataOB(string type, IList<string> dnLines, IList<string> pnLines, string editor)
        {
            ...
            try
            {
                ...
                foreach (string tmp in dnLines)
                {
                    ...
                    if (ele.region != null && ele.region.Length == 3)
                    { 
                        regId = ele.region.Substring(1, 2);
                    }
                    ...
                    else
                    {
                     //   if (ele.region == "SCN") ele.ship_mode = "Truck";
                        //if (regId == "CN") ele.ship_mode = "Truck";
                        if (ActivityCommonImpl.Instance.CheckDomesticDN(regId)) ele.ship_mode = "Truck";
                    }
                    dotDNList.Add(ele);
                ...
                //檢查Comn.txt 裡的DeliveryNo 不存在paltno.txt
                //mantis 1674: PO data upload(OB) 上传船务之后显示dn的qty为0
                var pnDnList =(from p in palletList
                                         select p.internalID.Trim()).ToList();
                foreach (string tmp in dnList)
                {

                    //if (palletList.Where(t => t.internalID==tmp).ToList().Count==0) 
                    if (!pnDnList.Contains(tmp))
                    {
                        throw new FisException("CHK511", new string[] { tmp });
                    }
                }
                ...
                if (currentSession == null)
                {
                    ...
                    currentSession.AddValue("TextRecordCount", dnLines.Count.ToString());
                    currentSession.AddValue("UploadOKRecordCount", dotDNList.Count.ToString());
        ...
        */
        ...
        public int DeletePLDN(string dn, string editor)
        {
            ...
            try
            {
                ...
                //Docking DN Can not delete 
                DNQueryCondition cond = new DNQueryCondition();
                cond.DeliveryNo = dn;
                int recordCount = 0;
                IList<DNForUI> dockingdnlist= GetDNListbyProcReg(cond, "Docking", out recordCount);
                if (dockingdnlist != null && dockingdnlist.Count > 0)
                {
                    List<string> errpara = new List<string>();
                    errpara.Add(dn);
                    throw new FisException("CQCHK1084", errpara);
                }
                //end
                FisException ex;
                List<string> erpara = new List<string>();
        ...
        public void BatchDeleteOBDN(IList<string> dnList, string editor, out IList<string> fList)
        {
            ...
            try
            {
                ...
                if (currentSession == null)
                {
                    ...
                    currentSession.SetInstance(instance);
                    currentSession.AddValue(Session.SessionKeys.DeliveryNoList, delDNList);
        ...
        public string GetOBUnUploadDnList()
        {
       
            string strSQL = @"Select substring(a.DeliveryNo,1,10)  as DN
                                              from HPIMES.dbo.Delivery a,
                                                   HPIMES.dbo.DeliveryEx b 
                                            where a.DeliveryNo = b.DeliveryNo and
                                                   a.ShipDate >=@dDate and
                                                   b.OrderType='N' 
                                            except
                                            select substring(InternalID,1,10)
                                              from HPEDI.dbo.PAK_PAKComn
                                             where ACTUAL_SHIPDATE>=@sDate ";

            string dDate = DateTime.Now.ToString("yyyy-MM-dd");
            string sDate = DateTime.Now.ToString("yyyyMMdd");
            SqlParameter paraName = new SqlParameter("@dDate", SqlDbType.DateTime);
            paraName.Direction = ParameterDirection.Input;
            paraName.Value = dDate;
            SqlParameter paraName2 = new SqlParameter("@sDate", SqlDbType.VarChar,32);
            paraName2.Direction = ParameterDirection.Input;
            paraName2.Value = sDate;
            ...
            if (dr.HasRows)
            {
                ...
                while (dr.Read())
                {
                    dnList.Add(dr[0].ToString().Trim());

                }
               return string.Join(",",dnList.ToArray());
            }
           return "";
        
        }
        private IList<DNForUI> GetDNListbyProcReg(DNQueryCondition cond, string ModelCategory,out int realCount)
        {
            IList<DNForUI> DNList = deliveryRepository.GetDNListByCondition(cond, out realCount);
            IList<Regex> lstRegex =  CommonImpl.GetInstance().GetRegexByProcReg(ModelCategory);
            IList<DNForUI> retdnlist = new List<DNForUI>();
           foreach (DNForUI tmp in DNList)
           {
               if (CommonImpl.GetInstance().CheckModelByProcReg(tmp.ModelName, ref lstRegex))
               {
                   retdnlist.Add(tmp);
               }
