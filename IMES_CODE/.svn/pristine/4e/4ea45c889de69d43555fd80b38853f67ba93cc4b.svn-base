<%--
 ...
 Description: PCA Test Station For Trans
 Update: 
 Date        Name                 Reason 
 ==========  ==================   ==============================
 2012/08/03  Kaisheng             Create
 
 Known issues:
 --%>

<%@ MasterType VirtualPath="~/MasterPage.master" %>
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master"  ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="PCATestStationTrans.aspx.cs" Inherits="SA_PCATestStationTrans" %>
...
<div>
    ...
    <tr>
	    ...
	    <td colspan="4"><iMES:CmbTestStation ID="cmbTestStation" runat="server" Width="100" IsPercentage="true"/></td>
	    <td align="right" rowspan="2" colspan="2"><asp:Label ID="lblPromptTrans" runat="server" CssClass="iMes_label_30pt_Red" /></td>
    </tr>
    <tr>
	    <td style="width:12%" align="left"><asp:Label ID="lbpdLine" runat="server" CssClass="iMes_label_13pt" /></td>
	    <td colspan="4"><iMES:CmbPdLine ID="cmbPdLine" runat="server" Width="100" IsPercentage="true"/></td>
    </tr>
    <tr>
	    <td colspan="7">
	    <hr>
    ...
    <tr>
	    ...
	    <td align="left" colspan="2" nowrap="noWrap"><asp:Label ID="txt111" runat="server"  CssClass="iMes_label_11pt" /></td>
	    <td></td>
    </tr>
    </table>
    <table border="0" width="98%">
    <tr>
        <td style="width:12%" align="left"><asp:Label ID="lbDefectList" runat="server" CssClass="iMes_label_13pt" /></td>
	   ...
	   <td style="width:48%">
          ...
          <ContentTemplate>
	         <iMES:GridViewExt ID="gridviewMB" runat="server" AutoGenerateColumns="False" AutoHighlightScrollByValue="true" Width="99.9%"
                ...
                <Columns>
                    <asp:BoundField DataField="MBCode" SortExpression="DefectId" />
                    <asp:BoundField DataField="PassQty" SortExpression="DefectDescr" />
                </Columns>
             </iMES:GridViewExt>
            </ContentTemplate>   
          </asp:UpdatePanel> 
    ...
    try {
        ...
        if (!errorFlag) 
        {
            ...
            else if (document.getElementById("<%=txtMBSn.ClientID%>").innerText == "") 
            {
                ...
                {
                    //if (((inputID.length == 10) || (inputID.length == 11)) && (inputID.substring(4, 5) == "M"))
                    ...
                    if ((inputID.length == 10) || (inputID.length == 11)) //MBSN
                    {
                        ...
                        //            </bug>
                        WebServicePCATestStation.inputMBCodeTrans(getPdLineCmbValue(), getTestStationCmbValue(), sFirstInput, document.getElementById("<%=useridHidden.ClientID%>").value, document.getElementById("<%=customerHidden.ClientID%>").value, onMBSucceed, onMBFail);
                        //WebServicePCATestStation.inputMBCodeForLot(getPdLineCmbValue(), getTestStationCmbValue(), sFirstInput, document.getElementById("<%=useridHidden.ClientID%>").value, document.getElementById("<%=customerHidden.ClientID%>").value, onMBSucceed, onMBFail);  //SubStringSN(inputID, "MB")
                     }
                     else
                     {
                        //invalidMBSN
            ...
            else 
            {
                ...
                if (inputID == "9999") 
                {
                    ...
                    beginWaitingCoverDiv();
                    WebServicePCATestStation.savePageTrans(document.getElementById("<%=txtMBSn.ClientID%>").innerText, saveDefectArray, onSaveSucceed, onSaveFail);
                } 
                else 
                {
...
{
    //var mbcode = document.getElementById("<%=txtMBSn.ClientID%>").innerText;
    //mbcode = mbcode.substr(0, 2);
    var mbcodeAll = document.getElementById("<%=txtMBSn.ClientID%>").innerText;
    var mbcode = "";
    //mbcode = mbcode.substr(0, 2);
    if (mbcodeAll.substr(4, 1) == "M" || mbcodeAll.substr(4, 1) == "B")
        mbcode = mbcodeAll.substr(0, 2);
    else if (mbcodeAll.substr(5, 1) == "M" || mbcodeAll.substr(5, 1) == "B")
...
//Addby Kaisheng 2012/05/22  -Uc Update
function CheckStation191A() 
{
    ...
    else if ((curStation == '03') && (saveDefectArray.length == 0)) 
    {
        sCkeckresult = meserrStation19;
    }
    //-----end---------    
