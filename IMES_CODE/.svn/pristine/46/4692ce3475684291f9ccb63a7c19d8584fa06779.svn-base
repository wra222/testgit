...
namespace IMES.Station.Implementation
{
    ...
    public class PalletVerify_BIRCH : MarshalByRefObject, IPalletVerify_BIRCH
    {
        ...
        public ArrayList GetCustsnListByBoxId(string sn)
        {
            ...
            if (objCarton != null)
            {
                ...
                foreach (CartonProduct cartonPrd in lstCartonPrd)
                {
                    var product = CommonImpl.GetProductByInput(cartonPrd.ProductID, CommonImpl.InputTypeEnum.ProductIDOrCustSN);
                    {
                        lstSn.Add(product.CUSTSN);
                    }
                }
        ...
        private bool CheckCTO(string model)
        {
            bool isCTO = false;
            int tmp;
            if (int.TryParse(model.Substring(6, 1), out tmp) && model.Substring(0, 2) == "PC")
            { isCTO = true; }
            return isCTO;
        }
        private bool CheckCTObyDN(string dn)
        {
             IDeliveryRepository iDeliveryRepository = RepositoryFactory.GetInstance().GetRepository<IDeliveryRepository, Delivery>();
             string BTRegId = iDeliveryRepository.GetDeliveryInfoValue(dn, "RegId");
             string BTShipTp = iDeliveryRepository.GetDeliveryInfoValue(dn, "ShipTp");
             string BTCountry = iDeliveryRepository.GetDeliveryInfoValue(dn, "Country");
             if (BTCountry != null) { BTCountry = BTCountry.ToUpper(); }
            bool isCTO = false;
