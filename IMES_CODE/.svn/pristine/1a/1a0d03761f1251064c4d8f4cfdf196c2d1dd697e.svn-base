...
{
    /// <summary>
    /// IPAQCRepairForDocking接口的实现类
    /// </summary>
    public class PAQCRepairForDockingImpl : MarshalByRefObject, IPAQCRepairForDocking 
    {
        ...
        {
            logger.Debug("(PAQCRepairForDockingImpl)InputProdId start, [pdLine]:" + pdLine
                + " [prodId]: " + prodId
                + " [editor]:" + editor
                + " [station]:" + stationId
                + " [customer]:" + customer);
            ...
            try
            {
                ...
                if (session == null)
                {
                    ...
                    RouteManagementUtils.GetWorkflow(stationId, "PAQCRepairForDocking.xoml", "PAQCRepairForDocking.rules", out wfName, out rlName);
            ...
            {
                logger.Debug("(PAQCRepairForDockingImpl)InputProdId end, [pdLine]:" + pdLine
                    + " [prodId]: " + prodId
                    + " [editor]:" + editor
                    + " [station]:" + stationId
                    + " [customer]:" + customer);
        ...
        {
            logger.Debug("(PAQCRepairForDockingImpl)Edit start, [prodId]:" + prodId
                + " [rll]: " + rll);   
            FisException ex;
            List<string> erpara = new List<string>();
            string sessionKey = prodId;
            ...
            {
                logger.Debug("(PAQCRepairForDockingImpl)Edit end, [prodId]:" + prodId
                    + " [rll]: " + rll);               
            }
        }
        ...
        {
            logger.Debug("(PAQCRepairForDockingImpl)Add start, [prodId]:" + prodId
                + " [rll]: " + rll); 
            FisException ex;
            List<string> erpara = new List<string>();
            string sessionKey = prodId;
            ...
            {
                logger.Debug("(PAQCRepairForDockingImpl)Add end, [prodId]:" + prodId
                    + " [rll]: " + rll);             
            }
        }
        ...
        {
            logger.Debug("(PAQCRepairForDockingImpl)Save start, [prodId]:" + prodId);
            FisException ex;
            List<string> erpara = new List<string>();
            string sessionKey = prodId;
            ...
            {
                logger.Debug("(PAQCRepairForDockingImpl)Save end, [prodId]:" + prodId);            
            }
        }
        */
        ...
        */
        ...
        public List<string> Save(string prodId, string returnStation, string returnStationText)
        {
            ...
            try
            {
                ...
                IProduct currentProduct1 = (IProduct)session.GetValue(Session.SessionKeys.Product);
                session.AddValue(Session.SessionKeys.DeliveryNo, currentProduct1.DeliveryNo);

                //2012-8-1, Jessica Liu, 闇€姹傚彉鏇达細澧炲姞Return Station 璁＄畻閫昏緫, 淇敼瑙N鏉′欢
        ...
        {
            logger.Debug("(PAQCRepairForDockingImpl)Cancel start, [prodId]:" + prodId);
            FisException ex;
            List<string> erpara = new List<string>();
            string sessionKey = prodId;
            ...
            {
                logger.Debug("(PAQCRepairForDockingImpl)Cancel end, [prodId]:" + prodId);
            }
        }
