function MzTreeView(Tname)
{
 ...
 //for tabledata................
  ...
  this.focusNode = null;//when this.enableDblClick is true, this currentNode != this.focusNode.
 //add by lzy
        ...
        if(e.id.indexOf(this.name +"_icon_")==0)
		  {
		    if (this.enableDblClick){
                ...
                this.hilightNode(e.id.substr(e.id.lastIndexOf("_") + 1));
                break;
              }
...
//changed by lzy
//only hilight the clicked node, but don't call the outer function.
MzTreeView.prototype.hilightNode= function(id)
{
    var a = this.getElementById(this.name +"_link_"+ id);
    ...
    if(a)
    {
        ...
        this.focusNode = this.node[id];//when dblclick is enabled, this currentNode != this.focusNode.
...
MzTreeView.prototype.dblClickHandle = function(e)
{
  ...
  if (!this.enableDblClick)
  {
   ...
   }else {//dblclick to select the node
    if(e.id)
    {
      if(e.tagName=="IMG")
      { 
...
//fresh parent node and locate the new inserted node,2009-05-04
//before calling this method, user must manual add new node info to user's source array.
MzTreeView.prototype.insertNode  = function(newUUID)
{	
    this.freshPath = new Array(); 
    this.freshPath[this.freshPath.length] = newUUID;
	
	var id = this.currentNode.parentId; 
...
//delete the selected node, if no Nodes then empty the parentnode and 
//focus on the parentNode,2009-05-04
MzTreeView.prototype.deleteNode  = function()
{	
    if (this.currentNode.parentNode.childNodes.length == 1)
    {
        this.emptyNode(this.currentNode.parentNode);
        return true;
...
MzTreeView.prototype.addNodeClient  = function(id, data)
{	
	if (id == "")
...
MzTreeView.prototype.deleteNodeClient = function()
{
...
//删除最后一个节点的时候，前一个节点的子节点的前边的竖线要去掉
...
{ 
	this.recordNodePath(this.currentNode,1000, key);
	
	return this.freshPath;

}

MzTreeView.prototype.freshRootNode = function(key)
{	  
    var node = this.currentNode,backLayer;    
    //var node = this.findRootNode(), backLayer;
    this.freshPath = new Array();
	...
	{ 	 		
	    var source = this.nodes[node.sourceIndex];
        this.freshPath[this.freshPath.length] = this.getAttribute(source, key);
...
MzTreeView.prototype.recordNodePath = function(startNode, backLayer, key)
{
	var node = startNode;
