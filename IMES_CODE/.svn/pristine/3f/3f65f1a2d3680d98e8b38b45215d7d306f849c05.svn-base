    ...
    public class FA_ProductEfficiency : MarshalByRefObject, IFA_ProductEfficiency
    {
                             ...
                             string Family, IList<string> Model, IList<string> Station, IList<string> Line)
        {
            ...
            try
            {
                ...
                //sb.AppendLine("WHERE a.Stage IN ('FA' ,'PAK') AND c.Station IN('40','67','69','81') ");//  '40','67','69','81' ,'40','65','85'

                sb.AppendLine(string.Format("WHERE a.Stage IN ('FA' ,'PAK') AND c.Station IN('{0}') ", string.Join("','", Station.ToArray())));
                if (Line.Count != 0)
                {
                    sb.AppendLine(string.Format("AND a.Line IN('{0}') ", string.Join("','", Line.ToArray())));
                }
                ...
                sb.AppendLine("FOR Line IN ([A],[B],[C],[D],[E],[F],[G],[H],[J],[K],[L],[M],[R],[TT]) ");
                sb.AppendLine(" )");
                ...
                sb.AppendLine("[A],[B],[C],[D],[E],[F],[G],[H],[J],[K],[L],[M],[R], ");
                sb.AppendLine("(isnull([A],0)+isnull([B],0)+isnull([C],0)+isnull([D],0)+isnull([E],0)+ ");
                sb.AppendLine("isnull([F],0)+isnull([G],0)+isnull([H],0)+isnull([J],0)+isnull([K],0)+ ");
                sb.AppendLine("isnull([L],0)+isnull([M],0)+isnull([R],0)) as [TT] ");
                ...
                sb.AppendLine("sum([A]),sum([B]),sum([C]),sum([D]),sum([E]),sum([F]),sum([G]),sum([H]),sum([J]),sum([K]),sum([L]),sum([M]),sum([R]), ");
                sb.AppendLine("(isnull(sum([A]),0)+isnull(sum([B]),0)+isnull(sum([C]),0)+isnull(sum([D]),0)+isnull(sum([E]),0)+ ");
                sb.AppendLine("isnull(sum([F]),0)+isnull(sum([G]),0)+isnull(sum([H]),0)+isnull(sum([J]),0)+isnull(sum([K]),0)+ ");
                sb.AppendLine("isnull(sum([L]),0)+isnull(sum([M]),0)+isnull(sum([R]),0)) ");
