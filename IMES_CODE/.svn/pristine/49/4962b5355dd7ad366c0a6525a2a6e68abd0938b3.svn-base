...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="OQCRepair.aspx.cs" Inherits="FA_OQCRepair" Title="无标题页" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" Runat="Server">
    ...
    <script language="javascript" type="text/javascript">
        ...
        function processFun(backData)
        {
           ...
           backData= Get2DCodeCustSN(backData);
        ...
        function clkSave()
        {
            ...
            if (inputFlag)
            {
                ...
                if (isFinishAllRepair())
                {
                    /* 2012-8-1, Jessica Liu, 需求变更：增加Return Station 计算逻辑, 修改解DN条件
                    beginWaitingCoverDiv();
                    OQCRepairService.save(globalProID, saveSucc, saveFail);
                    */
                    ...
                    if (!emptyPattern.test(returnStation)) {
                        ...
                        OQCRepairService.save(globalProID, returnStation, returnStationText,saveSucc, saveFail);
                    }
                    else {
                        ShowMessage(msgSelectReturnStation);
                        ShowInfo(msgSelectReturnStation);
...
//            editObj.cause = tblObj.rows[selectedRowIndex].cells[4].innerText;   
        ...
        function OnCancel()
        {
            OQCRepairService.cancel(globalProID);
        }
