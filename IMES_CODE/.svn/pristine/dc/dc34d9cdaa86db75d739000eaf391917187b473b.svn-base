...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master"  ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="PCATestStation.aspx.cs" Inherits="SA_PCATestStation" %>
...
var mesCanOnlyScanDefec = '<%=this.GetLocalResourceObject(Pre + "_mesCanOnlyScanDefec").ToString()%>';
        ...
        if (!errorFlag) 
        {
            ...
            else if (document.getElementById("<%=txtMBSn.ClientID%>").innerText == "") 
            {
                ...
                else 
                {
                    ...
                    {
                        var sFirstInput = "";
                        if ((inputID.length == 11) && (inputID.substr(5, 1) == "M" || inputID.substr(5, 1) == "B")) {
                            sFirstInput = inputID.substr(0, 11);
                        }
                        else if ((inputID.length == 11) && (inputID.substr(4, 1) == "M" || inputID.substr(4, 1) == "B")) {
                        ...
                        //            </bug>
                        WebServicePCATestStation.inputMBCode(getPdLineCmbValue(), getTestStationCmbValue(), sFirstInput, document.getElementById("<%=useridHidden.ClientID%>").value, document.getElementById("<%=customerHidden.ClientID%>").value, onMBSucceed, onMBFail);
            ...
            else 
            {
                ...
                if (inputID == "9999") 
                {
                    ...
                    beginWaitingCoverDiv();
                    WebServicePCATestStation.savePage(document.getElementById("<%=txtMBSn.ClientID%>").innerText, saveDefectArray, onSaveSucceed, onSaveFail);
                } 
                else 
                {
    ...
    try {
        ...
        if (result == null) {
            ...
            if (result[6] == "NO!") {
                //alert(AllowPass);
                //alert(result[5]);
                if (AllowPass == 'N' && result[5] == '10A') {
                    ShowMessage(mesCanOnlyScanDefec);
                    ShowInfo(mesCanOnlyScanDefec);
                    //getAvailableData("processDataEntry");
                    callNextInput();
                    return;
                }
...
function CheckStation191A() 
{
    ...
    //-----end---------  
    //Vincent 2013-12-20 for CQ don't check Fru Station  
