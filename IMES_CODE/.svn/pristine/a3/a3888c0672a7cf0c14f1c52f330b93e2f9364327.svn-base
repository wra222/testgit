...
namespace IMES.Query.Implementation
{
    public class SA_PCBTestReport : MarshalByRefObject, ISA_PCBTestReport
    {
        ...
        public DataTable GetPCBTestReportResult(string Connection, DateTime FromDate, DateTime ToDate, IList<string> lstFamily, IList<string> lstTestItem, IList<string> lstFixtureID, IList<string> lstStation, IList<string> lstOP)
        {
            ...
            try
            {
                ...
                sb.AppendLine("SELECT a.ActionName, a.PCBNo,a1.MAC,a.[Type], ");
                ...
                sb.AppendLine("a.FixtureID,a.Station,a.Line as PdLine , a1.PCBModelID, a.ErrorCode,isnull(c.Descr,a.Descr) as Descr, a.Editor, ");
                ...
                if (lstFamily.Count > 0)
                {
                    ...
                    {
                        if (lstFamily[i].Length == 2)
                        {
                            MB_PC.Add(lstFamily[i]);
                        }
                        else if (lstFamily[i].Length == 3)
                        {
                            MB_AIO.Add(lstFamily[i]);
                        }
                    }

                    sb.AppendLine(string.Format("AND ( LEFT(a1.PCBNo,2) in ('{0}') ", string.Join("','", MB_PC.ToArray())));
                    sb.AppendLine(string.Format("or LEFT(a1.PCBNo,3) in ('{0}') ) ", string.Join("','", MB_AIO.ToArray())));
                }
                ...
                {
                    sb.AppendLine(string.Format("AND a.Station in ('{0}') ", string.Join("','", lstStation.ToArray())));
                }
                if (lstOP.Count > 0)
                {
                    sb.AppendLine(string.Format("AND a.Editor in ('{0}') ", string.Join("','", lstOP.ToArray())));
                }
