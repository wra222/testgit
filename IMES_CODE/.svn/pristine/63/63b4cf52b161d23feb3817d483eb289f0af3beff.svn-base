...
<script type="text/javascript">
    ...
    var isNeedInputCartonPartNo = false;
    var arrCartonPartNo;
    var cartonPn = "";
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //| Name		:	onload
        ...
        if (uutInput) 
        {
            ...
            {              
                ...
                {                  
                    ...
                    WebServiceSNCheck.inputCustSNOnProduct2(inputObj.value, line, "<%=UserId%>", station, "<%=Customer%>", onFSNSucceed, onFSNFail);
                }
                
            }
            else {

                //Check  Carton Part No
//                if (isNeedInputCartonPartNo && !CheckCartonPn(inputObj.value)) {
//                    alert("Please input correct Carton Part No!");
//                    getAvailableData("processDataEntry");
//                    inputObj.value = "";
//                    inputObj.focus();
//                    return;
...
//                    ShowInfo(msgScanAnotherSN);
                //                }
                if (isNeedInputCartonPartNo) {
                    if (!CheckCartonPn(inputObj.value)) {
                        ShowInfo("Wrong Carton Part No!");
                        ShowMessage("Wrong Carton Part No!");
                    }
                    else {
                        ShowInfo(msgScanAnotherSN);
                    }
    ...
    function CheckCartonPn(data) {
     
        for (var key in arrCartonPartNo) {
            if (arrCartonPartNo[key] == data)
            { isNeedInputCartonPartNo = false;cartonPn = data;  return true; }
        }
        return false;
    }
        ...
        try {
            ...
            else if ((result.length ==4) && (result[0] == SUCCESSRET)) 
            {
                ...
                if (result[3].length > 0) {
                    isNeedInputCartonPartNo = true;
                    arrCartonPartNo = result[3];
                    ShowInfo("Please scan Carton PartNo"); 
                }
                else {
                    ShowInfo(msgScanAnotherSN); 
                }
    ...
    function onCheckSucceed(result) {
        ...
        try {
            ...
            if (result == null) {
                ...
                if (IsChinaEnergyLabel) {
                    var printItemlist = getPrintItemCollection();
                    WebServiceSNCheck.inputCustSNOnPizzaReturnPrint(custsnOnPizza, custsnOnProduct, line1, "<%=UserId%>", station, "<%=Customer%>",cartonPn, printItemlist, onSucceedPrint, onFail);
                }
                else
                    WebServiceSNCheck.inputCustSNOnPizzaReturn(custsnOnPizza, custsnOnProduct, line1, "<%=UserId%>", station, "<%=Customer%>", cartonPn,onSucceed, onFail);
            }
            else {
                ShowInfo("");
                endWaitingCoverDiv();
        ...
        try {
            ...
            if (result == null) {
                ...
                for (var i in result[1]) {
                    if (result[1][i].LabelType == "EnergyLabel") {
                        prePrint = true;
                    }
                }
                if (prePrint) {
                    successInfo = "[" + custsnOnProductValue + "] " + msgSuccess + "  " + result[2] + "。 Begin to print.";
                    ShowSuccessfulInfo(true, successInfo);
                    setPrintItemListParam1(result[1], custsnOnProductValue);
                    printLabels(result[1], true);
                }
                else {
                    var content = "Error Getting Print setting!";
                    ShowMessage(content);
                    ShowInfo(content);
                }
            }
