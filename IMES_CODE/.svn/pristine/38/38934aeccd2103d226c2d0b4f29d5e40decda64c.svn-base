...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" ContentType="text/html;Charset=UTF-8"
    AutoEventWireup="true" CodeFile="ReprintCombinPoInCartonForRCTO.aspx.cs" Inherits="SA_ReprintCombinPoInCarton"
    Title="Untitled Page" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" runat="Server">
    ...
    <script type="text/javascript" language="javascript">
        ...
        function clkReprint() {
            ...
            else  if (inputData.length == 14 || inputData.length == 15 || inputData.length == 18) {
                inputData = inputData; //Vendor CT
            }
            ...
            WebServiceCombinePoInCartonForRCTO.ReprintCartonLabel(inputData, reason, "", editor, station, customer, lstPrintItem, onPrintSucceed, onPrintFail); 
        }
        ...
        function onPrintSucceed(result) {
            ...
            try {
                if (result == null) {
                    ...
                    if (printflag == "Y")
                    {
                        setPrintItemListParam(result[1], result[3]); // 使用Reprint的存储过程
                        //==========================================print process=======================================
                    ...
                    //ResetPage();
                    customerSN = result[5];
                    var cartonSn = result[3];
                    if (customerSN.trim() !="")
                        ShowSuccessfulInfoFormat(true, "Customer SN", customerSN); // Print 成功，带成功提示音！
                    else {
                        ShowSuccessfulInfoFormat(true, "CartonSN", "CartonSN:" + cartonSn); // Print 成功，带成功提示音！
                    }
                }
                else {
                    ResetPage();
