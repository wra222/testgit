...
public partial class Query_SA_PCBNoQuery: IMESQueryBasePage
{
   ...
   // IMES.Query.Interface.QueryIntf.IFamily Family = ServiceAgent.getInstance().GetObjectByName<IMES.Query.Interface.QueryIntf.IFamily>(WebConstant.IFamilyBObject);
    ...
    ISA_PCBNoQuery SA_PCBNoQuery = ServiceAgent.getInstance().GetObjectByName<ISA_PCBNoQuery>(WebConstant.ISA_PCBNoQuery);

    public string[] GvQueryColumnName = { "OldPCBNo", "NewPCBNo", "Cdt" };
    public int[] GvQueryColumnNameWidth = { 80, 80, 100};
    ...
    protected void btnQuery_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (txtNewPCBNo.Text != "")
            {
                PCBNo = txtNewPCBNo.Text;
            }
            else
            {
                PCBNo = hidModelList.Value;
            }

            DataTable dt = new DataTable();
            //int sum = 0;
            dt = SA_PCBNoQuery.GetPCBNoQueryResult(DBConnection, DateTime.Parse(txtFromDate.Text), DateTime.Parse(txtToDate.Text),PCBNo);
