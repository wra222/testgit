...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="AssignModelRePrint.aspx.cs" Inherits="FA_AssignModelRePrint" %>
...
<div>
    <asp:ScriptManager ID="ScriptManager1" runat="server" >
        <Services>
            <asp:ServiceReference Path="~/FA/Service/AssignModel.asmx" />
        </Services>
    </asp:ScriptManager>
    <center>
            ...
            if (!errorFlag) {
                ...
                try {
                   ...
                   //     RePrint(string sn, string reason, string line, string editor, string station, string customer, IList<PrintItem> printItems)
                        AssignModel.RePrint(inpuProdid, reason, line, editor, station, customer, lstPrintItem, onSucceed, onFail);
                    }
                }
                catch (e1) {
    ...
    function onSucceed(result) {
        try {
            ...
            for (var i = 0; i < result[1].length; i++) {
                ...
                setPrintItemListParam(labelCollection, result[1][i].LabelType, result[2]);
                printLabels(labelCollection, false);

            }
//..
          //      setPrintItemListParam(result[1], result[1][0].LabelType, inpuProdid);
		      //  setPrintItemListParam(result[1], result[1][0].LabelType, result[2]);
