...
public class PalletWeightWebService  : System.Web.Services.WebService 
{
    ...
    public ArrayList InputPallet(string inputID, string actualWeight,string type, string line, string editor, string station, string customer)
    {
        ...
        try
        {
           ArrayList tempList = palletweightManager.InputPallet(inputID, decimal.Parse(actualWeight),type, 
            ...
            retlist.Add(tempList[8]); //checkType 是否检查pallettype
        }
    ...
    [WebMethod]
    public ArrayList InputCustSN(string custSN, string palletNo,string palletType)
    {
        ArrayList retlist = null;
        try
        {
            String customer = palletweightManager.InputCustSN(custSN, palletNo, palletType);
            retlist = new ArrayList();
            retlist.Add(WebConstant.SUCCESSRET);
    ...
    [WebMethod]
    public ArrayList InputPallet_CQ(string inputID, string actualWeight, string type, string line, string editor, string station, string customer)
    {
        ArrayList retlist = new ArrayList();

        try
        {
            ArrayList tempList = palletweightManager.InputPallet_CQ(inputID, decimal.Parse(actualWeight), type,
                                                     line, editor, station, customer);
