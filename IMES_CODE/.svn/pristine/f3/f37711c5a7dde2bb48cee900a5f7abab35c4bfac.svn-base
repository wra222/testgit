        ...
        static public WindowsImpersonationContext ImpersinateUser(string sUsername, string sDomain, string sPassword, Log log)
        {
            ...
            try
            {
                ...
                if (false == bImpersonated)
                {
                    ...
                    log.write(LogType.error, 0, "LogOn", "LogTest", "Log On failed with error code" + nErrorCode);
                    return null;
                }
                ...
                if (false == bRetVal)
                {
                    ...
                    log.write(LogType.error, 0, "LogOn", "LogTest", "DuplocateToken() failed with error code" + nErrorCode);
                    return null;
                }
                else
                {
            ...
            {
                log.write(LogType.error, 0, "LogOn", "LogFaile", "Logon Fail:" + ex.Message);
            }
            finally
            {
