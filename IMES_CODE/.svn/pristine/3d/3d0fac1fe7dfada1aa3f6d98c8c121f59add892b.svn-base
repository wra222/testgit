...
namespace IMES.Activity
{
    ...
    public partial class UpdateMBStatus : BaseActivity
    {
        ...
        protected internal override ActivityExecutionStatus DoExecute(ActivityExecutionContext executionContext)
        {
            ...
            {
                status = new MBStatus(mb.Key.ToString(), station, this.IsPass, mb.MBStatus.TestFailCount, this.Editor, line, DateTime.Now, DateTime.Now);
            }
            else
            {
                if (mb.MBStatus.Station == station && this.IsPass == MBStatusEnum.Fail)  //檢查相同測試站需累計測試次數
                {
                    status = new MBStatus(mb.Key.ToString(), station, this.IsPass, mb.MBStatus.TestFailCount + 1, this.Editor, line, DateTime.Now, DateTime.Now);
                }
                else if (this.IsPass == MBStatusEnum.Fail)
                {
                    status = new MBStatus(mb.Key.ToString(), station, this.IsPass, 1, this.Editor, line, DateTime.Now, DateTime.Now);
                }
                else
                {
