...
namespace IMES.CheckItemModule.ODD.Filter
{
    [Export(typeof(IFilterModule))]
    [ExportMetadata("ProgramName", "IMES.CheckItemModule.ODD.Filter.dll")]
    public class Filter : IFilterModule, ITreeTraversal
    {
        private const string part_check_type = "ODD";
        ...
        {
            //不同的是(Descr like 'CD%' or  Descr like 'DVD%'or Descr like 'ODD%' or Descr like 'COMBO%'or  Descr like 'VCD%'）
            if (((BOMNode)node).Part == null)
            {
                return false;
            }
            bool is_contain_cd = ((BOMNode)node).Part.Descr.Trim().Contains("CD");
            int start_point_cd = ((BOMNode)node).Part.Descr.Trim().IndexOf("CD");
            bool is_contain_dvd = ((BOMNode)node).Part.Descr.Trim().Contains("DVD");
            int start_point_dvd = ((BOMNode)node).Part.Descr.Trim().IndexOf("DVD");
            bool is_contain_odd = ((BOMNode)node).Part.Descr.Trim().Contains("ODD");
            int start_point_odd = ((BOMNode)node).Part.Descr.Trim().IndexOf("ODD");
            bool is_contain_combo = ((BOMNode)node).Part.Descr.Trim().Contains("COMBO");
            int start_point_combo = ((BOMNode)node).Part.Descr.Trim().IndexOf("COMBO");
            bool is_contain_vcd = ((BOMNode)node).Part.Descr.Trim().Contains("VCD");
            int start_point_vcd = ((BOMNode)node).Part.Descr.Trim().IndexOf("VCD");
            if ((is_contain_cd && start_point_cd == 0) || (is_contain_dvd && start_point_dvd == 0) || (is_contain_odd && start_point_odd == 0) || (is_contain_combo && start_point_combo == 0) ||
                (is_contain_vcd && start_point_vcd == 0))
                return true;
            return false;
        }
