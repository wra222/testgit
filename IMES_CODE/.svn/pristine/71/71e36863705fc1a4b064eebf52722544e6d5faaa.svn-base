...
namespace IMES.Station.Implementation
{
    ...
    { 
        ...
        /// Update COA
        /// </summary>
        public void UpdateCOAList(List<string> lotCoaList, string editor, string status, string pdLine, string station)
        {
            logger.Debug("(_COAStatusChange)UpdateCOAList start." );
            try
            {
                foreach (string tempCoa in lotCoaList)
                {
                
                    string pno = "";
                    string preStatus = "";
                    IList<COAStatus> coaList = currentRepository.GetCOAStatusRange(tempCoa, tempCoa);
                    foreach (COAStatus tmp in coaList)
                    {
                        string preLine = "";
                        if ("" == pno)
                    ...
                    COATransLog newLog = new COATransLog();
                    newLog.begNo = tempCoa;
                    newLog.endNo = tempCoa;
                    newLog.cdt = DateTime.Now;
                    newLog.status = status.Substring(0, 2);
                    newLog.editor = editor;
                    newLog.pno = pno;
            ...
            {
                logger.Debug("(_COAStatusChange)UpdateCOAList end.");
            }
        }
        #endregion
    }
