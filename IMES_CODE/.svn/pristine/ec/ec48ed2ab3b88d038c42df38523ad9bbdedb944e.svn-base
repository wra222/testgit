        ...
        protected internal override ActivityExecutionStatus DoExecute(ActivityExecutionContext executionContext)
        {
            ...
            */
            ...
            foreach (IProductPart partNode in productParts)
            {
                ...
                if (partNode.BomNodeType == "AT" && (partNode.PartType == "ATSN3" || partNode.PartType == "ATSN5")) bCDSICond2 = true;
            ...
            //3) 如果Product_Part. PartSn where ProductID=@prdid and PartNo in (bom中BomNodeType=’AT’  and Descr like ’ATSN%’) 不存在记录，且通过Model 检查 bom中BomNodeType=’AT’  （Descr=’ATSN7’ Or Descr=’ATSN8’）判断存在（ATSN7，ATSN8）并且ModelInfo.Value（ModelInfo.Name=‘Cust’ Model=Prodcut.Model）<>’’，则提示“这个Product需要在线列印AST,go  online create ast”
            //2012-04-09 删除此条件
            /*
            if (!bExistPart && (bExistATSN7 || bExistATSN8))
            {
                if (CurrentProduct.GetModelProperty("Cust") as string != "")
                {
                    throw new FisException("CHK197", errpara);
                }
            }
            */
            ...
            if (bNeedPrintPPLabel)
            {
                PrintLog cond = new PrintLog();
                cond.BeginNo = CurrentProduct.ProId;
                //cond.EndNo = "";
                cond.Name = "AT";
                //cond.Descr = "ATSN";
                ...
                {
                    throw new FisException("CHK523", new string[] { });
                }
            }
