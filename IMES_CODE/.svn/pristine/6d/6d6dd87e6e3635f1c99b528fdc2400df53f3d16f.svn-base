...
public partial class Query_FA_MPInputEx : IMESQueryBasePage
{
    ...
    private void InitCondition()
    {
        ...
        hfPAKCosmetic_SUM.Value = PAKCosmetic_SUM;
        hfCOA_SUM.Value = COA_SUM;

     //   hfProcess_All.Value = FAOnlineStation + "," + PAKOnlineStation;
        hfProcess_All.Value = FAShipmentStation + "," + PAKBTOnlineStation;

        //FAShipmentStation = FindValue(dt, "FAShipmentStation");
        //PAKBTOnlineStation = FindValue(dt, "PAKBTOnlineStation"); 
    ...
    {       
        ...
        try
        {
           ...
           hfShipDate.Value = DateTime.Parse(txtShipDate.Text.ToString()).ToString();
          //  hfToDate.Value = DateTime.Parse(txtToDate.Text.ToString()).ToString();
            ...
            DataTable dt = MPInputEx.GetQueryResult(DBConnection, DateTime.Parse(hfShipDate.Value),
                            lstPdLine, ddlFamily.SelectedValue, Model, StationList, bool.Parse(hfLineShife.Value),"", false, prdType);
    ...
    public void btnQueryDetail_Click(object sender, System.EventArgs e)
    {
        try
        {
            ...
            DataTable dt = MPInputEx.GetSelectDetail(DBConnection, DateTime.Parse(txtShipDate.Text),  lstPdLine, Family, lstModel, Line, Station, bool.Parse(hfLineShife.Value));
    ...
    {
        IFA_MPInputEx mpinput = ServiceAgent.getInstance().GetObjectByName<IFA_MPInputEx>(WebConstant.MPInputEx);
        ...
        int[] dnQty;
        dnQty = mpinput.GetDNShipQty(Connection, shipD, inputModel, PrdType);
