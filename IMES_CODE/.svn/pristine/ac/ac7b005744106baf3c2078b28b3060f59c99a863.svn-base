...
namespace IMES.Activity
{
    ...
    public partial class CheckPCMBRCTOMB : BaseActivity
	{
        ...
        protected internal override ActivityExecutionStatus DoExecute(ActivityExecutionContext executionContext)
        {
            ...
            if (check == "RCTO")
            {
                if (!(mbSNo.Substring(4, 1) == "M" || mbSNo.Substring(4, 1) == "B" ) || (mbSNo.Substring(5, 1) == "M"||mbSNo.Substring(5, 1) == "B"))
                {
                ...
                if (strCheckCode != "R" )
                {
                    ...
                    condition.type = "R";
                    condition.code = mbSNo;
                    IList<RctombmaintainInfo> getRctoInfo = currentMBMORepository.GetRctombmaintainInfoList(condition);
                    if (null == getRctoInfo || getRctoInfo.Count == 0)
                    {
                        List<string> errpara = new List<string>();
                        throw new FisException("CHK158", errpara);
                    }
