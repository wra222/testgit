...
{
	  //alert("getAttribute");
  var reg = new RegExp("(^|" +this.attribute_d + "|\\s)"+ name +"\\s*:\\s*([^"+this.attribute_d +"]*)(\\s|"+this.attribute_d +"|$)", "i"); 
  if (reg.test(source)) {
//    return RegExp.$2.replace(/[\x0f]/g, this.attribute_d ).replace(/\'/g, "&#39"+this.attribute_d +""); return "";
    return RegExp.$2.replace(/[\x0f]/g, this.attribute_d ); return "";//changed by lzy,date:2006-1-12
  }  
};
...
MzTreeView.prototype.focusClientNode      = function(id)
{
 ...
 //add by lzy
 	//if (this.currentNode.id == id || this.loadingdata == true )//modified date:2007-4-12
...
*/
		...
		this.colors.highLightText +"; font-size: 9pt'>Please waiting...</A></NOBR></DIV>";
...
{	 
	...
	{		 
		...
		{	
			if (i <= backLayer - 1) {				
				this.recordFreshPath(node, "UUID");
			}
...
MzTreeView.prototype.recordFreshPath = function( node, key )
{
	var source = this.nodes[node.sourceIndex];
	this.freshPath[this.freshPath.length] = this.getAttribute(source, key);
	//alert("UUID = " + this.getAttribute(source, key))
	return true;
}
...
MzTreeView.prototype.searchInNodes = function (data)
{
	var node = null;			
	var data; 
	...
	{
		alert("Abnormal:no selected node.");
		return false;
 	}
	
	if (!this.currentNode.isLoad)
	{
		alert("The Child info of current node has not been loaded.");
		return false;
 	}  	 	
 	 
 	//clear 
	this.clearHilight(this.node["0"]);	 
	...
	{	 
		...
		var source = this.nodes[node.sourceIndex];
		var nodeText = this.getAttribute(source, "text");
	 
		if (nodeText.indexOf(data) >= 0){					
			//found
...
{
	var sHTML='<div id="divReject"  align="center" style="filter:Chroma(Color=skyblue); background-color: skyblue;; display: none; top: 0; width: 100%; height: 100%; z-index: 10000; position: absolute"></div>';
