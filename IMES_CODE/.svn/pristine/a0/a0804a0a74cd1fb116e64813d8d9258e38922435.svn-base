...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true"
    CodeFile="UnpackOfflinePizza.aspx.cs" Inherits="PAK_UnpackOfflinePizza" Title="Untitled Page" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" runat="Server">
    ...
    <asp:ScriptManager ID="ScriptManager1" runat="server" EnablePartialRendering="true">
        <Services>
            <asp:ServiceReference Path="Service/WebserviceUnpackOfflinePizza.asmx" />
        </Services>
    </asp:ScriptManager>
<div>
   <center >
            ...
            if (sn != "") {
                if (9 <= sn.length){
					if (ConfirmUnpack()){
						...
						WebserviceUnpackOfflinePizza.Unpack(sn, "<%=UserId%>", station, "<%=Customer%>", onSuccess, onFail);
						return;
					}
				}
				else{
        ...
        function onSuccess(result) {
            ...
            try {
                if (result == null) {
                    ...
                    ShowSuccessfulInfo(true, "Pizza Unpack成功");
                }
                inputControl.focus();
                getAvailableData("processDataEntry");
            }
