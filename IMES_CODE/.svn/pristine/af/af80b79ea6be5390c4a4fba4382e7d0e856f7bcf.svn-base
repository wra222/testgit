 ...
 * Description: PCA OQC Output for Docking
...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master"  ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="PCAOQCOutputForDocking.aspx.cs" Inherits="Docking_PCAOQCOutputForDocking" %>

<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" Runat="Server">
    <asp:ScriptManager ID="ScriptManager1" runat="server" >
    <Services>
        <asp:ServiceReference Path="~/Docking/Service/WebServicePCAOQCOutputForDocking.asmx"/>
    </Services>
    </asp:ScriptManager>
    ...
    var msgMBSnoBitSix = '<%=this.GetLocalResourceObject(Pre + "_msgMBSnoBitSix").ToString()%>';
...
//    }
    ...
    function checkMBSno(inputData) {
        ...
        if (inputData.length == 11) {
            if (inputData.substr(4, 1) == "M" || inputData.substr(4, 1) == "B") {
                MBSno = inputData.substr(0,10);
                return true;
            }
            else if (inputData.substr(5, 1) == "M" || inputData.substr(5, 1) == "B") {
                MBSno = inputData;
                return true;
            }
            else {
                alert(msgMBSnoBitSix);
                return false;
            }
        }
...
//                alert(msgDefectNull);
            ...
            inputDefect(inputData);
                ...
                WebServicePCAOQCOutputForDocking.InputMBSno(MBSno, editor, station, customer, onInputSucceeded, onFailed);
        }
        else {
            callNextInput();
        }
    ...
    function save() {
     //   beginWaitingCoverDiv();
        ...
        WebServicePCAOQCOutputForDocking.save(MBSno, defectInTable, saveSucc, onFailed);
    }
    ...
    function ExitPage() {
        if (MBSno != "") {
            WebServicePCAOQCOutputForDocking.Cancel(MBSno);
