...
namespace IMES.Activity
{
    ...
    public partial class UpdateParts : BaseActivity
	{
            ...
            try
            {
                ...
                else if (string.Compare(RepairStationType, "FA", true) == 0 || string.Compare(RepairStationType, "OQC", true) == 0)
                {
                    ...
                    if (string.Compare(defect.PartType, "KP/ME", true) == 0)
                    {
                        ...
                        string partCheckItemType = "";

                        //if old part exist, update Product_Parts table
                        if (part != null)
                        {
                            partCheckItemType = part.CheckItemType;
                            backupProductPartById(part);
                            part = getUpdatedPart(part, true, defect.NewPartSno, defect.PartType, "Product");
                        }

                        if ("FA".Equals(RepairStationType))
                        {
                            if ("CPU".Equals(partCheckItemType))
                            {
                                IProduct tempProduct = (IProduct)CurrentSession.GetValue(Session.SessionKeys.Product);

                                // mantis 538
                                Session session = CurrentSession;
                                if (ActivityCommonImpl.Instance.Material.NeedCheckMaterialCpuStatus(tempProduct, defect.NewPartSno, ref session))
                                {
                                    // 舊料
                                    ActivityCommonImpl.Instance.Material.UpdateMaterialCpuStatus(tempProduct.CVSN, "FA Repair", "Dismantle", true, false, ref session);

                                    // 新料
                                    ActivityCommonImpl.Instance.Material.UpdateMaterialCpuStatus(defect.NewPartSno, "FA Repair", "Assembly", true, true, ref session);
                                }

                                tempProduct.CVSN = defect.NewPartSno;
                                //productRepository.Update(tempProduct, CurrentSession.UnitOfWork);
                    ...
                    else if (string.Compare(defect.PartType, "MB", true) == 0)
                    {
                        ...
                        if (type == "MB")
                        {
                            ...
                            // mantis 628
                            string tmpMbct = newMB.GetExtendedProperty("MBCT") as string;
                            tempProduct.SetExtendedProperty("MBCT", tmpMbct, this.Editor);
                        ...
                        backupProductPartBySn(cond.ProductID, cond.PartSn);
        ...
        private void backupProductPartBySn(string productId,string sn)
        {
            IProductRepository prodRep = RepositoryFactory.GetInstance().GetRepository<IProductRepository>();
            ProductPart condition =new ProductPart() {
                  PartSn =sn};
            prodRep.BackUpProductPart(new string[] { productId }, condition, this.Editor);
        }
	}
}
