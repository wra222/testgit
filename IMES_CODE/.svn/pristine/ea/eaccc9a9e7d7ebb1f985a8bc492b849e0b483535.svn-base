...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="PackingListForPLReprint.aspx.cs" Inherits="PAK_PackingListForPLReprint" Title="Untitled Page" %>
...
<script type="text/javascript">
    ...
    function processDataEntry(inputData) {
        try {
            ...
            if (!errorFlag) {
                //if (inputData.toString().length != 10                        || inputData.toString().indexOf(mesSN, 0) != 0) {
				if (!CheckCustomerSN(inputData)) {
                    alert(mesSNError);
                }
                else {
                    beginWaitingCoverDiv();
                    WebServicePackingListForPL.CheckSN(inputData, "reprint", fullcount, getDocTypeCmbValue(), "", station, editor, customer, onSucceed, onFail);
                }
                getAvailableData("processDataEntry");
                getCommonInputObject().focus();
            }
            ...
            if (result == null) {
                ...
                for (var i = 0; i < result[1][1].length; i++) {
                    ...
                    for (var j = 0; j < result[1][1][i][2].length; j++) {
						...
						CallEDITSFunc(xmlnameNew, dn, "", xsdname, template.toUpperCase());
    ...
    function CallEDITSFunc(filename, dn, p3, xsd, xsl) {
		...
		var IsSuccess = false;
        if (xsl.indexOf("QVC") >= 0)
			IsSuccess = invokeEDITSFunc(webEDITSaddr, "PackingShipment_QVCLabel", Paralist);
		else if (xsd.toUpperCase().indexOf('BSAM') > 0)
