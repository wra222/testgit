...
var msgScanWin8Part = '<%=this.GetLocalResourceObject(Pre + "_msgScanWin8Part").ToString() %>';
...
//        }
       ...
       else
       {
        ...
        if (printlabeltype2 == "PODLabel")
        {
         ...
         if(checkFAI!="")
         {
           document.getElementById("<%=lblPAQC.ClientID %>").innerText = document.getElementById("<%=lblPAQC.ClientID %>").innerText +', '+checkFAI;
         }
...
//ITC-1360-1247
function FinishWin8(data)
{ 
    if(win8Arr.length==checkWin8Arr.length)
     {return true;}
   if( isInArray(win8Arr,data) && !isInArray(checkWin8Arr,data))
   {
      var i=checkWin8Arr.length;
      checkWin8Arr[i++]=data;
       if(win8Arr.length==checkWin8Arr.length)
     {return true;}
   }
...
function inputOptional(inputData)
 {
    ...
    upperInputData = inputData.toUpperCase();
    
   if(isWin8)
    {
          if(isInArray(win8Arr,upperInputData) && !isInArray(checkWin8Arr,upperInputData))
          {
               var i=checkWin8Arr.length;
               checkWin8Arr[i++]=upperInputData;
                 if(win8Arr.length==checkWin8Arr.length)
                  {
                    isWin8=false;
                     checkData();
                    ...
                    {
                        ShowInfo("Please scan the other WIN8 No.!");
                         callNextInput();
                        return;
                    }
          }
          ...
          {
            ShowInfo("Wrong WIN8 No.!");
            callNextInput();
            return;
          }
         
            //      if(!FinishWin8(upperInputData))
            //      {
            //      //
            //       if( isInArray(win8Arr,data) && !isInArray(checkWin8Arr,data))
            //           {
...
//        }
    ...
    if (rmnflag == "0" && (inputData.substr(3,1) == "-"|| inputData.substr(5,1) == "-" || inputData == "Probook 6475b") && upperInputData != AssetTagItemValue)
    {
        if (!RMNInput) {
            InputType = "R";
            checkRMN(inputData);       
    ...
    // ITC-1360-1451
    // if ((!BoxInput) && (((partType == "YBJ" && inputData != ConfigID) || (partType == "YB") || (partType == "YAB" && inputData != AssetTagItemValue) || (partType == "YABJ" && inputData != ConfigID && inputData != AssetTagItemValue))))  {
     if ((!BoxInput) && ((upperInputData.length == 12 && upperInputData.substr(0,2) != "PC") ||(upperInputData.length == 10 || upperInputData.length == 20)) && (((partType == "YBJ" && upperInputData != ConfigID) || (partType == "YB") || (partType == "YAB" && upperInputData != AssetTagItemValue) || (partType == "YABJ" && upperInputData != ConfigID && upperInputData != AssetTagItemValue))))  {
        if (upperInputData.length == 12 && upperInputData.substr(0,2) != "PC") {
            BoxID = upperInputData.substr(2, 10);
        }
        else if (upperInputData.length == 10 || upperInputData.length == 20) {
            BoxID = upperInputData;
        }
        else {
            alert(msgScanBoxIDorUCC);
            ShowInfo(msgScanBoxIDorUCC);
    ...
    if (upperInputData == ConfigID.toUpperCase()) {
         InputType = "C";
         document.getElementById("<%=lblConfigID_Display.ClientID %>").innerText = ConfigID;
         ConfigInput = true;
         checkData();
   ...
   if (upperInputData.substr(0,5) == AssetTagItemValue.substr(0,5).toUpperCase()) {
           if (upperInputData!=AssetTagItemValue.toUpperCase()){
            alert(msgASTFail);
            ShowInfo(msgASTFail);
            callNextInput();
            return;
             ...
             if (site == "ICC")
              {
                         ...
                         for (var i = 0; i < lstPrintItem.length; i++) {
                             ...
                             if (lstPrintItem[i].LabelType.toUpperCase().indexOf("WHITE") == -1 && lstPrintItem[i].LabelType.toUpperCase().indexOf("BLACK") == -1) {
                                 newPrintItemArr.push(lstPrintItem[i]);
                             }
                }
                 WebServiceUnitWeight.save_CQ(productID,model, newPrintItemArr,onSaveSuceed,onSaveFailed);
           }
           else
           {
...
////==================================Print=======================================         
      ...
      //  ShowSuccessfulInfo(true, msgSuccess); //Use for test, need delete
        ...
        if(checkFAI!="")
        {
          ...
          ShowSuccessfulInfo(true, SuccessItem + " " + _msg);
     }
     else {
        ShowInfo("");
        var content = result[0];
