...
namespace IMES.CheckItemModule.CQ.PizzaCustomSN.Filter
{
    [Export(typeof(IFilterModule))]
    [ExportMetadata("ProgramName", "IMES.CheckItemModule.CQ.PizzaCustomSN.Filter.dll")]
    public class Filter : IFilterModule
    {
        private int qty = 1;
        private string part_check_type = "PizzaCustomSN";
        ...
        private string GetCheckCode(IProduct Pobject)
        {
            string checkcode = string.Empty;
            IPartRepository partRep = RepositoryFactory.GetInstance().GetRepository<IPartRepository>();
            IList<ConstValueInfo> lstConst = partRep.GetConstValueListByType("PizzaCustSNCheckCode");
            var prefixList = (from p in lstConst
                              where p.name == Pobject.Family.Trim()
                              select p.value).ToList();
            if (prefixList != null && prefixList.Count > 0)
            {
                checkcode = prefixList[0].Trim();
            }
            return checkcode;
        }
    }
