...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" runat="Server">
    ...
    <div>
        ...
        <script type="text/javascript">
            ...
            var msgDDRCTNull = '<%=this.GetLocalResourceObject(Pre + "_msgDDRCTNull").ToString() %>';
                        ...
                        try {
                          ...
                          try {  
                            if(lstPrintItem!=null) {
								...
								/*if (nextStep=='Print'){
									//var lstPrintItem = setPrintPara();
									MasterLabelPrintService.display(line, strProdID, lstPrintItem, "<%=UserId%>", station, "<%=Customer%>", onDisplaySuccess, onDisplayFail);
								}*/
								if (nextStep=='CheckCustsn'){
								    strCustSN = inputData;
									MasterLabelPrintService.CheckCustsn(strProdID, strCustSN, line, "<%=UserId%>", station, "<%=Customer%>", onCheckCustsnSuccess, onDisplayFail);
								}
								else if (nextStep=='CheckDDRCT'){
								    strDDRCT = inputData;
									MasterLabelPrintService.CheckDDRCT(strProdID, strDDRCT, line, "<%=UserId%>", station, "<%=Customer%>", onCheckDDRCTSuccess, onDisplayFail);
								}
								else {
								    strProdID = inputData;
									MasterLabelPrintService.CheckJamestown(strProdID, line, "<%=UserId%>", station, "<%=Customer%>", onCheckJamestownSuccess, onDisplayFail);
								}
                            }
                        }
                        catch (e) {
			...
			function onCheckJamestownSuccess(result) {
                if(result[0]=='notJC' || result[1]=='notCheckSN') {
                    if(result[3]=='notCheckDDRCT')
                    {
					    var lstPrintItem = setPrintPara();
					    MasterLabelPrintService.display(line, strProdID, lstPrintItem, "<%=UserId%>", station, "<%=Customer%>", onDisplaySuccess, onDisplayFail);
				    }
				    	...
				    	nextStep='CheckDDRCT';
					    ShowInfo(msgDDRCTNull);
					    setInputOrSpanValue(document.getElementById("<%=lblInputProdID.ClientID %>"), strProdID);
					    setInputOrSpanValue(document.getElementById("<%=lblInputModel.ClientID %>"), result[2]);
				    ...
				    if(result[3]=='notCheckDDRCT')
				    {
				        nextStep='CheckCustsn';
					    ShowInfo(msgCustSNNull);
					    setInputOrSpanValue(document.getElementById("<%=lblInputProdID.ClientID %>"), strProdID);
					    setInputOrSpanValue(document.getElementById("<%=lblInputModel.ClientID %>"), result[2]);
            ...
            function onCheckDDRCTSuccess(result) {
                if(result[1]=='notCheckSN') {
					var lstPrintItem = setPrintPara();
					nextStep = '';
					MasterLabelPrintService.display(line, strProdID, lstPrintItem, "<%=UserId%>", station, "<%=Customer%>", onDisplaySuccess, onDisplayFail);
				} else {
			...
			function onCheckCustsnSuccess(result) {
				...
				if ("SUCCESSRET"==result) {
					...
					ShowInfo('Err when Check CustSN!');
