...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master"  ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="PCAOQCInput.aspx.cs" Inherits="SA_PCAOQCInput" %>
...
<div>
    <asp:ScriptManager ID="ScriptManager1" runat="server" >
        <Services>
            <asp:ServiceReference Path="Service/WebServicePCAOQCInput.asmx" />
        </Services>
    </asp:ScriptManager>
    <center>
        ...
        if (!errorFlag) 
        {
            ...
            else 
            {
                if (sFirstInput == "") // Input LotNo or MBSN
                {
                    ...
                    if (IsCheckLot == "Y") {
                        WebServicePCAOQCInput.InputMBorLot(sFirstInput, InputType, editor, station, customer, onInputSucceeded, onInputFailed);
                    }
                    else if(IsCheckLot == "N" && InputType == "MBSN")
                    {
                        WebServicePCAOQCInput.InputMB(sFirstInput, InputType, editor, station, customer, onInputMBSucceeded, onInputMBFailed);
                    }
                    else  //Please LotNo or MBSN
                    {
                        ShowMessage(mesNoInputLotorMB);
                ...
                else  //
                {
                    ...
                    if ((StrCMD == "LOCK") || (StrCMD == "UNDO") || (StrCMD == "PASS") || (StrCMD == "UNLOCK"))  //KeyCode£∫LOCK/UNDO/PASS
                    {
                        ...
                        WebServicePCAOQCInput.save(sFirstInput, sKeyCode, editor, strLine, customer, onSaveSucc, onSaveFail);
                    }
                    else if ((StrCMD.length == 10) || (StrCMD.length == 11)) //MBSN
                    {
                        ...
                        WebServicePCAOQCInput.InsertmbsntoPcbLotCheck(document.getElementById("<%=lblLotNoContext.ClientID%>").innerText,
                                                                                       intputMBSNforlot,
...
function onInputMBSucceeded(result) {
    try {
        endWaitingCoverDiv();
        if (result == null) {
            ShowInfo("");
        ...
        else if (result[0] == SUCCESSRET) //OK
        {
            var SuccessItem = "[" + sFirstInput + "]";
            NoCheckLotPassQty += 1;
            sFirstInput = "";
            document.getElementById("<%=lblCheckQtyContext.ClientID%>").innerText = NoCheckLotPassQty;
            //ClearData();
...
function onInputMBFailed(result) {
    endWaitingCoverDiv();
    ShowMessage(result.get_message());
    ShowInfo(result.get_message());
    sFirstInput = "";
    ...
    try {
        ...
        else if ((result.length == 5) && (result[0] == SUCCESSRET)) //OK
        {
            ...
            if (result[4] == "GotoPass") {
                ...
                WebServicePCAOQCInput.save(document.getElementById("<%=lblLotNoContext.ClientID%>").innerText, 
