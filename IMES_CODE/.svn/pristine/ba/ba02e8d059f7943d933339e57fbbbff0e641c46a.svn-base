    ...
    public partial class CheckBTSKU : BaseActivity
    {
        ...
        protected internal override ActivityExecutionStatus DoExecute(ActivityExecutionContext executionContext)
        {
            ...
            if (!CurrentProduct.IsBT)
            {
                ...
                string modelCategory=utl.CheckModelByProcReg(CurrentProduct.Model);
                if (!string.IsNullOrEmpty(modelCategory))
                {
                    string allowStation = utl.GetConstValueByTypeAndName("AllowBTChangeStation", modelCategory,null);
                    if (!string.IsNullOrEmpty( allowStation))
                    {
                        string[] allowStationList = allowStation.Split(new char[] { ',', '~'});
                        if (allowStationList.Contains(CurrentProduct.Status.StationId))  // need add ProductBT
                        {
                            session.AddValue("isBT", "Y");
