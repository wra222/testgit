...
public partial class CommonFunction_RunningSQLStatement : System.Web.UI.Page
{
    ...
    public void DisplayResultGV(object sender, System.EventArgs e)
    {
        try
        {
            string DB = this.HiddenDB.Value;
            string SQLText = this.HiddenSQLText.Value;
            this.Legend1.InnerText = SQLText;
            DataTable dtData = GetDTList(DB, SQLText);
    ...
    private DataTable GetDTList(string DB, string SQLText)
    {
        ...
        if (dt != null && dt.Rows.Count != 0)
        {
            ...
            //        gridviewWidth += 5;//dt.Columns[i].ColumnName.Length;
            //    }
            //}
            ////if (gridviewWidth < 98)
            ////{
    ...
    {
        String script = "<script language='javascript'>" + "\r\n" +
            "document.getElementById('BtnQuery').disabled = false;document.getElementById('btnOpenTran').disabled = false;document.getElementById('btnBlockSpid').disabled = false;document.getElementById('btnExcel').disabled = false;endWaitingCoverDiv();" + "\r\n" +
            "</script>";
