...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" Runat="Server">
    ...
    <script language="javascript" type="text/javascript">
        ...
        function DFARepair_isCustSN(input) {
            if (input.length != 10) return false;
            pattCustSN = /^CN.{8}$/;
            //if (pattCustSN.exec(input)) return true;
			if (CheckCustomerSN(input)) return true;
            else return false;
        }

        function DFARepair_isProdID(input, line) {
            if (input.length != 9 && input.length != 10) return false;
            pattProdID = new RegExp("^[A-Z0-9][0-9][1-9ABC][0-9]{3}.{3,4}$", "i");
            if (pattProdID.exec(input)) return true;
            else return false;
        }

        function DFARepair_isProdIDorCustSN(input, line) {
            return DFARepair_isCustSN(input) || DFARepair_isProdID(input, line);
        } 
        ...
        function processData(productID) {
            ...
            if (!DFARepair_isProdIDorCustSN(productID, getPdLineCmbValue())) {
                alert(mesInput);
