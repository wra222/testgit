...
public class WebServiceTravelCardExcel : System.Web.Services.WebService
{
    ...
    public ArrayList PrintTCWithProductIDForBN(string pdLine, string model, string mo, int qty, bool IsNextMonth,
                                ...
                                string bomremark, string remark, string exception, string ModelSelect,
                                string pilotmo, string pilotmoremainqty)
    {
        ...
        try
        {
            ...
            ITravelCardPrintProductPlan iTravelCardPrint2 = ServiceAgent.getInstance().GetObjectByName<ITravelCardPrintProductPlan>(WebConstant.TravelCardPrintProductPlanObject);
            if (iTravelCardPrint2.CheckedBSamModel(ModelSelect))
            {
                //CheckedBSamModelData();
                if (!iTravelCardPrint2.CheckedC_SKU(ModelSelect, "CSKU"))
                {
                    throw new Exception("Bsam Model: " + model + " 未設定CSKU，請聯繫IE Maintain...");
                }
                else if (!iTravelCardPrint2.CheckedBSamModelAndC_SKU(ModelSelect, "CSKU"))
                {
                    throw new Exception("Bsam Model之CSKU資料錯誤，請聯繫相關人員...");
                }
            }
            //if (!iTravelCardPrint2.CheckedC_SKU(ModelSelect, "CSKU"))
            //{
            ...
            //    throw new Exception("Bsam Model之CSKU資料錯誤，請聯繫相關人員...");
            //}
            retPrintItemLst = iTravelCardPrint.PrintTCWithProductIDForBN(pdLine, model, mo, qty, IsNextMonth, editor, station, customer, out snLst, printItemLst, out battery, out lcm, deliverydate, bomremark, remark, exception, pilotmo, pilotmoremainqty);
