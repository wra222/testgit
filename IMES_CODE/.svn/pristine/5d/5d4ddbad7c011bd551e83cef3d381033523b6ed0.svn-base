...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true"
    CodeFile="UnpackCombinePizza.aspx.cs" Inherits="PAK_UnpackCombinePizza" Title="Untitled Page" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" runat="Server">
    ...
    <asp:ScriptManager ID="ScriptManager1" runat="server" EnablePartialRendering="true">
        <Services>
            <asp:ServiceReference Path="Service/WebserviceUnpackCombinePizza.asmx" />
        </Services>
    </asp:ScriptManager>
<div>
   <center >
            ...
            if (sn != "") {
                if (9 == sn.length){
					if (ConfirmUnpack()){
						...
						WebserviceUnpackCombinePizza.Unpack("", sn, "<%=UserId%>", station, "<%=Customer%>", onSuccess, onFail);
						return;
					}
				}
				else if ((10 == sn.length) && ('7CD' == sn.substr(0,3))){
					if (ConfirmUnpack()){
						beginWaitingCoverDiv();
						WebserviceUnpackCombinePizza.Unpack(sn, "", "<%=UserId%>", station, "<%=Customer%>", onSuccess, onFail);
						return;
        ...
        function onSuccess(result) {
            ...
            try {
                if (result == null) {
                    ...
                    ShowSuccessfulInfo(true, "UnPack 成功！");
                }
                inputControl.focus();
                getAvailableData("processDataEntry");
            }
