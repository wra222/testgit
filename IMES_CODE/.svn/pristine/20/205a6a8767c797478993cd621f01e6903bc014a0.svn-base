<%@ Page Language="C#" AutoEventWireup="true" CodeFile="LightNoUploadFile.aspx.cs" Inherits="DataMaintain_LightNoUploadFile" %>
...
<head id="Head1" runat="server">
    <title>Upload File Dialog</title>
...
<script type="text/javascript">    
    ...
    {   
        ...
        if(document.getElementById("<%=HidCurPdLine.ClientID %>").value!="")
        {
            getMaintainLightNoPdLineCmbObj().value=document.getElementById("<%=HidCurPdLine.ClientID %>").value;
        }
...
//    }
    ...
    function checkSetEnableCombox()
    {
        ...
        var KittingTypeValue=document.getElementById("<%=hidKittingType.ClientID %>").value;
        ...
        else //if(getMaintainLightNoKittingTypeCmbObj().value.trim()==KittingTypePAKKitting)
        {
            DealKitTypePAK();
        }
...
<body style="background-color: #ECE9D8" onload="page_load()" onunload="page_Unload()">
    ...
    <%--    <Triggers>
     ...
     <input type="hidden" id="hidKittingType" runat="server" />
     ...
     <input type="hidden" id="HidCurPdLine" runat="server" />
...
<script type="text/javascript">    
    ...
    function clkButton()
    {
        ...
        if(getMaintainLightNoPdLineCmbObj().disabled!=true)
        {
            var pdLineValue=getMaintainLightNoPdLineCmbObj().value.trim();
            if(pdLineValue=="")
            {
              //Please select [PdLine]!
                alert(msg7);
                trySetFocusPdLine();//
                return false;
            } 
        ...
        if(getMaintainLightNoPartTypeCmbObj().disabled!=true)
        {
        
            var partTypeValue=getMaintainLightNoPartTypeCmbObj().value.trim();
            if(partTypeValue=="")   
            {
                alert(msg8);
    //            Please select [PartType]!
                trySetFocusPartType();//
                return false;
            }
        }
