...
<%@ Page Language="C#" AutoEventWireup="true" CodeFile="OQCRepairAddEdit.aspx.cs" Inherits="FA_OQCRepairAddEdit" Title="iMES - OQC Repair Log" %>
...
<body style=" position: relative; width: 100%"> 
    ...
    <asp:ScriptManager ID="ScriptManager1" runat="server" >
        <Services>
            <asp:ServiceReference Path="~/Docking/Service/OQCRepairService.asmx" />
        </Services>
    </asp:ScriptManager>
    ...
    <script language="javascript" type="text/javascript">
        ...
        function addOkClick()
        {
            if (editInputCheck())
            {
                OQCRepairService.add(parentWindow.globalProID, getDefect(parentWindow.status), editor, stationId, customer , addOkSuccess, addOkFail); 
            }        
        }
        ...
        function addAddClick()
        {
            if (editInputCheck())
            {
                OQCRepairService.add(parentWindow.globalProID, getDefect(parentWindow.status), editor, stationId, customer, addAddSuccess, addOkFail);
            }         
        }
        ...
        function editOkClick()
        {
            ...
            {
                //edit action
                OQCRepairService.update(parentWindow.globalProID, getDefect(parentWindow.status), editor, stationId, customer, editSuccess, editFail);
            }
        }        
