...
<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="PalletDataVerifyForRCTO.aspx.cs" Inherits="PalletVerifyForRCTO" %>
...
<asp:ScriptManager runat="server" ID="SM" EnablePartialRendering="true">
    <Services>
        <asp:ServiceReference Path="~/PAK/Service/WebServicePalletVerifyDataForRCTO.asmx" />
    </Services>
</asp:ScriptManager>   
<style type="text/css">
</style>
...
<div id="divPalletVerify" style="z-index: 0;">
        ...
        <fieldset style="width:95%" align="center">
            <legend id="lblCartonLst" runat="server" style="color:Blue" class="iMes_label_13pt"></legend>
            <table width="100%" cellpadding="0" cellspacing="0" border="0" align="center">
                <tr>
...
//     var totalQty = 0;
     ...
     function inputCustSN(inputStr) {
         ...
         if (ifFirst) {
             ...
             WebServicePalletVerifyForRCTO.inputFirstCustSN(firstCustSn, line, editor, station, customer, onFirstSnSucceeded, onFirstSnFailed);
         }
         else {
             if (scanQty >= palletQty) {
                ...
                getAvailableData("processDataEntry");                
                 ...
                 custSn = inputStr;
                 secondSn = custSn;
                 WebServicePalletVerifyForRCTO.inputCustSN(firstCustSn, custSn, palletNo, onSucceeded, onFailed);
             }
         }
     }
 ...
 */
     ...
     function onFirstSnSucceeded(result) {
         ...
         if (result == null || result.length == 0) {
             ...
             palletNo =result[0][0];
             document.getElementById("txtPalletQty").value = result[0][3];
             palletQty = result[0][3];
             //palletQty = 2;
             ...
             var cartonNo = result[0][1];
             var DeliveryNo = result[0][2];
            ...
            // for (var i = 0; i < CustList.length; i++) {
                 ...
                 rowInfo.push(DeliveryNo);   //SVN 2569: 每成功接收一个正确的Customer S/N，需要填写在Product List 中对应记录的Collection Data 栏位，包括前面刷入的第一个Customer S/N
                 rowInfo.push(""); 
                 HighlightFlag = true;
     ...
     function onSucceeded(result) {
         ...
         else if ((result[0] == "CHK879") && result.length == 2) {  //再开启新的Pallet Verify 流程
             ResetPage();
            ...
            ShowErrorMessage(result[1]);
             ...
             if (scanQty > palletQty) {
                 ...
                 var newSelectedRowIndex = 1;
                     /// result[1]: [0],    [1],      [2],    
                 ///             palletNo,carton no,delivery no,
                if( CheckInTable(result[1][1],1)) {
         ...
         try {
             ...
             if (lstPrintItem == null)                 //判断 若PrintItem==null, 不继续打印，等待客户维护PrintSetting页面后，再刷入打印
             {
                ...
                // lstPrintItem[0].LabelType = "DK_Travel_Card";
                 WebServicePalletVerifyForRCTO.save(firstCustSn, lstPrintItem, palletNo, line, editor, station, customer, onSaveSucceed, onSaveFailed);
             }
         }
         catch (e) {
             PlaySound();
     ...
     function onSaveSucceed(result) {
         ...
         if (result == null) {
             ...
             var LabelType;
             LabelType = result[1][1]; //Consolidate Label
...
//    }
   ...
   function CmdGeneratePdf(DeliveryNo,Pallet,plttp, Template, Attp, TCount, Sntp)
    ...
    catch (err)
    {
     ...
     function ExitPage() {
         if (firstCustSn != "") {
             WebServicePalletVerifyForRCTO.Cancel(firstCustSn);
