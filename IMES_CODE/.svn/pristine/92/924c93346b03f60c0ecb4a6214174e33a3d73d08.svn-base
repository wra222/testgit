...
namespace IMES.Station.Interface.StationIntf
{
    ...
    public interface IShipToCartonLabel
    {
        ...
        ArrayList InputProcess(string data, int type, string sessionKey, IList<PrintItem> printItemLst, string pdLine, string station, string editor, string customer, out MRPLabelDef MRP);

        string GetMessageByPalletAndLocation(string deliveryNo, string pallet);

        ArrayList Reprint(string prodid, int key, string editor, string station, string customer, string reason, string pCode, IList<PrintItem> printItems, out MRPLabelDef MRP);
