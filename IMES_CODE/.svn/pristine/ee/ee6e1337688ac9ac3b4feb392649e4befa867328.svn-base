		...
		function _buildTabs()
		{
			...
			if (this.imagePath != "")
				...
				sContent += "<td></td></TR>";
			sContent += '<TR><td colspan="' + (i+1) + '" style=\"background-color: rgb(147,191,218);\"></td></TR></TABLE>'
		...
		{							
			...
			document.getElementById("imgButton" + this.index).src = this.selPic;
			document.getElementById("imgButton" + this.index).style.cursor = "default";
		}
		...
		function doUnSelect()
		{
			...
			document.getElementById("imgButton" + this.index).src = this.normalPic;
			document.getElementById("imgButton" + this.index).style.cursor = "hand";
		...
		function _changeStatus(status)
		{
		    ...
		    if (status == true){
		        document.getElementById("imgButton" + this.index).src = this.disablePic;
		        document.getElementById("imgButton" + this.index).style.cursor = "default";
		    }else{
		        document.getElementById("imgButton" + this.index).src = this.normalPic;
		        document.getElementById("imgButton" + this.index).style.cursor = "hand";
