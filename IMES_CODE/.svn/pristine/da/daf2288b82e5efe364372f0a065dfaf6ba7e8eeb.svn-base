...
public class WebServiceCombineAST : System.Web.Services.WebService
{
	...
	[WebMethod]
    public ArrayList dosaveBeforeCheckATSN9(string prodid, string ast, string stationId, string editor, string customer)
    {
        ArrayList ret = new ArrayList();
        try
        {
            ArrayList arys = iCombineAST.DoSave(prodid, ast, stationId, editor, customer);
            ret.Add(WebConstant.SUCCESSRET);
			ret.Add(arys[0]); // partno of ATSN9
			ret.Add(arys[1]); // prompt of ATSN9
            return ret;
        }
        catch (FisException ex)
        {
    ...
    [WebMethod]
    public ArrayList dosaveAfterCheckATSN9(string prodid, string stationId, string editor, string customer)
    {
        ArrayList ret = new ArrayList();
        try
        {
            iCombineAST.DoSaveAfterCheckATSN9(prodid, stationId, editor, customer);
            ret.Add(WebConstant.SUCCESSRET);
            return ret;
        }
        catch (FisException ex)
    ...
    */
    ...
    {
		return doBlockcheck(prodid, length, pdline, status, stationId, editor, customer, false);
	}
	
	[WebMethod]
    public List<string> blockcheck_CombineAndGenerateAST(string prodid, int length, string pdline, string status, string stationId, string editor, string customer)
    {
		return doBlockcheck(prodid, length, pdline, status, stationId, editor, customer, true);
	}
	
	[WebMethod]
    public List<string> doBlockcheck(string prodid, int length, string pdline, string status, string stationId, string editor, string customer, bool isCombineAndGenerateAST)
    {
        List<string> ret = new List<string>();
        string astreturn = "";
        //2012-4-10
        ...
        try
        {
            ...
            retLst = iCombineAST.BlockCheck(prodid, length, pdline, status, stationId, editor, customer, isCombineAndGenerateAST);
            if (retLst != null)
            {
                astreturn = retLst[0];
