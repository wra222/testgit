...
//     var totalQty = 0;
     ...
     function inputCustSN(inputStr) {
         if (ifFirst) {
             ...
             WebServicePalletVerify.inputFirstCustSN(sCheckBoxId, firstCustSn, line, editor, station, customer, onFirstSnSucceeded, onFirstSnFailed);
         }
         else {
                 ...
                 for (var i = 0; i < result[2].length; i++) {
                     for (var j = 0; j < result[1].length; j++) {
                         if (result[1][j].LabelType == "Non-bulk Pallet Label for Manual order") {
						  ...
						  if(result[1][j].LabelType == "AIO_PALLET_SN") {
                             setPrintItemListParam(result[1][j], result[2][i], "AIO_PALLET_SN");    //手动单:Pallet Label
                            // printLabels(result[1][j], false);
                         }
