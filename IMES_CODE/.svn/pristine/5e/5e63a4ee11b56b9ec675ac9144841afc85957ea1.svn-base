    ...
    public class FA_ProductInfo : MarshalByRefObject,IFA_ProductInfo
    {
        ...
        ProdutData IFA_ProductInfo.ProductInfo(string Connection, string ID, out List<NextStation> NextStationList)
        {
            ...
            string strSQL = @"
                ...
                ELSE IF EXISTS(SELECT PizzaID FROM Pizza_Part (NOLOCK) WHERE PartSn=@Product)
                BEGIN
                   SELECT  a.ProductID , a.CUSTSN, a.MAC, a.MBECR as ECR, a.MO, a.UnitWeight, 
                           a.PCBID, a.PCBModel, a.Model,d.Family,
                           a.CartonSN, a.PalletNo, a.DeliveryNo,
                   ...
                   WHERE   a.ProductID IN
                            (SELECT TOP 1 ProductID 
                             FROM Product a (NOLOCK) 
                             INNER JOIN Pizza_Part b (NOLOCK) on a.PizzaID = b.PizzaID and b.PartSn=@Product)                           
                END
                ELSE IF EXISTS(
                 SELECT ProductID FROM  ProductInfo WHERE InfoValue=@Product
                ) 
...
//                            ORDER BY a.Cdt";
        ...
        DataTable IFA_ProductInfo.GetProductHoldHistory(string Connection, string ID)
        {
            string methodName = MethodBase.GetCurrentMethod().Name;
            ...
            try
            {
                string strSQL = @"IF EXISTS(SELECT ProductID FROM ProductStatus (NOLOCK) WHERE ProductID=@Product and Station like '%Hold%')
                BEGIN
                  SELECT a.Line,a.Family,a.Model,a.CUSTSN,HoldStation,CheckInStation,HoldDescr,a.Editor FROM dbo.HoldRuleLog a,dbo.Product b ,dbo.Model c WHERE 
                  (a.CUSTSN=b.CUSTSN OR a.Model=b.Model OR a.Family=c.Family) AND b.Model=c.Model AND b.ProductID=@Product                        
                END
               ";
             SqlParameter paraName = new SqlParameter("@Product", SqlDbType.VarChar, 32);
             paraName.Direction = ParameterDirection.Input;
        ...
        DataTable IFA_ProductInfo.GetProductInfo(string Connection, string ID)
        {
            ...
            try
            {
                string strSQL = @"SELECT InfoType,InfoValue,Editor,CONVERT (nvarchar(20),a.Cdt,120) AS Cdt,CONVERT (nvarchar(20),a.Udt,120) AS Udt
        ...
        DataTable IFA_ProductInfo.GetUnpackProductPart(string Connection, string ID)
        {
            string methodname = MethodBase.GetCurrentMethod().Name;
            BaseLog.LoggingBegin(logger, methodname);
            try
            {
                string strSQL = @"select ProductID, PartNo, PartType, IECPn, CustmerPn, PartSn, Station, BomNodeType, CheckItemType, UEditor, UPdt
                                    from UnpackProduct_Part
        ...
        DataTable IFA_ProductInfo.GetUnpackPizzaPart(string Connection, string ID)
        {
            string methodname = MethodBase.GetCurrentMethod().Name;
            BaseLog.LoggingBegin(logger, methodname);
            try
            {
                string strSQL = @"select b.ProductID ,a.PizzaID, a.PartNo, a.PartType, a.IECPn, a.CustmerPn, 
                                           a.PartSn, a.Station, a.BomNodeType, a.CheckItemType, a.UEditor, a.UPdt
                                    from UnpackPizza_Part a, Product b
                                    where a.PizzaID = b.PizzaID and b.ProductID = @ProductID";
