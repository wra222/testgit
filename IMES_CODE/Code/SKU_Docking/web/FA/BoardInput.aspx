...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" runat="Server">
         ...
         <table border="0" width="95%">   
                ...
                <tr>
                    ...
                    <td align="right">
                        ...
                        ，<asp:Label ID="lbSpeedSum" runat="server" CssClass="iMes_label_13pt"></asp:Label>
                        <asp:Label ID="txtSpeedSum" runat="server" CssClass="iMes_label_13pt"></asp:Label>
                        <asp:Label ID="lbSecond2" runat="server" CssClass="iMes_label_13pt"></asp:Label>
                        </div>
                    </td>
                </tr>
                <tr>
    ...
    <script type="text/javascript">
        ...
        function OT() { //OverTime
            if (EnableComn) objMSComm.Output = "n";
        }
        function NOT() { //NotOverTime
            if (EnableComn) objMSComm.Output = "p";
        }
        function ChkSpeed() { // mantis 1522
            try {
                if (',' == '<%=SpeedLimit%>') {
                    return;
                }
                ...
                if (LatestLine != line) {
                    ...
                    OTLimit = 0;
                    OTSummary = 0;
                    var pos1 = '<%=SpeedLimit%>'.indexOf(line.substr(0, 1) + '=');
                    if (pos1 >= 0) {
                        var pos2 = '<%=SpeedLimit%>'.indexOf(',', pos1 + 2);
                        OTLimit = parseInt('<%=SpeedLimit%>'.substring(pos1 + 2, pos2));
                        ...
                        document.getElementById("<%=txtSpeedNow.ClientID%>").innerText = "";
                        document.getElementById("<%=txtSpeedSum.ClientID%>").innerText = "";
        ...
        function processDataEntry(inputData) {
            ...
            //    inputData = SubStringSN(inputData, "ProdId");
                ...
                if (!errorFlag) {
                    ...
                    {
                        if ((inputData.length == 10 || inputData.length == 11) && (inputData.substr(4, 1) == "M" || inputData.substr(4, 1) == "B")) {
                        ...
                        else if ((inputData.length == 11) && (inputData.substr(5, 1) == "M" || inputData.substr(5, 1) == "B")) {
                            lstPrintItem = getPrintItemCollection();
                            var proid = document.getElementById("<%=txtProdId.ClientID%>").innerText;
                            proid = SubStringSN(proid, "ProdId");
                            hidMBSn = SubStringSN(inputData, "MB");
                            ...
                            hidinputcode = inputData.trim().toUpperCase();
                            WebServiceBoardInput.inputMBSn(document.getElementById("<%=txtProdId.ClientID%>").innerText, hidinputcode, onSucceed, onFail);
                        }
                    }
                }
        ...
        function onSaveSucceed(result) {
            ...
            try {
                ...
                if (printflag && lstPrintItem != null && result[1][0].TemplateName != "" && result[2] != null) {
                                ...
                                setAllPrintParam(result[1], keyCollection, valueCollection);
