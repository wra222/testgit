...
public class WebServiceKBCTCheck : System.Web.Services.WebService
{
    ...
    public ArrayList checkProdId(string prodid, string pdline, string editor, string stationId, string customerId)
    {
        ...
        try
        {
            iKBCTCheck.CheckProdId(prodid, pdline, editor, stationId, customerId);
    ...
    [WebMethod]
    public ArrayList checkProdIdOffline(string prodid, string pdline, string editor, string stationId, string customerId)
    {
        ArrayList ret = new ArrayList();

        try
        {
            iKBCTCheck.CheckProdIdOffline(prodid, pdline, editor, stationId, customerId);
            ret.Add(WebConstant.SUCCESSRET);
            return ret;
        }
        catch (FisException ex)
    ...
    [WebMethod]
    public ArrayList checkAndSaveOffline(string prodid, string kbct, string pdline, string editor, string stationId, string customerId)
    {
        ArrayList ret = new ArrayList();

        try
        {
            string notMatch = iKBCTCheck.CheckAndSaveOffline(prodid, kbct, pdline, editor, stationId, customerId);
            if (!"Y".Equals(notMatch))
                ret.Add(WebConstant.SUCCESSRET);
            else
                ret.Add("fail");
            return ret;
    ...
    [WebMethod]
    public ArrayList SaveDefect(string prodid, string defect)
    {
        ArrayList ret = new ArrayList();

        try
        {
            iKBCTCheck.SaveDefect(prodid, defect);
            ret.Add(WebConstant.SUCCESSRET);
            return ret;
        }
        catch (FisException ex)
