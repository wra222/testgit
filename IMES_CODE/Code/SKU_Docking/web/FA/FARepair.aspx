...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" Runat="Server">
	...
	var feature = "dialogHeight:475px;dialogWidth:800px;center:yes;status:no;help:no";     //the style of pop up window
	var url = "FARepairAddEdit.aspx?Station=<%=Request["Station"]%>&PCode=<%=Request["PCode"]%>&Stage=<%=Request["Stage"]%>&UserId=<%=UserId%>&Customer=<%=Customer%>&CauseCustomerID=<%=Request["CauseCustomerID"]%>";
	...
	var pCode = "";
        ...
        function processData(productID) {
            ...
            document.getElementById("<%=hidProdId.ClientID %>").value = productID;
        ...
        function inputFinish() {
			...
			inputObj.value = document.getElementById("<%=hidProdId.ClientID %>").value;
            document.getElementById("<%=btnAdd.ClientID %>").disabled = false;
            //document.getElementById("<%=btnEdit.ClientID %>").disabled = false;
            document.getElementById("<%=btnSave.ClientID %>").disabled = false;
			globalProID = document.getElementById("<%=hidProdId.ClientID %>").value;
            //getAvailableData("processData");  
        }
            ...
            if (inputFlag)
            {
                ...
                {                    
                    ...
                    if (!emptyPattern.test(returnStation)) {
						...
						FARepairService.save(document.getElementById("<%=hidProdId.ClientID %>").value, returnStation, "0", saveSucc, saveFail);
                    }
                    else
                    {
        ...
        function checkFinish(par) {
            ...
            if (par == 1) {
				...
				FARepairService.save(document.getElementById("<%=hidProdId.ClientID %>").value, returnStation, "1", saveSucc, saveFail);
            }
            else {
...
//            editObj.cause = tblObj.rows[selectedRowIndex].cells[4].innerText;   
        ...
        function window.onbeforeunload() {
                FARepairService.wfcancel(document.getElementById("<%=hidProdId.ClientID %>").value);            
        }
