...
{
    IPalletWeight palletweightManager = (IPalletWeight)ServiceAgent.getInstance().GetObjectByName<IPalletWeight>(WebConstant.PalletWeightForDocking);
    /// <summary>
    /// 
    /// </summary>
    ...
    public ArrayList InputPallet(string inputID, string actualWeight,string type, string line, string editor, string station, string customer)
    {
        ...
        try
        {
           ArrayList tempList = palletweightManager.InputPallet(inputID, decimal.Parse(actualWeight),type, 
            ...
            retlist.Add(tempList[0]); //PalletNo
            retlist.Add(tempList[1]); //modelList
            retlist.Add(tempList[2]); //standardWeight
            retlist.Add(tempList[3]); //tolerance
            retlist.Add(tempList[4]);//strRepeat);
            retlist.Add(tempList[5]);//fruFLag);
    ...
    [WebMethod]
    public ArrayList InputCustSN(string custSN, string palletNo)
    {
        ArrayList retlist = null;
        try
        {
            String customer = palletweightManager.InputCustSN(custSN, palletNo);
            retlist = new ArrayList();
            retlist.Add(WebConstant.SUCCESSRET);
    ...
    public ArrayList GetWeightSetting(string hostname)
    {
        ...
        try
        {
            UnitWeighInfo = palletweightManager.GetWeightSettingInfo(hostname);
