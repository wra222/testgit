<%@ WebService Language="C#" Class="WebServiceMBLabelPrint" %>
using System;
using System.Web;
...
public class WebServiceMBLabelPrint : System.Web.Services.WebService
{
    IMBLabelPrint iMBLabelPrint = ServiceAgent.getInstance().GetObjectByName<IMBLabelPrint>(WebConstant.MBLabelPrintDocking);
    ...
    public ArrayList print(string pdLine, string month, string mbCode, string mo, string qty, string datecode, string editor, string station, string customer, string _111, string factor, IList<PrintItem> printItems, string labelType)
    {
        ...
        try
        {
            retPrintItemLst = iMBLabelPrint.Print(pdLine, Convert.ToBoolean(month), mbCode, mo, int.Parse(qty), datecode, editor, station, customer, out snLst, _111, factor, printItems);
