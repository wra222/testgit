...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master"  ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="FPCBoardLabelPrint.aspx.cs" Inherits="DOCKING_FPCBoardLabelPrint"  Title="无标题页"%>
...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" Runat="Server">
    ...
    <asp:ScriptManager ID="ScriptManager1" runat="server" >
        <Services>
            <asp:ServiceReference Path="Service/WebServiceFPCBoardLabelPrint.asmx" />
        </Services>
    </asp:ScriptManager>
    <center>
...
function print()
{
    try {
        ...
        if (!errorFlag) {
            ...
            labelType = "DK_FPC_Label";


            beginWaitingCoverDiv();
            WebServiceFPCBoardLabelPrint.print(getPdLineCmbValue(), month, mbSno, getMOCmbValue(), qty, dateCode, editor, stationId, customer, get111LevelCmbValue(), factor, lstPrintItem, labelType, onSucceed, onFail);
        }
    } catch(e) {
        alert(e);
    ...
    try {
        ...
        else if (result[0] == SUCCESSRET) // 0:Success,1:printitem lst 2 : pdline 3:_111   4:snlst
        {
            ...
            if (result[1].length > 0) {
                ...
                keyCollection[2] = "@MQty";
                valueCollection[2] = generateArray(result[4]);
                ...
                if (result[4] > 1) {
                    var star = result[2];
                    var end = result[3];
                    var star_A = star.substring(0, 5);
                    var star_B = star.substring(10, 6);
                    var end_A = end.substring(0, 5);
                    var end_B = end.substring(10, 6);

                    for (i = 2; i <= result[4]; i++) {
                        var print_star = star_A + i + star_B;
                        var print_end = end_A + i + end_B;
                        succe_star += print_star + ",";
                        succe_end += print_end + ",";
                    }
                    succe_star = succe_star.substring(0, succe_star.length - 1);
                    succe_end = succe_end.substring(0, succe_end.length - 1);
                    ShowSuccessfulInfo(true, "MB (" + result[2] + "," + succe_star + ") ~ (" + result[3] + "," + succe_end + ") " + mesIsPrinted);
                }
                else {
...
function reprint() {
    var url = "FPCBoardLabelPrint.aspx" + "?Station=" + stationId + "&PCode=" + pCode + "&UserId=" + editor + "&Customer=" + customer + "&AccountId=" + AccountId + "&Login=" + Login;
...
function OnCancel() {
    WebServiceFPCBoardLabelPrint.cancel(getMOCmbValue());
}
