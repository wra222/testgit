...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="UnpackDNbySN.aspx.cs" Inherits="PAK_UnpackDN" Title="Untitled Page" %>
...
var msgSnNoExistErr = '<%=this.GetLocalResourceObject(Pre + "_msgSnNoExistErr").ToString()%>';
//var 
var sn = "";
...
function btnUnpackClick()
{
    ...
    {    
            ...
            UnpackService.UnpackDNbySNCheck(sn, "<%=UserId%>", station, "<%=Customer%>", onSuccess, onFail);   
    ...
    try 
    {
        ...
        else if (result==SUCCESSRET)
        {
            ...
            if (confirm(msgConfirmUnpack)) {
                ...
                //alert("save active");
                UnpackService.UnpackDNbySNSave(sn, station, onSaveSuccess, onFail);
            }
            else {
                //alert("cancle active");
...
function onSaveSuccess(result) {
    ...
    try {
        ...
        if (result[1] == "0")
            ...
            ShowInfo("Unpack OK，请重新上传船务至SAP!");
        }
        inputControl.focus();
        getAvailableData("processDataEntry");
    }
