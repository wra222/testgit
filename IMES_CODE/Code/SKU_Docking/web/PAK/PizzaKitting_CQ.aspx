...
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true"
    CodeFile="PizzaKitting_CQ.aspx.cs" Inherits="PAK_PizzaKitting_CQ" Title="无标题页" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" runat="Server">
    ...
    <script language="javascript" type="text/javascript">
		...
		var hostname = getClientHostName();
        	var msgInputPizzaID ="Please scan Pizza ID!!";
        //error message
        ...
        window.onload = function() {
            ...
            pCode = '<%=Request["PCode"] %>';
            PartNotMatchResetPage = '<%=Request["PartNotMatchResetPage"] %>';
        ...
        function checkSN(inputData) {
            var flag = false;
            if (inputData.length == 10 && CheckCustomerSN(inputData)) {
                flag = true;
            }
           // else if (inputData.length == 11 && (inputData.substring(0, 4) == "S5CG" || inputData.substring(0, 4) == "A5CG" || inputData.substring(0, 4) == "P5CG")) {
            else if (inputData.length == 11 && CheckCustomerSNinPizzaForCQ(inputData)) {
                flag = true;
            }
               return flag;
       //     return CheckCustomerSN(inputData);
        }

        function inputCustomSN(inputData) {

            if (!checkSN(inputData)) {
                alert(msgNoCustSN);
                callNextInput();
                return;
            }
...
//            }
        ...
        function onCustSNSucc(result) {
            ...
            if (result[0] == SUCCESSRET) {
				...
				if (('Y' == IsBSam) && ('PK04' == station)) {
				    ...
				    StartPrint('Success. [' + customerSN + '] is BSaM, start Lighting', result);
				}
				else {
        ...
        function inputPizzaID(inputData) {
            ...
            if (inputData.length == 10) {
                ...
                beginWaitingCoverDiv();
                WebServicePizzaKitting.InputPizzaID_NoWaitingInput(productID, inPizzaID, line, station, model, editor, stationId, customer, lstPrintItem, onPizzaIDSucc, onPizzaIDFail);
            }
            else {
                alert(msgNoCustSN);
                callNextInput();
        ...
        function StartPrint(msg, result) {
            try {
