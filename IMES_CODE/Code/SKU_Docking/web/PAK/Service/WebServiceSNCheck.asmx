...
public class WebServiceSNCheck : System.Web.Services.WebService
{
    ...
    public ArrayList inputCustSNOnProduct2(string custsn, string pdLine, string editor, string stationId, string customerId)
    {
        ...
        try
        {
            ArrayList ret2 = iSNCheck.InputCustSNOnProduct2(custsn, pdLine, editor, stationId, customerId);
            ret.Add(WebConstant.SUCCESSRET);
            ret.Add(ret2[0].ToString()); // Product ID
            ret.Add((bool)ret2[1]); // China Label
            ret.Add(ret2[2]); // Carton PartNo List
            return ret;
        }
        catch (FisException ex)
        {
    ...
    [WebMethod]
    public ArrayList inputCustSNOnPizzaReturn(string custsnOnPizza, string custsnOnProduct, string pdLine, string editor, string stationId, string customerId,string cartonPn)
    {
        ArrayList ret = new ArrayList();

        try
        {
            string retInfo = iSNCheck.InputCustSNOnPizzaReturn(custsnOnPizza, custsnOnProduct, pdLine, editor, stationId, customerId,cartonPn);
            ret.Add(WebConstant.SUCCESSRET);
            ret.Add(retInfo);
            return ret;
        }
    ...
    [WebMethod]
    public ArrayList inputCustSNOnPizzaReturnPrint(string custsnOnPizza, string custsnOnProduct, string pdLine, string editor, string stationId, string customerId,string cartonPn, IList<PrintItem> items)
    {
        ArrayList ret = new ArrayList();
        IList<PrintItem> backPrintItems = null;
        PrintItem piEnergyLabel = new PrintItem();
        string SNCheckReturn = "";

        try
        {
            backPrintItems = iSNCheck.InputCustSNOnPizzaReturn(custsnOnPizza, custsnOnProduct, pdLine, editor, stationId, customerId,cartonPn, items);
            if (backPrintItems != null)
            {
                for (int i = 0; i < backPrintItems.Count; i++)
                {
                    ...
                    else if (backPrintItems[i].LabelType == "SNCheckReturn")
                    {
                        SNCheckReturn = backPrintItems[i].TemplateName;
                    }
                }
    ...
    [WebMethod]
    public ArrayList InputCustsnForCQ(string custsn, string pdLine, string editor, string stationId, string customerId,bool isPrintEnergy)
    {
        ArrayList ret = new ArrayList();

        try
        {
            string prodID = iSNCheck.InputCustsnForCQ(custsn, pdLine, editor, stationId, customerId, isPrintEnergy);
            ret.Add(WebConstant.SUCCESSRET);
            ret.Add(prodID);
            return ret;
        }
    ...
    [WebMethod]
    public ArrayList InputCustsnOnPizzaForCQ(string custsnOnPizza, string custsnOnProduct, string pdLine, string editor, string stationId, string customerId, IList<PrintItem> items)
    {
        ArrayList ret = new ArrayList();
        try
        {
          //  string retInfo = iSNCheck.InputCustsnOnPizzaForCQ(custsnOnPizza, custsnOnProduct, pdLine, editor, stationId, customerId);
           ArrayList arr = iSNCheck.InputCustsnOnPizzaForCQ(custsnOnPizza, custsnOnProduct, pdLine, editor, stationId, customerId, items);
            // 0: ret 1:energy label 2:printitem
            ret.Add(WebConstant.SUCCESSRET);
            ret.Add(arr[0]);
            ret.Add(arr[1]);
            ret.Add(arr[2]);
