...
public class WebServicePizzaKitting : System.Web.Services.WebService
{
    ...
    public IList inputSN(string pdLine, string curStation, string custSN, string editor, string station, string customer)
    {
        ...
        try 
        {
			...
			if("Y".Equals(tmpList[2]) && "PK04".Equals(curStation)){
				ret.Add(tmpList[0]);//DataModel.ProductInfo // ret idx 1
				ret.Add(tmpList[1]);//wipBufferList // ret idx 2
			}
			else{
    ...
    [WebMethod]
    public IList inputPizzaCheckSN(string pdLine, string curStation, string custSN, string editor, string station, string customer)
    {
        ArrayList ret = new ArrayList();
        ...
        try
        {
            ArrayList tmpList = pizza.InputPizzaCheckSN(custSN, pdLine, curStation, editor, station, customer);
    ...
    [WebMethod]
    public IList inputPizzaCheckSNforSorting(string pdLine, string curStation, string custSN, string editor, string station, string customer)
    {
        ArrayList ret = new ArrayList();
        try
        {
            ArrayList tmpList = pizza.InputPizzaCheckSNforSorting(custSN, pdLine, curStation, editor, station, customer);
            //    ArrayList tmpList=new ArrayList();
            ret.Add(WebConstant.SUCCESSRET); // ret idx 0
    ...
    [WebMethod]
    public IList InputPizzaID_NoWaitingInput(string productID, string pizzaID, string line, string curStation, string model,
                                string editor, string stationId, string customer, IList<PrintItem> printItems)
    {
        try
        {
            IList ret = new ArrayList();
            ArrayList tempLst= pizza.InputPizzaID(productID, pizzaID, line, curStation, model, editor, stationId, customer,printItems);
           // pizza.InputPizzaID(productID, pizzaID, line, curStation, model, editor, stationId, customer);
            ret.Add(WebConstant.SUCCESSRET);
            ret.Add(tempLst[0]);//printList
            ret.Add(tempLst[1]);//labelList
    ...
    [WebMethod]
    public IList CheckPartByResetLevel(string productID, string part,string resetLevel)
    {
        try
        {
            IList ret = new ArrayList();

            MatchedPartOrCheckItem item = pizza.TryPartMatchCheck(productID, part, resetLevel);
            ret.Add(WebConstant.SUCCESSRET);
            ret.Add(item);
                                ...
                                string editor, string station, string customer,IList<PrintItem> printItems)
    {
        ...
        try
        {
            ...
            retLst.Add(tempLst[1]);//labelList
        }
        catch (FisException ex)
        {
            throw new Exception(ex.mErrmsg);
