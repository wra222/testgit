<%--
 ...
 Description: VGA Label Print(SA)
 Update: 
 Date        Name                 Reason 
 ==========  ==================   ==============================
 2012-01-06  zhu lei              Create
 2012-04-10  Li.Ming-Jun          ITC-1360-1606
 Known issues:
 --%>

<%@ MasterType VirtualPath="~/MasterPage.master" %>
<%@ Page Language="C#" MasterPageFile="~/MasterPage.master"  ContentType="text/html;Charset=UTF-8" AutoEventWireup="true" CodeFile="VGALabelPrint.aspx.cs" Inherits="SA_VGALabelPrint"  Title="无标题页"%>
...
function print()
{
    try {
        ...
        if (!errorFlag) {
            ...
            WebServiceVGALabelPrint.print(getPdLineCmbValue(), month, mbSno, getMOCmbValue(), qty, dateCode, editor, stationId, customer, get111LevelCmbValue(), factor, lstPrintItem, onSucceed, onFail);
        }
    } catch(e) {
        alert(e);
    }
...
//            setPrintParam(result[1], "VGA Label", keyCollection, valueCollection);
//            printLabels(result[1], false);

            var printList = new Array();
            if (result[1] && result[1].length > 0) {
                for (var i = 0; i < result[2].length; i++) {
                    printList[i] = new PrintItem(result[1][0].PrintMode, result[1][0].RuleMode, result[1][0].LabelType, result[1][0].TemplateName, result[1][0].Piece, result[1][0].SpName, null, null, result[1][0].OffsetX, result[1][0].OffsetY, result[1][0].PrinterPort, result[1][0].dpi, result[1][0].BatPiece);
                    printList[i].ParameterKeys = new Array("@PCBNo");
                    printList[i].ParameterValues = new Array(new Array(result[2][i]));
                }
                printLabels(printList, true);

                var end = result[2].length -1;
                //ShowSuccessfulInfo(true, result[0]);
                ShowSuccessfulInfo(true, "MB (" + result[2][0] + ") ~ (" + result[2][end] + ") " + mesIsPrinted);
            }
        }
        else {
            ShowInfo("");
...
function reprint() {
    var url = "../SA/VGALabelReprint.aspx" + "?Station=" + stationId + "&PCode=" + pCode + "&UserId=" + editor + "&Customer=" + customer + "&AccountId=" + AccountId + "&Login=" + Login;
...
function OnCancel() {
    WebServiceVGALabelPrint.cancel(getMOCmbValue());
}
