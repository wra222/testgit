...
public class WebServicePCAICTInput : System.Web.Services.WebService
{
    ...
    [WebMethod]
    public IList input(string MBSno, string ecr, string dCode, string editor, string line, string station,
        string customer, string PCBVer, string pilotmodel, bool IsPilotMo)
    {
        ...
        try
        {
            ...
            iPCAICTInput.InputMBSno(MBSno, ecr, warrantyID, editor, line, station, customer, PCBVer, pilotmodel, IsPilotMo);
    ...
    [WebMethod]
    public IList save(bool IsRCTO, string mbSno, string aoi, IList<string> defectList, IList<PrintItem> printItems, bool IsPilotMo, string PilotMo, int PilotMoQty)
    {
        IList ret = new ArrayList();
        ...
        {
            iPCAICTInput = ServiceAgent.getInstance().GetObjectByName<IICTInput>(WebConstant.PCAICTInputObject);
            var retLst = iPCAICTInput.Save(IsRCTO, mbSno, aoi, defectList, printItems, IsPilotMo, PilotMo, PilotMoQty);
            ret.Add(WebConstant.SUCCESSRET);
            ret.Add(retLst[0]);
            ret.Add(retLst[1]);
            ret.Add(retLst[2]);
    ...
    [WebMethod]
    public IList CheckPCBVer(string MBSno)
    {
        try
        {
            IList ret = new ArrayList();
            string s = "false";
            iPCAICTInput = ServiceAgent.getInstance().GetObjectByName<IICTInput>(WebConstant.PCAICTInputObject);
            if (iPCAICTInput.CheckPCBVer(MBSno))
            {s = "true";}
            else
            {s = "false";}
            ret.Add(s);        
    ...
    [WebMethod]
    public IList GetPilotMoInfo(string pilotmo, string mbsn)
    {
        try
        {
            IList ret = new ArrayList();
            iPCAICTInput = ServiceAgent.getInstance().GetObjectByName<IICTInput>(WebConstant.PCAICTInputObject);
            PilotMoInfo temp = iPCAICTInput.GetPilotMo(pilotmo, mbsn);
            ret.Add(temp.mo);
            ret.Add(temp.model);
            ret.Add(temp.combinedQty);
            ret.Add(temp.causeDescr);
            return ret;
        }
        catch (FisException ex)
        {
