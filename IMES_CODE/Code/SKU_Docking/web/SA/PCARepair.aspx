...
<asp:Content ID="Content1" ContentPlaceHolderID="iMESContent" runat="Server">
    ...
    <script type="text/javascript">        
        ...
        function getDefectInfo() {
            ...
            var valueArray = tblObj.rows[selRowNo].cells[6].innerText.split("\u0003");
        ...
        function canSave() {
            ...
            tblObj = document.getElementById("<%=GridViewExt1.ClientID %>");
            for (i = 1; i <= cnt; i++) {
                if (tblObj.rows[i].cells[3].innerText.trim() == "") return false;
            }
            return true;
        }
        ...
        function isExistDefectInTable(defect, objId)
        {
            ...
            for (i = 0; i < recordCount; i++)
            {
                if (tblObj.rows[i + 1].cells[1].innerText.indexOf(defect) == 0)
                {
                    if (status == "E")
                    {
                        id = tblObj.rows[i + 1].cells[6].innerText.split("\u0003")[0];
