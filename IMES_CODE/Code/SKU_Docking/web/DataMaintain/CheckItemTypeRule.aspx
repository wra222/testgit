...
<script type="text/javascript" src="../UserManager/commoncontrol/function/commonFunction.js"></script>
    ...
    <div id="container" class="iMes_div_MainTainContainer">
        ...
        <div id="div3">
            <table width="100%" class="iMes_div_MainTainEdit" >
                    ...
                    <tr>
                        <td colspan="2">
                            <asp:Label ID="lblFilterExpression" runat="server" CssClass="iMes_label_13pt" Width ="98%"></asp:Label>
                        </td>
                        <td colspan="2">
                            <%--<asp:TextBox ID="txtMatchRule" runat="server" MaxLength="255" Width="98%"></asp:TextBox>--%>
                            <input type="text" id="txtFilterExpression" runat="server" maxlength="255" style="width:98%" />
                        </td>
                        <td colspan="2">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="button" id="btntestExpression" runat="server" onclick="ShowDialog2()" class="iMes_button" onmouseover="this.className='iMes_button_onmouseover'" onmouseout="this.className='iMes_button_onmouseout'" ></input>
                        </td>
                    </tr>
            </table>
        </div>  
   ...
   function clkSave()
   {
    ...
    function setNewItemValue()
    {
        ...
        document.getElementById("<%=txtFilterExpression.ClientID %>").value = "";
    ...
    function ShowRowEditInfo(con)
    {
        ...
        document.getElementById("<%=txtFilterExpression.ClientID %>").value = con.cells[6].innerText.trim();

        document.getElementById("<%=txtMatchRule.ClientID %>").value = con.cells[7].innerText.trim();
        document.getElementById("<%=txtCheckRule.ClientID %>").value = con.cells[8].innerText.trim();
        document.getElementById("<%=txtSaveRule.ClientID %>").value = con.cells[9].innerText.trim();
        document.getElementById("<%=cmbNeedUniqueCheck.ClientID %>").value = con.cells[10].innerText.trim();
        document.getElementById("<%=cmbNeedCommonSave.ClientID %>").value = con.cells[11].innerText.trim();

        document.getElementById("<%=cmbNeedSave.ClientID %>").value = con.cells[12].innerText.trim();
        document.getElementById("<%=cmbCheckTestKPCount.ClientID %>").value = con.cells[13].innerText.trim();

        document.getElementById("<%=txtDescr.ClientID %>").value = con.cells[14].innerText.trim();
        var currentID = con.cells[18].innerText.trim();
        if (currentID == "") {
            setNewItemValue();
        }
        else {
    ...
    function AddUpdateComplete(id)
    {
        ...
        for (var i = 1; i < gdObj.rows.length; i++) {
            if (gdObj.rows[i].cells[18].innerText == id) {
                selectedRowIndex = i;
                break;
            }
        }
    ...
    function ShowDialog() {
        var ret = window.showModalDialog("CheckItemTypeRuleforTestREDialog.aspx?AccountId=111&UserName=" + document.getElementById("<%=HiddenUserName.ClientID %>").value + "&UserId=" + editor, 0, "dialogwidth:1000px; dialogheight:200px;status:no;help:no; ");
    }
    function ShowDialog2() {
        var ret = window.showModalDialog("CheckItemTypeRuleforTestExpressionDialog.aspx?AccountId=111&UserName=" + document.getElementById("<%=HiddenUserName.ClientID %>").value + "&UserId=" + editor, 0, "dialogwidth:1000px; dialogheight:200px;status:no;help:no; ");
    }
    </script>
</asp:Content>
