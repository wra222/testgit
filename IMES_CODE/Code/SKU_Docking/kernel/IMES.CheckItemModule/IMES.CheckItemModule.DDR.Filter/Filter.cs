...
namespace IMES.CheckItemModule.DDR.Filter
{
    ...
    {
        private const string part_check_type = "DDR";
        ...
        {
            //描述同上，不同的是(Descr like 'MEM%' or Descr like 'RAM%'）
            if (((BOMNode)node).Part == null)
            {
                return false;
            }
            bool is_contain_mem = ((BOMNode)node).Part.Descr.Trim().Contains("MEM");
            int start_point_mem = ((BOMNode)node).Part.Descr.Trim().IndexOf("MEM");
            bool is_contain_ram = ((BOMNode)node).Part.Descr.Trim().Contains("RAM");
            int start_point_ram = ((BOMNode)node).Part.Descr.Trim().IndexOf("RAM");

            if ((is_contain_mem && start_point_mem == 0) || (is_contain_ram && start_point_ram == 0))
                return true;
            return false;
        }
