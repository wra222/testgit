...
namespace IMES.CheckItemModule.CQ.LeapMotion.Filter
{
    ...
    public class Filter : IFilterModule
    {
        //private int qty = 1;//问题：Pizza Kitting。该情况下，如何计算qty。
          private const string part_check_type = "LeapMotion";
            ...
            try
            {
                if (bom.FirstLevelNodes != null)
                {
                    for (int i = 0; i < bom.FirstLevelNodes.Count; i++)
                    {
                        ...
                        if (part != null && part.BOMNodeType.Trim().Equals("PL") && "TPM".Equals(part.Descr) && (0 == part.PN.IndexOf("6")))
                        {
        ...
        public bool CheckCondition(object node)
        {
            ...
            if (part_infos != null)
            {
                ...
                {
                    if ("TYPE".Equals(part_info.InfoType) && "LEAP MOTION".Equals(part_info.InfoValue.ToUpper()))
                    {
                        return true;
                    }
                }
