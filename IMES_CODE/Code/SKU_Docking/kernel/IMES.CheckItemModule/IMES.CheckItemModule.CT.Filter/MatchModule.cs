...
namespace IMES.CheckItemModule.CT.Filter
{
    [Export(typeof(IMatchModule))]
    [ExportMetadata("ProgramName", "IMES.CheckItemModule.CT.Filter.dll")]
            ...
            if (subject.Length == 14)
            {
                ...
                if (coa_status == null)
                {
                    foreach (IPart flat_bom_item in flat_bom_items)
                    {
                        ...
                        if (flat_bom_item.Attributes.Any(x => x.InfoType == "VendorCode" &&
