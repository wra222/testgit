...
namespace IMES.CheckItemModule.CQ.OfflineHDD.Filter
{
    ...
    {
        private const string part_check_type = "HDD";
        ...
        public object FilterBOM(object hierarchical_bom, string station, object main_object)
        {
            ...
            IProduct iprd = (IProduct)session.GetValue(Session.SessionKeys.Product);
            ...
            IList<IMES.DataModel.ConstValueTypeInfo> lstConst = partRepository.GetConstValueTypeList("AllowHDDCopyStation");
            bool contains = lstConst.Any(p => p.value == iprd.Status.StationId);
            if (!contains)
            {
                throw new FisException("Station must be configured in ConstValueType with Type=AllowHDDCopyStation.");
            }
...
//                                                    }
        ...
        public bool CheckCondition(object node)
        {
            ...
            bool is_contain_hdd = ((BOMNode)node).Part.Descr.Trim().Contains("HDD");
            int start_point = ((BOMNode)node).Part.Descr.Trim().IndexOf("HDD");
