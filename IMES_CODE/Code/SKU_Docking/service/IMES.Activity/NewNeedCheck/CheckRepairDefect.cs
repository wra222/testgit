...
namespace IMES.Activity
{
    ...
    ///             3.与 Faulty Part Sno匹配到相同的part no作part check(對於new sno是否已經使用還沒有做出定義)
    ///         PAQC Repair站：
            ...
            try
            {
                ...
                if (string.Compare(RepairStationType, "SA", true) == 0)
                {
                    ...
                    if (!string.IsNullOrEmpty(defect.OldPartSno) && !string.IsNullOrEmpty(defect.NewPartSno))
                    {
                        ...
                        if ("CPU".Equals(defect.MajorPart))
                        {
                    ...
                    if ("CPU".Equals(defect.MajorPart))
                    {
                        ...
                        {
                            throw new FisException("CPU Not combined !");
                        }

                        string inputCpuOld = defect.OldPartSno;
                        if (string.IsNullOrEmpty(inputCpuOld))
                            inputCpuOld = defect.OldPart;

                        if (!mb.CVSN.Equals(inputCpuOld))
                        {
                            throw new FisException("CPU Not Equal !");
                        }

                        string inputCpuNew = defect.NewPartSno;
                        if (string.IsNullOrEmpty(inputCpuNew))
                            inputCpuNew = defect.NewPart;
                        if (string.IsNullOrEmpty(inputCpuNew))
                        {
                            throw new FisException("New CPU SN was Empty !");
                        }
                        session.AddValue(Session.SessionKeys.CPUVendorSn, inputCpuNew);//存入CPU,后面需要check 是否有仓库入料记录使用
                ...
                {                    
                    ...
                    {                        
                            ...
                            !string.IsNullOrEmpty(defect.OldPartSno.Trim()))
                        {
                            ...
                            if (!defect.PartType.Trim().Equals("MB"))
                            {
                                if ((null != mb_repository.Find(defect.OldPartSno.Trim())) || (null != mb_repository.Find(defect.NewPartSno.Trim())))
                                {
                                    List<string> errpara = new List<string>();
                                    errpara.Add(defect.NewPartSno);
                                    FisException e = new FisException("CQCHK1083 ", errpara);
                                    e.stopWF = false;
                                    throw e;
                                }
                                ...
                                if (!String.IsNullOrEmpty(checkItemType))
                                {
                                    ...
                                    if (bom == null)
                                    {
                                        FisException e = new FisException("HierarchicalBOM is null");
                                        e.stopWF = false;
                                        throw e;
                                    ...
                                    if (policy == null)
                                    {
                                        FisException e = new FisException("IPartPolicy is null");
                                        e.stopWF = false;
                                        throw e;
                                    ...
                                    if (fBom == null)
                                    {
                                        FisException e = new FisException("IFlatBOM is null");
                                        e.stopWF = false;
                                        throw e;
                                    ...
                                    else
                                    {
                                        ...
                                        IList<PartForbidPriorityInfo> partForbidPriorityInfoList = partRep.GetPartForbidWithFirstPriority(this.Customer,
                                                                                                                                                                                    iProductTarget.Status.Line,
                                                                                                                                                                                    iProductTarget.Family,
                                                                                                                                                                                    iProductTarget.Model,
                                                                                                                                                                                    iProductTarget.ProId);
                                        foreach (FlatBOMItem bomItem in fBom.BomItems)
                                        {
                                            //FlatBOMItem bomItem = item as FlatBOMItem;
                                            if (bomItem != null)
                                            {
                                                bomItem.GetType().GetField("_policy", BindingFlags.NonPublic | BindingFlags.Instance).SetValue(bomItem, policy);
                                                ...
                                                bomItem.GetType().GetField("_sessionKey", BindingFlags.NonPublic | BindingFlags.Instance).SetValue(bomItem, session.Key);
                                            }
                                        }
                                    }
                                    ...
                                    if (oldUnit == null)
                                    {
                                        FisException e = new FisException("OldPart is null");
                                        e.stopWF = false;
                                        throw e;
                                    ...
                                    if (newUnit == null)
                                    {
                                        FisException e = new FisException("NewPart is null");
                                        e.stopWF = false;
                                        throw e;
                                ...
                                {
                                    FisException e = new FisException("CheckItemType is null");
                                    e.stopWF = false;
                                    throw e;
