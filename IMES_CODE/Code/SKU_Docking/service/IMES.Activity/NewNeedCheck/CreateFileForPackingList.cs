...
namespace IMES.Activity.Special
{
    ...
    public partial class CreateFileForPackingList : BaseActivity
    {
            ...
            switch (InputType)
            {
                ...
                case InputTypeEnum.DNForPL:
                    ...
                    foreach (string temp in processList)
                    {
                        ...
                        IList<string> xsdList = new List<string>();
                        ...
                        foreach (string d in docsetnumberList)
                        {
                            ...
                            GetXslAndXsd(temp, d, ref tempList, ref xsdList);
                        ...
                        {
                            // mantis 2109
                            if (temp.ToUpper().IndexOf("SUMMARY") > 0)
                            {
                                continue;
                            }
                        ...
                        foreach (VPakComnInfo v in vpakList)
                        {
                            ...
                            //        && !String.IsNullOrEmpty(v.waybill_number))
                            {
                                ...
                                if (String.IsNullOrEmpty(v.shipment) && String.IsNullOrEmpty(v.waybill_number) && String.IsNullOrEmpty(v.PackId))
                                {
                                    pdfFileName = dnForPL.TrimEnd() + "-" + "-" +"-"+ "-[" + temp.Replace(" ", "") + "].pdf";
                                }
                                else if (String.IsNullOrEmpty(v.shipment) && !String.IsNullOrEmpty(v.waybill_number) && String.IsNullOrEmpty(v.PackId))
                                {
                                ...
                                if (String.IsNullOrEmpty(v.shipment) && String.IsNullOrEmpty(v.waybill_number) && !String.IsNullOrEmpty(v.PackId))
                                {
                                    pdfFileName = dnForPL.TrimEnd() + "-" + "-" + "-" +v.PackId.TrimEnd()+ "-[" + temp.Replace(" ", "") + "].pdf";
                                }
                                else if (String.IsNullOrEmpty(v.shipment) && !String.IsNullOrEmpty(v.waybill_number) && !String.IsNullOrEmpty(v.PackId))
                                {
                                    pdfFileName = dnForPL.TrimEnd() + "-" + "-" + v.waybill_number.TrimEnd() + "-" +v.PackId.TrimEnd()+ "-[" + temp.Replace(" ", "") + "].pdf";
                                }
                                else if (!String.IsNullOrEmpty(v.shipment) && String.IsNullOrEmpty(v.waybill_number) && !String.IsNullOrEmpty(v.PackId))
                                {
                                    pdfFileName = dnForPL.TrimEnd() + "-" + v.shipment.TrimEnd() + "-" + "-"+v.PackId.TrimEnd() + "-[" + temp.Replace(" ", "") + "].pdf";
                                }
                                else
                                {
                                    pdfFileName = dnForPL.TrimEnd() + "-" + v.shipment.TrimEnd() + "-" + v.waybill_number.TrimEnd() + "-" +v.PackId.TrimEnd()+ "-[" + temp.Replace(" ", "") + "].pdf";
                                }
                                //4.)生成pdf文件
                ...
                foreach (string temp in internalId)
                {
                    ...
                    else
                    {
                        ...
                        foreach (string num in docnumList)
                        {
                            ...
                            GetXslAndXsd(type, num, ref tempList, ref xsdList);
                            if (tempList != null && tempList.Count != 0)
                            {
                                bFind = true;
                                break;
                        ...
                        foreach (VPakComnInfo v in pList)
                        {
                            ...
                            //        && !String.IsNullOrEmpty(v.waybill_number) && v.internalID.Length >= 10)
                            {
                                ...
                                if (String.IsNullOrEmpty(v.shipment) && String.IsNullOrEmpty(v.waybill_number) && !String.IsNullOrEmpty(v.PackId))
                                {
                                    pdfFileName = v.internalID.Substring(0, 10).TrimEnd() + "-" + "-" + "-" +v.PackId.TrimEnd()+ "-[" + type.Replace(" ", "") + "].pdf";
                                }
                                else if (String.IsNullOrEmpty(v.shipment) && !String.IsNullOrEmpty(v.waybill_number) && !String.IsNullOrEmpty(v.PackId))
                                {
                                    pdfFileName = v.internalID.Substring(0, 10).TrimEnd() + "-" + "-" + v.waybill_number.TrimEnd() + "-"+v.PackId.TrimEnd() + "-[" + type.Replace(" ", "") + "].pdf";
                                }
                                else if (!String.IsNullOrEmpty(v.shipment) && String.IsNullOrEmpty(v.waybill_number) && !String.IsNullOrEmpty(v.PackId))
                                {
                                    pdfFileName = v.internalID.Substring(0, 10).TrimEnd() + "-" + v.shipment.TrimEnd() + "-" + "-"+v.PackId.TrimEnd() + "-[" + type.Replace(" ", "") + "].pdf";
                                }
                                else
                                {
                                    pdfFileName = v.internalID.Substring(0, 10).TrimEnd() + "-" + v.shipment.TrimEnd() + "-" + v.waybill_number.TrimEnd()+"-"+v.PackId.TrimEnd() + "-[" + type.Replace(" ", "") + "].pdf";
                                }
                                //pdfFileName = @delivery+'-'+@shipment+'-'+@waybill &"-["& replace(cmbdoctpye.value," ","")&"].pdf"
        ...
        private void GetXslAndXsd(string DOC_CAT, string DOC_SET_NUMBER, ref IList<string> tempList, ref IList<string> xsdList)
        {
            string strSQL = "SELECT XSL_TEMPLATE_NAME, SCHEMA_FILE_NAME FROM [PAK.PAKRT] WHERE DOC_CAT = @DOC_CAT AND DOC_SET_NUMBER = @DOC_SET_NUMBER";
            SqlParameter paraNameDOC_CAT = new SqlParameter("@DOC_CAT", SqlDbType.VarChar, 100);
            paraNameDOC_CAT.Direction = ParameterDirection.Input;
            paraNameDOC_CAT.Value = DOC_CAT;
            SqlParameter paraNameDOC_SET_NUMBER = new SqlParameter("@DOC_SET_NUMBER", SqlDbType.VarChar, 100);
            paraNameDOC_SET_NUMBER.Direction = ParameterDirection.Input;
            paraNameDOC_SET_NUMBER.Value = DOC_SET_NUMBER;
            DataTable tb = SqlHelper.ExecuteDataFill(SqlHelper.ConnectionString_HP_EDI, System.Data.CommandType.Text,
                strSQL, paraNameDOC_CAT, paraNameDOC_SET_NUMBER);
            if (tb != null)
            {
                foreach (DataRow row in tb.Rows)
                {
                    tempList.Add(row["XSL_TEMPLATE_NAME"].ToString().Trim());
                    xsdList.Add(row["SCHEMA_FILE_NAME"].ToString().Trim());
                }
            }
        }
    }
