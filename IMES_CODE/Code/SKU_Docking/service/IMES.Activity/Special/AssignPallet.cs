        ...
        protected internal override ActivityExecutionStatus DoExecute(ActivityExecutionContext executionContext)
        {
            ...
            if (string.IsNullOrEmpty(assignPalletNo))
            {
                if (site == "ICC")
                {
                    ...
                    paramArr[0] = new SqlParameter("DeliveryNo", curProduct.DeliveryNo);
                    paramArr[1] = new SqlParameter("Floor", CurrentSession.GetValue(Session.SessionKeys.Floor));
                    paramArr[2] = new SqlParameter("Editor", this.Editor);
                    DataTable dt = productRep.ExecSpForQuery
                        (SqlHelper.ConnectionString_PAK, "IMES_GetAutoAssignPalletByFloor", paramArr);
                    assignPalletNo = dt.Rows[0][0].ToString();
                }
                else
                {
