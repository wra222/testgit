                ...
                else //ii.	如果数量不为1，则按照如下方法分配库位
                {
                  ...
                  //  neqconf.wc = "JP";
                    ...
                    if (jpflag)
                    {
                        ...
                        conf.tp = "PakLoc";
                        conf.wc = "JP";
                        macList = palletRep.GetPakLocMasList(conf); //SELECT Cdt,Editor,FL,ID,PdLine,Pno,SnoId,Tp,Udt,WC FROM PAK_LocMas WHERE  1=1   ORDER BY CONVERT(INT,SnoId) DESC
                        if (macList.Count == 0)
                        {
                            conf.wc = "";
                    ...
                    {
                     //   macList = palletRep.GetPakLocMasList(normalconf, neqconf);//(curPallet.PalletNo, "PakLoc");
                        macList = palletRep.GetPakLocMasList(normalconf); //SELECT Cdt,Editor,FL,ID,PdLine,Pno,SnoId,Tp,Udt,WC FROM PAK_LocMas WHERE  1=1   ORDER BY CONVERT(INT,SnoId) DESC
