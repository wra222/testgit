...
namespace IMES.Activity
{
    ...
    public partial class CombineASTBlockStation : BaseActivity
	{
		...
		private void ThrowWhenATSNXYZ(string ProdidOrCustsn, bool isPPExist, bool isATSN3Exist, bool isATSN5Exist)
		{
			List<string> errpara = new List<string>();
			if (isPPExist == true)
			{
				CurrentSession.AddValue("WantFinishFlow_when_CombineAndGenerateAST", "PP"); //需要打印PP类标签
			...
			{
				CurrentSession.AddValue("WantFinishFlow_when_CombineAndGenerateAST", "ATSN3"); //需要打印ATSN3标签
			...
			{
				CurrentSession.AddValue("WantFinishFlow_when_CombineAndGenerateAST", "ATSN5"); //需要打印ATSN5标签
        ...
        protected internal override ActivityExecutionStatus DoExecute(ActivityExecutionContext executionContext)
        {
            ...
            */
            ...
            if (isExist == false)
            {
                ThrowWhenATSNXYZ(ProdidOrCustsn, isPPExist, isATSN3Exist, isATSN5Exist);
            ...
            */
            //3）检查是否结合资产标签
            //若不存在数据，则报错：“此机器：XXX（ProductID#）已经结合资产标签：XXX（AST No#）”
            //都不为null则报错
            IList<string> astList = bomRep.CheckIfExistProductPartWithBom("ATSN1", currentProduct.Model, currentProduct.ProId);
            bool nullflag = false;
            string aststr = "";
            foreach(string tmp in astList)
            {
            ...
            if (!nullflag)
            {
                ...
                errpara.Add(aststr);
                throw new FisException("CHK964", errpara);  //此机器：XXX（ProductID#）已经结合资产标签：XXX（AST No#）
            }
            //2012-4-10, for image
