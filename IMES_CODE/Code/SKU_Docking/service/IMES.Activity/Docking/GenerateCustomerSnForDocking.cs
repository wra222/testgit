            ...
            try
            {
               ...
               // string year = curDate.Year.ToString();
                ...
                else
                {
                    ...
                    {
                        custSn = "5C" + "G" + custSn + year.Substring(year.Length - 1, 1) + weekCode;
                    }
                    else //IPC 上海
                    {
                ...
                var maxObj = numControl.GetMaxValue("CPQSNO", prestr);
                //檢查有沒有lock index, 沒有lock index, 改變查詢條件
                if (maxObj == null)
                {
                    //lock NoType='CPQSNO' and NoName='Lock'
                    var data = numControl.GetMaxValue("CPQSNO", "Lock");
                    maxObj = numControl.GetMaxValue("CPQSNO", prestr);
                }
                ...
                else
                {
                    ...
                    temstr = GetSN(temstr);
                    int cntGetSN = 0;
                    while (regex.Match(temstr.ToUpper().Substring(temstr.Length - 3)).Success)
                    {
                        cntGetSN++;
                        if (cntGetSN > 3)
                        {
