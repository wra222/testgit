            ...
            string editor, string stationId, string customer, IList<PrintItem> items)
        {
            ...
            try
            {
                ...
                if (session == null)
                {
                    ...
                    RouteManagementUtils.GetWorkflow(stationId, "PickCard.xoml", "PickCard.rules", out wfName, out rlName);
                    ...
                    session.AddValue("SessionEditsPrintName", "EditsPosLabel");
                    session.AddValue("SessionEditsPrintDescr", dateStr);
                ...
                string xsl = "";
                bool needEdits = "Y".Equals(session.GetValue("IsAutoBOL") as string);
                if (needEdits)
                {
                    IPizzaRepository repPizza = RepositoryFactory.GetInstance().GetRepository<IPizzaRepository, Pizza>();
                    IList<string> tempList = repPizza.GetXslTemplateNameListFromPakDashPakComnByDocCatAndDocSetNumer("POS", "DS-NA-00001");
                    if (tempList.Count > 0)
                        xsl = tempList[0];
                }
				...
				lstRet.Add(session.GetValue("IsAutoBOL") as string); //是自動單?
                lstRet.Add(xsl);
                return lstRet;
            }
