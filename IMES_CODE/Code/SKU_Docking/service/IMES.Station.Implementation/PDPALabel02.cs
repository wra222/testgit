...
namespace IMES.Station.Implementation
{
    ...
    public class PDPALabel02 : MarshalByRefObject, IPDPALabel02
    {
        ...
        /// <returns></returns>
        public ArrayList InputSNforCQ(Boolean queryflag, string custSN, string line, string code, string floor,
                                                    string editor, string station, string customer)
        {
            logger.Debug("(PDPALabel01)InputSNforCQ start, custSn:" + custSN);
            string prdID="";
            try
            {
                //TEST
                ...
                IList<WipBuffer> defList = (IList<WipBuffer>)productRep.GetWipBufferByPnoListAndCode(PnList.ToArray(), code);
                var partList = defList.Select(x => x.PartNo).Distinct();
                IPartRepository PartRepository = RepositoryFactory.GetInstance().GetRepository<IPartRepository, IPart>();
              //  S_TpPartNo tpPn = new S_TpPartNo();
                List<S_TpPartNo> tpPnLst = new List<S_TpPartNo>();
                foreach (string pn in partList)
                {
                    if (PartRepository.GetPartInfoValue(pn, "MC") != null)
                    {
                        tpPnLst.Add(new S_TpPartNo
                        {
                            Tp = PartRepository.GetPartByPartNo(pn).Descr,
                            PartNo = PartRepository.GetPartInfoValue(pn, "MC").Trim()
                        });
                    }
                ...
                List<S_TpPartNo> tpPnLst2 = (from ks in tpPnLst select ks).Distinct().ToList();
                if (tpPnLst2.Count == 0)
                {
                    // mantis 424, PD PA Label 新机种无CPU Label
					/*
					cancel(prdID);
                    throw new FisException("PAK182", new string[] {curProduct.Model, "CPU Label" });
					*/
                }
                                                    ...
                                                    string editor, string station, string customer)
        {
            ...
            try
            {
                ...
                foreach (IBOMNode bomNode in bomNodeList)
                {
                    ...
                    if (bomNode.Part.BOMNodeType == "AT")
                    {
                        if (bomNode.Part.Descr.Length >= 5)
                        {
                            if (bomNode.Part.Descr.ToUpper().Substring(0, 5) == "ATSN4")
                            {
                                AT4Check = true;
                            }
                        }
                    ...
                    if (wwanCheck && hitachiCheck && AT4Check)
                    {
                        break;
                    }
                }
                ...
                if ((tmpstr == "29") || (tmpstr == "39") && CheckJapanByPart(curProduct.Model) )
                {
                    SNCheck = true;
                }
        ...
        public ArrayList checkCOA(string prodId, string WWANSN)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    IList<WipBufferDef> wipBufferList=null;
                    if (site != "ICC")
                    { 
						wipBufferList = (IList<WipBufferDef>)curSession.GetValue("WipBuffer"); 
					}
                    ...
                    retList.Add(CheckNeedPrintAST(curProduct.Model));
        ...
        private string CheckNeedPrintAST(string model)
        {
            string strSQL = @"select  c.*  From ModelBOM a,Part b,AstDefine c where 
                              a.Component=b.PartNo and b.Descr=c.AstCode and c.CombineStation='PKAT'and c.NeedPrint='Y'
                             and a.Material=@model ";
            SqlParameter paraName = new SqlParameter("@model", SqlDbType.VarChar, 32);
            paraName.Direction = ParameterDirection.Input;
            paraName.Value = model;
            ...
            if (dr.HasRows)
            {
                while (dr.Read())
                {
                    astlist = dr["AstCode"].ToString().Trim() + ";" + astlist;
