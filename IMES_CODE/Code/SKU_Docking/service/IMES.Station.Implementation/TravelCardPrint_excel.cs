...
namespace IMES.Station.Implementation
{
    ...
    /// 
    /// </summary>
    public partial class TravelCardPrintExcel : MarshalByRefObject, ITravelCardPrintExcel, IMO
    {
       ...
       // private static readonly Session.SessionType theType = Session.SessionType.Common;
                                          ...
                                          string deliveryDate, string bomremark, string remark, string exception)
        {
            ...
            try
            {
                ...
                if (currentCommonSession == null)
                {
                    ...
                    RouteManagementUtils.GetWorkflow(station, "TravelCardPrint_excel.xoml", "TravelCardPrint_excel.xoml", out wfName, out rlName);
        ...
        public IList<PrintItem> PrintTCWithProductIDForBN(string pdLine, string model, string mo,
                                          ...
                                          out string battery, out string lcm,
                                          string deliveryDate, string bomremark, string remark, string exception,
                                          string pilotmo, string pilotmoremainqty)
        {
            logger.Debug(" PrintTCWithProductID start, mo:" + mo + " ,pdLine:" + pdLine + " ,qty:" + qty.ToString());
            string currentSessionKey = mo;
            try
            {
                ...
                if (currentCommonSession == null)
                {
                    ...
                    wfArguments.Add("SessionType", Session.SessionType.Common);
                    string wfName, rlName;
                    RouteManagementUtils.GetWorkflow(station, "TravelCardPrint_excel.xoml", "TravelCardPrint_excel.rules", out wfName, out rlName);
                    WorkflowInstance instance = WorkflowRuntimeManager.getInstance.CreateXomlFisWorkflow(wfName, rlName, wfArguments);
                    ...
                    currentCommonSession.AddValue(ExtendSession.SessionKeys.DeliveryDate, deliveryDate);
                    currentCommonSession.AddValue(Session.SessionKeys.ModelName, model);
                    currentCommonSession.AddValue("IsPilotMo", false);
                    if (!string.IsNullOrEmpty(pilotmo))
                    {
                        currentCommonSession.AddValue("IsPilotMo", true);
                        currentCommonSession.AddValue(Session.SessionKeys.PilotMoNo, pilotmo);
                        if (qty >= Convert.ToInt32(pilotmoremainqty))
                        {
                            currentCommonSession.AddValue(Session.SessionKeys.PilotMoQty, Convert.ToInt32(pilotmoremainqty));
                        }
                        else
                        {
                            currentCommonSession.AddValue(Session.SessionKeys.PilotMoQty, qty);
                        }
                    }
                   string baseModel = "";
                    bool isPrintComboTravelCard = CheckIsPrintComboTravelCard(mo, out baseModel);
        ...
        public List<SerialNumberExcel> GetPrintLogProidList(string mo)
        {
            List<SerialNumberExcel> retLst = new List<SerialNumberExcel>();
            ...
            if (prtLst != null)
            {
                foreach (PrintLog s in prtLst)
                {
                    ...
                    retLst.Add(new SerialNumberExcel { beginNumber = s.BeginNo, endNumber = s.EndNo });
                }
            }
            return retLst;
        }
