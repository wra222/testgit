 ...
 * Description:Implementation for MB Combine CPU
...
{
    public class MBCombineCPU : MarshalByRefObject, IMBCombineCPU
    {
           ...
           string editor, string stationId, string customerId)
        {
            ...
            try
            {
                ...
                if (CurrentSession == null)
                {
                    ...
                    RouteManagementUtils.GetWorkflow(stationId, "MBCombineCPU.xoml", null, out wfName, out rlName);
        ...
        public string CombineCPU(string MB_SNo, string CPUVendorSN)
        {
            logger.Debug("(MBCombineCPU)MBCombineCPU start, "
                           + " [MB_SNo]: " + MB_SNo
            ...
            {
                logger.Debug("(MBCombineCPU)MBCombineCPU End, "
                           + " [MB_SNo]: " + MB_SNo
                           + " [CPUVendorSN]:" + CPUVendorSN);
            }
        }
        ...
        public void ProductCombineInputProdID(string pdLine,
         ...
         string editor, string stationId, string customerId)
        {
            logger.Debug("(MBCombineCPU)ProductCombineInputProdID start, "
            + " [pdLine]:" + pdLine
           + " [prodID]: " + prodID
           + " [editor]:" + editor
           + " [station]:" + stationId
           + " [customer]:" + customerId);
            ...
            try
            {
                ...
                if (CurrentSession == null)
                {
                    ...
                    wfArguments.Add("Customer", customerId);
                    wfArguments.Add("SessionType", TheType);
                    WorkflowInstance instance = WorkflowRuntimeManager.getInstance.CreateXomlFisWorkflow("ProductCombineCPU.xoml", null, wfArguments);
            ...
            {
                logger.Debug("(MBCombineCPU)ProductCombineInputProdID End, "
        + " [pdLine]:" + pdLine
       + " [prodID]: " + prodID
       + " [editor]:" + editor
       + " [station]:" + stationId
       + " [customer]:" + customerId);
        ...
        {
            logger.Debug("(MBCombineCPU)ProductCombineCPU start, "
               + " [prodID]: " + prodID
               + " [CPUVendorSN]:" + CPUVendorSN);
            FisException ex;
            List<string> erpara = new List<string>();
            ...
            {
                logger.Debug("(MBCombineCPU)ProductCombineCPU End, "
               + " [prodID]: " + prodID
               + " [CPUVendorSN]:" + CPUVendorSN);
            }
        }
