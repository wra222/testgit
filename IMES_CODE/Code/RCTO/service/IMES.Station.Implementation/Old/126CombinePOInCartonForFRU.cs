...
namespace IMES.Station.Implementation
{
    ...
    /// CombinePOInCarton
    /// </summary>
    public class CombinePOInCartonForFRU : MarshalByRefObject, ICombinePOInCartonForFRU
    {
        ...
        
    {
        ...
        /// </summary>
        public IList<PrintItem> Save(IList<string> NewScanedProductIDList, IList<string> NewScanedModelList, IList<string> custSNList, string line, string editor, string station, string customer, IList<PrintItem> printItems,out string cartonSN)
        {
            cartonSN = "";
            ...
            try
            {
               ...
               //     currentSession.AddValue(Session.SessionKeys.isDNFull, false);
                    ...
                    IList<string> CartonList = (IList<string>)currentSession.GetValue(Session.SessionKeys.CartonNoList);
                    cartonSN = CartonList[0];
        ...
        private bool CheckIsFRU(string ModelName)
        {

            bool result = false;
            IModelRepository myRepository = RepositoryFactory.GetInstance().GetRepository<IModelRepository, Model>();
            Model modelObj = myRepository.Find(ModelName);
            if (!string.IsNullOrEmpty(modelObj.GetAttribute("IsFRU")) && modelObj.GetAttribute("IsFRU") == "Y")
            { result = true; }
            return result;
        }
    }
