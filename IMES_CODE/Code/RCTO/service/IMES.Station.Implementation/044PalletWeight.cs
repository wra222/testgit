    ...
    public class PalletWeight : MarshalByRefObject, IPalletWeight
    {
                            ...
                            string line, string editor, string station, string customer)
        {
            ...
            try
            {
                ...
                IList<ModelStatistics> modelList = new List<ModelStatistics>();
                bool pcflag = true;
                //foreach (var item in modelList)
                //{
                //    if (item.Model.Substring(0,2) != "PC")
                //    {
                //        pcflag = false;
                //        break;
                //    }
                //}
                pcflag = string.IsNullOrEmpty(curPallet.PalletModel);
               ...
               // 如果空运出货EMEA 地区时，没有选择[Pallet Type]，则报告错误：“请选择Pallet Type!”
                ...
                if (shipWay == "T001" && (regId == "SNE" || regId == "SCE"))
                {
                    checkType = true;
                }
                ...
                //满足如下条件之一，就必须输入Carton S/N
                //1.当前Pallet 结合的所有Product 都处于Pallet Verify 完成状态
                //（可以通过查看ProductStatus.Station 来判定）
                //2.当前Pallet 结合的Delivery(s) 结合的所有Pallet 均未称重
                //（可以通过查看Pallet.Weight 来判定）

                bool cartonflag = true;
                if (!pcflag)
                {
                    IList<IMES.FisObject.FA.Product.IProduct> productList = productRep.GetProductByPalletNo(plt);
                    foreach (Product node in productList)
                    {
                        if (node.Status.StationId != "9A")
                        {
                            cartonflag = false;
                            break;
                        }
                    ...
                    if (!cartonflag)
                    {
                        cartonflag = palletRep.CheckAllPalletWeightOnDeliverysByPallet(plt);
                    }
                }
                else
                {
        ...
        /// <returns></returns>
        public ArrayList InputCustSN(string custSN, string palletNo, string type, string modelType)
        {
            ...
            try
            {
                ...
                {
                    currentSession.AddValue("PalletType", type);
                    IMES.FisObject.FA.Product.IProduct currentProduct;
                    
                    
                    //检查custsn
                    if (modelType == "PC")
                    {
                        currentProduct = CommonImpl.GetProductByInput(custSN, CommonImpl.InputTypeEnum.CustSN);
                        //1.	Check Pallet No
                    ...
                    {
                        //如果Carton (Product.CartonSN)绑定的Pallet No (Product.PalletNo)
                        //与UI 上已经录入的Pallet No 不同，则报告错误：“此栈板序号与Carton S/N 序号比对错误！”
                        // Mark for CQ Mantis 0000373: 【RCTO 需求】146 Pallet Weight
                        /*
                        currentProduct = CommonImpl.GetProductByInput(custSN, CommonImpl.InputTypeEnum.Carton);
                        if (currentProduct.PalletNo != palletNo)
                        {
                            erpara.Add(palletNo);
                            ex = new FisException("PAK168", erpara);//此栈板序号与Carton S/N 序号比对错误！
                            throw ex;
                        }
                         */
                         tmpArr = GetPalletNoInProductOrPcbOrMaterial(custSN);//0: palletno  1:carton sn 2: 1st SN
                   //     string tmpPalletNo = GetPalletNoInProductOrPcbOrMaterial(custSN);
                        if (tmpArr[0] != palletNo)
                        {
                            erpara.Add(palletNo);
                            ex = new FisException("PAK168", erpara);//此栈板序号与Carton S/N 序号比对错误！
                            throw ex;
                    ...
                    //retList.Add(currentProduct.CUSTSN);
                    //retList.Add(currentProduct.CartonSN);
                    retList.Add(tmpArr[2]);
                    retList.Add(tmpArr[1]);
        ...
        private string[] GetPalletNoInProductOrPcbOrMaterial(string cartonSN)
        {
            string[] arr = new string[3]{"","",""}; //0: palletno  1:carton sn 2: 1st SN
            ...
            //{ arr[0] = currentProduct.PalletNo; arr[1] = currentProduct.CartonSN; arr[2] = currentProduct.CUSTSN; }
            ...
            { arr[0] = mbInfoList[0].PalletNo; arr[1] = ""; arr[2] = mbInfoList[0].PCBNo; }
            ...
            { arr[0] = materialBoxList[0].PalletNo; arr[1] = materialBoxList[0].CartonSN; arr[2] = materialBoxList[0].BoxId; }
            IList<Material> materialList = materialRep.GetMaterial(new Material() { CartonSN = cartonSN });
            if (materialList != null && materialList.Count > 0)
            { arr[0] = materialList[0].PalletNo; arr[1] = materialList[0].CartonSN; arr[2] = materialList[0].MaterialCT; }
            return arr;
        }
