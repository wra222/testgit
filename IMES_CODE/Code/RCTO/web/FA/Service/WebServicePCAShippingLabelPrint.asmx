...
public class WebServicePCAShippingLabelPrint : System.Web.Services.WebService
{
    ...
    [WebMethod]
    public IList InputMB(string PdLine, string MBSNo, string checkPCMBRCTOMB, string editor, string stationId, string customer, string model)
    {
        IList ret = new ArrayList();

        try
        {
            ret = iPCAShippingLabelPrint.InputMBSNo(PdLine, MBSNo, checkPCMBRCTOMB, stationId, editor, customer, model);
    ...
    [WebMethod]
    public IList save(string MBno, string dcode, string region, IList<PrintItem> printItems)
    {
        IList ret = new ArrayList();
        try
        {
            ret = iPCAShippingLabelPrint.save(MBno, dcode, region, printItems);
            return ret;
        }
        catch (FisException ex)
        {
