...
namespace IMES.CheckItemModule.Battery.Filter
{
    ...
    [ExportMetadata("ProgramName", "IMES.CheckItemModule.Battery.Filter.dll")]
    public class Filter : IFilterModule, ITreeTraversal
    {
        private const string part_check_type = "Battery";
...
*/
                        string partNoItem = string.Empty;
                        flat_bom_item.Tag = kpVCList;
                        IList<string> vendorCodeList = new List<string>();
                       
                        foreach (IPart part in flat_bom_item.AlterParts)
                        {
                            //var vendorCode = (from item in part.Attributes
                            //                               where item.InfoType.Equals("VendorCode")
                            //                                select item.InfoValue.Trim()).ToList();
                            var vendorCode = kpVCList.Where(x => x.PartNo == part.PN).Select(x => x.VendorCode).ToList();
                            if (checkIsPAKStation(station))
                            {
                                vendorCodeList.Add(part.PN);
                            }
                            else
                            { 
                                vendorCodeList = vendorCodeList.Union(vendorCode).ToList(); 
                            }
                          
                            //string vendorCode = string.Join(",", vendorCodeList);
                            ////string vendorCode = part.GetAttribute("VendorCode")
                            //if (string.IsNullOrEmpty(partNoItem))
                            //{
                            //    partNoItem = vendorCode;
                            //}
                            //else
                            //{
                            //    partNoItem = partNoItem + "," + vendorCode;
                            //}                        
                        }

                        partNoItem = string.Join(",", vendorCodeList.ToArray());
                        ...
                        {
                            flat_bom_item.Descr = flat_bom_item.AlterParts.First().Descr;    
                        }
...
//            }
        ...
        private bool checkIsPAKStation(string station)
        {
            IPartRepository partRepository = RepositoryFactory.GetInstance().GetRepository<IPartRepository, IPart>();
            IList<IMES.DataModel.ConstValueTypeInfo> lstConst= partRepository.GetConstValueTypeList("Display63CodeInBatteryFilter");
           return lstConst.Count>0 && lstConst[0].value.Split(',').ToList().Contains(station);
        }
    }     
}
