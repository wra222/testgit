...
namespace IMES.CheckItemModule.BTCB.Filter
{
    ...
    {
        private int _qty;
        private const string part_check_type = "BTCB";
        ...
        {
            //第一阶的PartInfo中的Descr描述为( 'BTCB'）和第一阶的Descr描述为( 'JGS'）
            ...
            foreach (PartInfo part_info in part_infos)
                if (part_info.InfoType.Trim().Equals("Descr") && part_info.InfoValue.Trim().Equals("BTCB"))
                {
                    is_btcb = true;
                    break;
                }
