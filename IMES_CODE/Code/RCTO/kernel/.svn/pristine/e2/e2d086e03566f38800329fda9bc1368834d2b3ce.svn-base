...
namespace IMES.CheckItemModule.SmartBoard.Filter
{
    [Export(typeof(IFilterModule))]
    [ExportMetadata("ProgramName", "IMES.CheckItemModule.SmartBoard.Filter.dll")]
    public class Filter : IFilterModule
    {
        private int _qty;
        private const string part_check_type = "SmartBoard";
...
//                    _qty = bom.FirstLevelNodes.ElementAt(0).Qty;
        ...
        public bool CheckCondition(object node)
        {
            ...
            bool is_descr = ((BOMNode)node).Part.Descr.Trim().Contains("Smartboard");
            if (is_descr)
                return true;
            return false;
        }
