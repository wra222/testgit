...
namespace IMES.CheckItemModule.KB.Filter
{
    [Export(typeof(IFilterModule))]
    [ExportMetadata("ProgramName", "IMES.CheckItemModule.KB.Filter.dll")]
    public class Filter : IFilterModule, ITreeTraversal
    {
        private const string part_check_type = "KB";
        ...
        {
            //描述同上，不同的是(Descr like 'KB%' or  Descr like 'K/B%'）
            if (((BOMNode)node).Part == null)
            {
                return false;
            }
            bool is_contain_kb = ((BOMNode)node).Part.Descr.Trim().Contains("KB");
            int start_point_kb = ((BOMNode)node).Part.Descr.Trim().IndexOf("KB");
            bool is_contain_k_or_b = ((BOMNode)node).Part.Descr.Trim().Contains("K/B");
            int start_point_k_or_b = ((BOMNode)node).Part.Descr.Trim().IndexOf("K/B");

            if ((is_contain_kb && start_point_kb == 0) || (is_contain_k_or_b && start_point_k_or_b == 0))
                return true;
            return false;
        }
        private IList<ShareMaterialType> FilterShareMaterialType(IList<QtyParts> qty_parts)
        {
            ...
            var material_type = new ShareMaterialType { Qty = 1, Descr = "PartDescr" };
