...
namespace IMES.CheckItemModule.AIO.HDD.Filter
{
    ...
    {
        private int _qty;
        private const string part_check_type = "AIOHDD";
        ...
        public bool CheckCondition(object node)
        {
            ...
            if (child_nodes != null)
            {
                 foreach (IBOMNode child_node in child_nodes)
                   {
                       if (child_node.Part != null && child_node.Part.BOMNodeType.Equals("VC") && child_node.Part.Descr=="HDD")
                       {
            ...
            bool is_jgs = ((BOMNode)node).Part.Descr.Trim().Equals("HDD");
