...
namespace IMES.CheckItemModule.LCMCable.Filter
{
    ...
    {
        private int _qty;
        private const string part_check_type = "LCMCable";
        ...
        {
            //第一阶的PartInfo中的Descr描述为( 'LCMCable'）和第一阶的Descr描述为( 'JGS'）
            if (((BOMNode)node).Part == null)
            {
                return false;
            }
            bool is_jgs = ((BOMNode)node).Part.Descr.Trim().Equals("JGS");
            bool is_lcmcable = false;
            IList<PartInfo> part_infos = ((BOMNode)node).Part.Attributes;
            foreach (PartInfo part_info in part_infos)
                if (part_info.InfoType.Trim().Equals("Descr") && part_info.InfoValue.Trim().Equals("LCMCable"))
                {
                    is_lcmcable = true;
                    break;
                }
            if (is_jgs && is_lcmcable)
                return true;
            return false;
        }
    }
