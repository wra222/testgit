...
namespace IMES.CheckItemModule.WL.Filter
{
    ...
    {
        private const string part_check_type = "WL";
                            ...
                            foreach (IBOMNode bom_node in bom_nodes)
                            {
                                if (bom_node.Part != null && bom_node.Part.BOMNodeType.Equals("BM"))
                                {
                                    ...
                                    if (child_nodes != null)
                                    {
                                        ...
                                        {
                                            if (child_node.Part != null && child_node.Part.BOMNodeType.Equals("KP") && child_node.Part.Attributes != null && !child_node.Part.Descr.Contains("BLUETOOTH"))
                                            {
...
//                                    collect_gather_node.Add(bom.FirstLevelNodes.ElementAt(i));
        ...
        {
            //描述同上，不同的是(Descr like 'WIRELESS%' ）
            if (((BOMNode)node).Part == null)
            {
                return false;
            }
            bool is_contain_wireless = ((BOMNode)node).Part.Descr.Trim().Contains("WIRELESS");
            int start_point_wireless = ((BOMNode)node).Part.Descr.Trim().IndexOf("WIRELESS");

            if ((is_contain_wireless && start_point_wireless == 0))
                return true;
            return false;
        }
