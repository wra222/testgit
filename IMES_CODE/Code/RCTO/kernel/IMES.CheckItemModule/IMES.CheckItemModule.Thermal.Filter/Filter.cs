...
namespace IMES.CheckItemModule.Thermal.Filter
{
    [Export(typeof(IFilterModule))]
    [ExportMetadata("ProgramName", "IMES.CheckItemModule.Thermal.Filter.dll")]
    public class Filter : IFilterModule
    {
        private int _qty;
        private const string part_check_type = "Thermal";
...
//                    _qty = bom.FirstLevelNodes.ElementAt(0).Qty;
        ...
        {
            //第一阶的PartInfo中的Descr描述PartInfo.InfoValue(InfoType='Descr')  Like ( '%Thermal%'）且第一阶的Descr描述为( 'JGS'）
            if (((BOMNode)node).Part == null)
            {
                return false;
            }
            bool is_jgs = ((BOMNode)node).Part.Descr.Trim().Contains("JGS");
            bool contain_thermal = false;
            IList<PartInfo> part_infos = ((BOMNode) node).Part.Attributes;
            foreach (PartInfo part_info in part_infos)
            {
                if (part_info.InfoType.Equals("Descr"))
                {
                    contain_thermal = part_info.InfoValue.Contains("Thermal");
                    break;
                }
            }
            if (is_jgs && contain_thermal)
                return true;
            return false;
        }
    }
