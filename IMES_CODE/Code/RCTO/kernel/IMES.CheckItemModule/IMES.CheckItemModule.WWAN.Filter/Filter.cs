...
namespace IMES.CheckItemModule.WWAN.Filter
{
    [Export(typeof(IFilterModule))]
    [ExportMetadata("ProgramName", "IMES.CheckItemModule.WWAN.Filter.dll")]
    public class Filter : IFilterModule, ITreeTraversal
    {
        private const string part_check_type = "WWAN";
...
//                                    collect_gather_node.Add(bom.FirstLevelNodes.ElementAt(i));
                ...
                if (kp_check_conditon_nodes != null && kp_check_conditon_nodes.Count > 0)
                {
        ...
        {
            //描述同上，不同的是(Descr like 'WWAN%' ）
            if (((BOMNode)node).Part == null)
            {
                return false;
            }
            bool is_contain_wwan = ((BOMNode)node).Part.Descr.Trim().Contains("WWAN");
            int start_point_wwan = ((BOMNode)node).Part.Descr.Trim().IndexOf("WWAN");

            if ((is_contain_wwan && start_point_wwan == 0))
                return true;
            return false;
        }
