<%@ Page Language="C#" AutoEventWireup="true" CodeFile="dashboardSmtSetting.aspx.cs" Inherits="webroot_aspx_dashboard_dashboardSmtSetting" %>
<%--
...
<body >
   ...
   <tr>
    ...
    <tr id="idLineBlock" style="display:none">
	    <td>
	    <fieldset style="width: 100%"><legend>Line & Family</legend>
	    <div id= "divLine">
	     <table >
	     <tr>
	        <td colspan=2>
...
var existFamilys=new Array();
var existSeriesList=new Array();
var isDoing=false;
...
var dlgTitleForStationEdit="Edit Family";
var operationForStationEdit="new";

//打开的对话框大小 
var dlgLineFeature = "dialogHeight:380px;dialogWidth:435px;center:yes;status:no;help:no";
...
var SA=String("<%=Constants.SA_STAGE %>");
var SMT=String("<%=Constants.SMT_STAGE %>");
...
function setDefaultStageWorkTime()
{
    ...
    if(stageId!="")
    {
         ...
         try
         {
            rtn = com.inventec.portal.dashboard.Smt.DashboardManager.GetStageWorkTimeInfo(stageId);
         }
         catch(e)
         {
...
function initPage()
{
     ...
     try
     {
        rtn = com.inventec.portal.dashboard.Smt.DashboardManager.GetDashboardWindowSetting(windowId,stageType);
     }
     catch(e)
     {
        alert("Can't get data from server.");
...
{     
     ...
     if(isSetCheckByGetWindowsData==true)
     {
         ...
         setCheckBoxValue(document.getElementById("dIsInputDisplay"), windowData.IsInputDisplay);
         setCheckBoxValue(document.getElementById("dIsOutputDisplay"), windowData.IsOutputDisplay);
         setCheckBoxValue(document.getElementById("dIsRateDisplay"), windowData.IsRateDisplay);         
         
     }
     else
     {
         setCheckBoxValue(document.getElementById("dIsGoalDisplay"), "True");
         setCheckBoxValue(document.getElementById("dIsInputDisplay"), "True");
         setCheckBoxValue(document.getElementById("dIsOutputDisplay"), "True");
         setCheckBoxValue(document.getElementById("dIsRateDisplay"), "True");       
     }
...
function initLineAndStation(selectedLineNum)
{
   ...
   else
   {
     ...
     try
     {
        rtn = com.inventec.portal.dashboard.Smt.DashboardManager.GetNewStationList();
     }
     catch(e)
     {
        alert("Can't get data from server.");
...
//    var tableWidth = parseInt(document.body.clientWidth*0.80);
    ...
    if (lineTable == null){
	    ...
	    var tmpWidths = new Array(10,11,12,12,12,26);  
        lineTable.Widths = getTableWidthsFixColumn(tmpWidths,tableWidth);
        lineTable.FieldsType = new Array( 0, 0, 0, 0, 0, 0);
        lineTable.HideColumn = new Array(true,true,true,true,true,true);
...
//    var tableWidth = parseInt(document.body.clientWidth*0.80);
    ...
    if (stationTable == null){
	    ...
	    var tmpWidths = new Array(10,10,15);  
        stationTable.Widths = getTableWidthsFixColumn(tmpWidths,tableWidth);
        stationTable.FieldsType = new Array( 0, 0, 0);
        stationTable.HideColumn = new Array(true,true,true);
        //lineTable.UseSort = "TDCData";
...
function getLineRs(lineList) {
    ...
    for (var i = 0; i < lineList.length; i++)
    {
        ...
        rsSite.Fields(2)=lineItem.Shift;
     
        rsSite.Fields(3)=lineItem.StartWorkHour+":"+lineItem.StartWorkMinute;
        rsSite.Fields(4)=lineItem.StopWorkHour+":"+lineItem.StopWorkMinute;
        
        var optionDis="";
        if(stageType==SA)
        {
            
            if(lineItem.IsInputDsp.toLowerCase()=="true")
            {     
                if(optionDis!="") optionDis+=",";      
                optionDis+="Input";
            }
            if(lineItem.IsDefectDsp.toLowerCase()=="true")
            {     
                if(optionDis!="") optionDis+=",";      
                optionDis+="Defect";
            }
            if(lineItem.IsYieldRateDsp.toLowerCase()=="true")
            {     
                if(optionDis!="") optionDis+=",";      
                optionDis+="Yield Rate";
            }
            if(lineItem.IsICTInputDsp.toLowerCase()=="true")
            {     
                if(optionDis!="") optionDis+=",";      
                optionDis+="ICT Input";
            }
            if(lineItem.IsICTDefectDsp.toLowerCase()=="true")
            {     
                if(optionDis!="") optionDis+=",";      
                optionDis+="ICT Defect";
            }
            if(lineItem.IsICTYieldRateDsp.toLowerCase()=="true")
            {     
                if(optionDis!="") optionDis+=",";      
                optionDis+="ICT Yield Rate";
            }                                                            
           
            rsSite.Fields(5)=optionDis;
        }
        else
        {
            ...
            if(lineItem.IsAOIOutputDsp.toLowerCase()=="true")
            {     
                if(optionDis!="") optionDis+=",";      
                optionDis+="AOI Output";
            }
            if(lineItem.IsAOIDefectDsp.toLowerCase()=="true")
            {     
                if(optionDis!="") optionDis+=",";      
                optionDis+="AOI Defect";
            }                    
            rsSite.Fields(5)=optionDis;
        }        
...
function getStationRs(stationList) {
    ...
    rsSite.Fields.Append("Family", 202,500);
    rsSite.Fields.Append("Series", 202,500);
    rsSite.Fields.Append("Plan", 202,500);
    rsSite.Open();
    ...
    for (var i = 0; i < stationList.length; i++)
    {
        ...
        rsSite.AddNew();
        rsSite.Fields(0)=stationItem.Family;
        rsSite.Fields(1)=stationItem.Series;
        rsSite.Fields(2)=stationItem.PlanQty;
        rsSite.Update();
    }
    return rsSite;
}               
...
//    var layoutStageDefHTML3= getLayoutHTML(layoutStageDefOptions3);
    ...
    itemArray[27]={baseName:"IsInputDisplay",type:"checkbox",item:"Input"}
    itemArray[28]={baseName:"IsOutputDisplay",type:"checkbox",item:"Output"}
    itemArray[29]={baseName:"IsRateDisplay",type:"checkbox",item:"Rate"} //!!!
...
function dealBtnClick()
{
   ...
   case "btnLineAdd" :
         ...
         try
         {
            rtn = com.inventec.portal.dashboard.Smt.DashboardManager.GetNewLine();
         }
         catch(e)
         {
            alert("Can't get data from server.");
        ...
        try
        {
           rtnLineList =  com.inventec.portal.dashboard.Smt.DashboardManager.GetLineListByStageEclipse(stageType,existLines);
        }
        catch(e)
        {
           alert("Can't get data from server.");
        ...
        if(lineListDataTable==null)
        {
 	    ...
 	    if(stageType==SA)
 	    {
 	        dlgReturn =window.showModalDialog("saLineEdit.aspx", window, dlgLineFeature);
        }
        else
        {
            dlgReturn =window.showModalDialog("smtLineEdit.aspx", window, dlgLineFeature);
        }
 	    ...
 	    if(dlgReturn=="OK")
 	    {
 	        ...
 	        showLineTable(lineRs);
 	        var stationList=addEditLineObj.WindowLineFamilyInfos;
   ...
   case "btnLineEdit":
        ...
        else
        {
 	    ...
 	    if(dlgReturn=="OK")
 	    {
 	        ...
 	        {
 	            stationList=addEditLineObj.WindowLineFamilyInfos;
 	        }
 	        else
 	        {
                 var rtn;    
                 ...
                 addEditLineObj.WindowLineFamilyInfos= stationList;      
 	        }
   ...
   case "btnStationAdd" :
         ...
         try
         {
            rtn = com.inventec.portal.dashboard.Smt.DashboardManager.GetNewStation();
         }
         catch(e)
         {
            alert("Can't get data from server.");
        ...
        existFamilys=new Array();
        existSeriesList=new Array();
        

        for (var i=0;i<getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos.length;i++)
        {
            existFamilys[i]=getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[i].Family;
            existSeriesList[i]=getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[i].Series;
        }
        var rtnFamilyList;   
         
        try
        {
           existFamilys.toJSON = function(){return toJSON(this);};	
           existSeriesList.toJSON = function(){return toJSON(this);};	
           rtnFamilyList = com.inventec.portal.dashboard.Smt.DashboardManager.GetFamilyList(existFamilys,existSeriesList);
        }
        catch(e)
        {
           alert("Can't get data from server.");
        ...
        //var rtnFamilyList = com.inventec.portal.dashboard.DashboardManager.GetStationListByLine(lineId);
        if (rtnFamilyList.error != null) 
        {
            alert(rtnFamilyList.error.Message);
            return;
        }
        else 
        { 
            stationListDataTable=rtnFamilyList.value;
        } 
        ...
        var dlgReturn=window.showModalDialog("familyEdit.aspx", window, dlgStationFeature);
        
 	    if(dlgReturn=="OK")
 	    {
 	        var currentHighlightRow= getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos.length;
 	        getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[currentHighlightRow]=addEditStationObj;  	       
 	        var stationList=getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos;
            var stationRs=getStationRs(stationList);
            showStationTable(stationRs);
   ...
   case "btnStationEdit":
        ...
        for (var i=0;i<getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos.length;i++)
        {
            if(i!=rownum)
            {
                existFamilys[existFamilys.length]=getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[i].Family;
                existSeriesList[existSeriesList.length]=getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[i].Series;
            }
        }
        ...
        addEditStationObj=getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[lineNumForStationEdit];
        var dlgReturn=window.showModalDialog("familyEdit.aspx", window, dlgStationFeature);
   ...
   case "btnStationDelete":
        ...
        getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos=RemoveFromArray(getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos,rownum);
        
        var stationRs=getStationRs(getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos);   
        showStationTable(stationRs);
        ...
        if(getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos.length>0)
        {
   ...
   case "btnStationMoveUp":
        ...
        var tmpSave=getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[rownum-1];
        getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[rownum-1]=getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[rownum];
        getEditData.WindowLineInfos[rownumLine].WindowLineFamilyInfos[rownum]=tmpSave;
...
function saveClick()
{
    ...
    if(getCheckBoxValue(document.getElementById("dIsGoalDisplay")) ==true)
    {
        dataItem.IsGoalDisplay ="True";
    }
    else
    {
     ...
     if(getCheckBoxValue(document.getElementById("dIsInputDisplay")) ==true)
    {
        dataItem.IsInputDisplay ="True";
    }
    else
    {
        dataItem.IsInputDisplay ="False";
    }
    
    if(getCheckBoxValue(document.getElementById("dIsOutputDisplay")) ==true)
    {
        dataItem.IsOutputDisplay ="True";
    }
    else
    {
        dataItem.IsOutputDisplay ="False";
    }
    
    if(getCheckBoxValue(document.getElementById("dIsRateDisplay")) ==true)
    {
    ...
    try
    {
       rtn = com.inventec.portal.dashboard.Smt.DashboardManager.SaveDashboardWindowSetting(dataItem,stageType);
    }
    catch(e)
    {
       isDoing=false;
...
//    {
//        disabledStationButton(true);
//        stationList=getEditData.WindowLineInfos[rownum].WindowLineFamilyInfos;
//    }
    else
    {
