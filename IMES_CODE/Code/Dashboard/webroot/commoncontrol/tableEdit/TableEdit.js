/*
    ...
    fixed bug ITC-1101-0006 on 2009 by itc94006
*/
function clsTable(RecordSet,ObjectName)
{
    ...
    this.AutoAdjustWidth = _AutoAdjustWidth;//add by lzy
    this.Container = null;//the container of this table object.
}
...
function _HeadHTML(sWid)
{
	...
	var HEAD_ROW_BEGIN = "<TR style='word-break:break-all; word-wrap:break-word; border-color:black' name=Head bgcolor=#C7D0D9 align=center>"
	var HEAD_ROW_END = "</TR>"
...
function _BodyHTML(iRow,iFast,iLeft)
{
	...
	for (var j = iRow; j<this.rs_main.RecordCount; j++)
	{
		...
		if (this.rs_main.Fields(this.GetOrder[this.Widths.length-1]).Name == "CC_COLOR" && this.rs_main.Fields(this.GetOrder[this.Widths.length-1]).value != "")
			sBackC = " Style='background-color:" + this.rs_main.Fields(this.GetOrder[this.Widths.length-1]).value + "; height:20px; color:black; word-break:break-all; word-wrap:break-word;'"
		else
		...
		for (field=0; field<this.Widths.length; field++)
		{
            ...
            var tmpWidth = this.Widths[this.GetOrder[field]] - 5;
            				
			sWidth = CELL_BEGIN.replace("FFIIEE","" + field + sDisplay + " width=" + tmpWidth)
	...
	{
		sBackC = " Style='background-color:" + this.arrColor[bl % 2] + "; height:20px; color:black; word-break:break-all; word-wrap:break-word;'"
...
function _AutoAdjustWidth()
{
    ...
    {
        if (this.HideColumn[i] == true)//not hidden,ignor the width of hidden column
            len += this.Widths[i];
    }        
    
    var containerWidth = this.Container.style.pixelWidth-18;
    for (var i=0; i<this.HideColumn.length; i++)
    {
        if (this.HideColumn[i] == true)//not hidden,ignore the width of hidden column
            temp[i] = Math.round((this.Widths[i]/len) * containerWidth);
        else
            temp[i] = this.Widths[i];//ignore
    } 
    
    this.Widths = temp;            
    this.TableWidth = this.Container.style.pixelWidth;    
    //this.Height =  this.Container.style.pixelHeight;
...
{
    //this.AutoAdjustWidth();//add by lzy
	...
	var HEAD_ROW_BEGIN = "<TR style='word-break:break-all;word-wrap:break-word;border-color:" + this.TableLineColor + "' name=Head bgcolor=#D1CEC9 align=center>"
	var HEAD_ROW_END = "</TR>"
