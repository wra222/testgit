...
#region WAYBILL_NUMBER
                        else if (column.ColumnName == "WAYBILL_NUMBER")
                        {
                            ...
                            else if (RegionTable.Rows[0]["REGION"].ToString().Trim() == "LA")
                            {
                                ...
                                //fieldsPackID.Add("SHIP_REF");
                                DataTable PackIDTable = DBFactory.PopulateTempTable("[v_Shipment_PAKPalletTypeBLineItem]", sqlCmd, fieldsPackID);
                                ...
                                {
                                    //string sqlCmd=" select count(*)  from v_Shipment_PAKPalletTypeBLineItem  where PALLET_ID='"+palletID+"'";
                                    newRow[column] = readRow[column.ColumnName];
                    ...
                    foreach (DataColumn column in newTable.Columns)
                    {
                        if (orderTable.Columns.Contains(column.ColumnName))
                        {
                            ...
                            if (column.ColumnName == "HP_PN")
                            {
                                ...
                                //modify by Duan
                                //string tempStr = newRow[column].ToString();
                                //if (newRow[column].ToString() != tempStr)
                                //{
                                //    indexpn++;
                                //}
                                //modify by Duan
