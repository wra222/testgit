	...
	public class BoxLabelXmlCreator : XmlCreator
	{
        ...
        public void LoadBoxLabelDatabaseData(string id)
        {
            ...
            foreach (DataRow readRow in packIDTable.Rows)
            {
                ...
                string whereBoxSEQUENCE = "WHERE Cdt<= (select max(Cdt) from [PAK_PackkingData] where InternalID='" + internalID + "' and SERIAL_NUM='" + SerialNum + "') and left(InternalID,10)='" + internalID.Substring(0, 10) + "'";// and InternalID='" + internalID + "'";
