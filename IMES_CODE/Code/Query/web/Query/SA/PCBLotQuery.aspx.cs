...
public partial class Query_SA_PCBLotQuery : IMESQueryBasePage
{
   ...
   // IMES.Query.Interface.QueryIntf.IFamily Family = ServiceAgent.getInstance().GetObjectByName<IMES.Query.Interface.QueryIntf.IFamily>(WebConstant.IFamilyBObject);
    ...
    ISA_PCBLotQuery SA_PCBLotQuery = ServiceAgent.getInstance().GetObjectByName<ISA_PCBLotQuery>(WebConstant.ISA_PCBLotQuery);

    public string[] GvQueryColumnName = { "LotNo", "PCBNo", "Status", "Editor","Cdt","Udt" };
    public int[] GvQueryColumnNameWidth = { 150, 150,50,150,150, 150 };
    ...
    protected void btnQuery_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            String LotNo = "", PCBNo = "";
            hidModelListLotNo.Value = hidModelListLotNo.Value.Replace("'", "");
            hidModelListPCBNo.Value = hidModelListPCBNo.Value.Replace("'", "");
            txtLotNo.Text = txtLotNo.Text.Replace("'", "");
            txtPCBNo.Text = txtPCBNo.Text.Replace("'", "");
            if (rdbutPCBLot.SelectedItem.Value == "LotNo")
            {
                if (txtLotNo.Text != "")
                {
                    LotNo = txtLotNo.Text;
            ...
            //int sum = 0;
            dt = SA_PCBLotQuery.GetPCBLotQueryResult(DBConnection, LotNo, PCBNo, NumType);
