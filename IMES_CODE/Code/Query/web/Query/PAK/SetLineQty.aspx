<%@ Page Language="C#" AutoEventWireup="true" CodeFile="SetLineQty.aspx.cs"
    Inherits="Query_PAK_SetLineQty"  %>

<%@ Import Namespace="com.inventec.iMESWEB" %>
<html>
<head runat="server">
<title>Set Line Qty</title>
        ...
        <div id="div3">
            <table width="100%"  class="iMes_div_MainTainEdit">
                <tr style="height:30px">
                    <td width="10%">
                        <asp:Label ID="lblItemName" runat="server" Text="" CssClass="iMes_label_13pt"></asp:Label>
                    </td>
                    <td width="90%">
                ...
                <td  align="right" colspan="2">
                  <input type="button" id="btnDelete" runat="server" style="display:none" onclick="if(clkDelete())"  onserverclick="btnDelete_ServerClick"
                            class="iMes_button" value="Delete" />
&nbsp;<input type="button" runat="server" id="btnAdd" onclick="if(clkAdd())" class="iMes_button"
                            onserverclick="btnAdd_ServerClick" value="Add" style="display:none" />
                        <input type="button" runat="server" id="btnSave" onclick="if(clkSave())" class="iMes_button"
    ...
    <script type="text/javascript">
        ...
        function clkSave() {
            var qty = document.getElementById("<%=txtInfoValue.ClientID %>").value;
            if(qty!='0')
			{
				var r = /^[0-9]*[1-9][0-9]*$/;///正整数
             var testR=  r.test(qty); //str为你要判断的字符 执行返回结果 true 或 false
             if (!testR)
             { alert('請輸入正整數'); return false; }
			}
		
            var line = $('#<%= lblItemName.ClientID %>').html();
            document.getElementById("<%=hidLine.ClientID %>").value = line;

            document.getElementById("<%=hidQty.ClientID %>").value = document.getElementById("<%=txtInfoValue.ClientID %>").value;
        ...
        function ShowRowEditInfo(con) {
            ...
            var idxP = '#' + "<%=lblItemName.ClientID %>";


            $(idxP).text(con.cells[0].innerText.trim());
