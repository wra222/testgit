...
public partial class Query_FA_ProductImageDownTimeout : IMESQueryBasePage
{
    ...
    IFA_ProductImageDownTimeout ProductImageDownTimeout = ServiceAgent.getInstance().GetObjectByName<IFA_ProductImageDownTimeout>(WebConstant.IFA_ProductImageDownTimeout);
    IQueryCommon QueryCommon = ServiceAgent.getInstance().GetObjectByName<IQueryCommon>(WebConstant.QueryCommon);

    public string[] GvQueryColumnName = { "ProductID", "Model", "Line" ,"Station" ,"Cdt" };
    public int[] GvQueryColumnNameWidth = { 80, 80, 100 ,80,80};
    ...
    private void InitPage()
    {
        ...
        this.lblPCBNo.Text = this.GetLocalResourceObject(Pre + "_lblPCBNo").ToString();
        ...
        try
        {
          ...
          foreach (ListItem item in lboxPdLine.Items)
           ...
           Line = string.Join(",", lstPdLine.ToArray());
            //int sum = 0;
           dt = ProductImageDownTimeout.GetQueryResult(DBConnection, Line, DateTime.Parse(txtFromDate.Text), DateTime.Parse(txtToDate.Text));
