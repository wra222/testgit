    ...
    public class FA_ProductStatement : MarshalByRefObject, IFA_ProductStatement
    {
                            ...
                            IList<string> lstPdLine, string Family, string Station, IList<string> Model, bool IsWithoutShift, string ModelCategory)
        {
            ...
            try
            {
                ...
                sb.AppendLine("SELECT DISTINCT a.ProductID, b.CUSTSN,a.Line ");
                ...
                sb.AppendLine(") ");
                sb.AppendLine("SELECT ROW_NUMBER()OVER(ORDER BY a.ProductID) AS [No], a.ProductID, m.Family, a.CUSTSN, mi.Value as 'Desc', d.Model , pn.Value as SKU , b.Station + '' + c.Name AS Station, ");
                sb.AppendLine("CASE b.Status WHEN '1' THEN 'PASS' WHEN '0' THEN 'FAIL' ELSe CONVERT(varchar(5),b.Status) END AS [Status],b.Line, b.Editor, b.Udt ,isnull(loc.SnoId,'') as LOC");
                ...
                sb.AppendLine("LEFT JOIN ModelInfo (NOLOCK) pn ON d.Model = pn.Model and pn.Name='PN'  ");
                sb.AppendLine("LEFT JOIN PAK_LocMas (NOLOCK) loc ON loc.Pno = d.PalletNo and loc.Pno<>''  ");
