...
//                                   ";         
                string SQLText = @" select distinct
                                                           Carrier as Carrier,
                                                          d.TruckID as [Truck No],
                                                          d.Driver,
                                                          a.PLT as [Pallet No],
                                                          a.RFIDCode as [RFID Pallet No],
                                                          a.Cdt,
                                                           a.Udt,
                                                          a.Editor
                                                    from Pallet_RFID a WITH(NOLOCK) 
                                                    join  Delivery_Pallet  b on a.PLT=b.PalletNo
                                                   join MAWB  c on b.DeliveryNo=c.Delivery
                                                   join Forwarder  d on c.MAWB=d.MAWB or c.HAWB=d.MAWB
