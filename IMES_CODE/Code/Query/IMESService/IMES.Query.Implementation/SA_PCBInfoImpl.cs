...
namespace IMES.Query.Implementation
{
    public class SA_PCBInfo : MarshalByRefObject, ISA_PCBInfo
    {
        ...
        public MBInfo GetMBInfo(string DBConnection, string MBSN, out List<NextStation> NextStationList)
        {
            ...
            string strSQL = @"SELECT  a.PCBNo , a.CUSTSN, a.MAC, a.ECR, a.SMTMO, a.IECVER,a.CVSN,
                             ...
                             WHERE a.PCBNo = @PCBNo or a.MAC = @PCBNo  or a.CVSN= @PCBNo ";
            ...
            if (tb.Rows.Count == 1)
            {
                ...
                mb.IECVER = tb.Rows[0]["IECVER"].ToString().Trim();
                mb.CPU = tb.Rows[0]["CVSN"].ToString().Trim();
        ...
        {
//            string strSQL = @"SELECT  a.Station , c.Descr as StationName, a.Line, 
//                                 (case a.Status when 1 then 'Pass' else 'Fail' end) as Status , 
//                                 a.Editor,CONVERT(varchar(20) , a.Cdt, 120) AS Cdt 
...
//                            WHERE a.PCBNo =@PCBNo
//                            union all
//	                            select 'Reprint' as Station, LabelName as StationName, '' as Line, 'Pass' AS Status,
//	                            Editor, CONVERT (nvarchar(20),Cdt,120) AS Cdt
//	                            from RePrintLog (NOLOCK) where 
//	                            (@PCBNo between BegNo and EndNo
//	                            AND BegNo LIKE SUBSTRING(@PCBNo,1,1)+'%'
//	                            AND EndNo LIKE SUBSTRING(@PCBNo,1,1)+'%')
//	                            or BegNo=@PCBNo or Descr=@PCBNo or
//	                            (@PCBNo between BegNo and EndNo) or BegNo=@PCBNo or Descr=@PCBNo
//                            union all
//	                            select 'Print' as Station, Name as StationName, '' as Line,'Pass' AS Status,
//	                            Editor, CONVERT (nvarchar(20),Cdt,120) AS Cdt
//	                            from PrintLog (NOLOCK) where 
//	                            (@PCBNo between BegNo and EndNo
//	                            AND BegNo LIKE SUBSTRING(@PCBNo,1,6)+'%'
//	                            AND EndNo LIKE SUBSTRING(@PCBNo,1,6)+'%')
//	                            or BegNo=@PCBNo or Descr=@PCBNo or EndNo=@PCBNo
//	                            
...
//                                                     strSQL,
//                                                     paraName);
            DataTable tb = SQLHelper.ExecuteDataFill(DBConnection, System.Data.CommandType.StoredProcedure,
                                                 "sp_Query_SA_GetMBHistory",
                                                 new SqlParameter("@PCBNo", MBSN));
            return tb;
        }
        ...
        public DataTable GetRepairInfo(string DBConnection, String MBSN)
        {
            string strSQL = @"SELECT r.Line,RTRIM(r.Station)+' - '+s.Descr as StationName, Status,
                             ...
                             WHERE a.PCBNo=@PCBNo OR a.MAC=@PCBNo or a.CVSN=@PCBNo  ORDER BY a.Cdt";
            SqlParameter paraName = new SqlParameter("@PCBNo", SqlDbType.VarChar, 32);
            paraName.Direction = ParameterDirection.Input;
            paraName.Value = MBSN;
            DataTable tb = SQLHelper.ExecuteDataFill(DBConnection,
        ...
        public DataTable GetPCBInfo(string DBConnection, string MBSN)
        {
            string Sql = @"SELECT InfoType,InfoValue,Editor,Cdt FROM dbo.PCBInfo(NOLOCK) WHERE PCBNo =@PCBNo";
            SqlParameter para = new SqlParameter("@PCBNo", SqlDbType.VarChar, 32);
            para.Direction = ParameterDirection.Input;
            para.Value = MBSN;
            DataTable dt = SQLHelper.ExecuteDataFill(DBConnection, System.Data.CommandType.Text, Sql, para);
