    ...
    public class FA_PIADefectList : MarshalByRefObject, IFA_PIADefectList
    {
              ...
              string Station, string Defect, string Cause, List<string> PdLine)
        {
            ...
            try
            {
                ...
                sb.AppendLine("SELECT distinct a.ProductID, f.Model, a.Line,b.Obligation,b.OldPart,b.OldPartSno,b.NewPart,b.NewPartSno, ");
              ...
              List<string> Station, List<string> Defect, List<string> Cause, List<string> PdLine, string ModelCategory) 
        {
                  ...
                  try
                  {
                      ...
                      sb.AppendLine("SELECT distinct a.ProductID, mo.Family, pro.Model,	SUBSTRING(CONVERT(varchar(8),DATEADD(HH,-8,c.Udt),112 ),5,2) AS [Month], ");
                       ...
                       sb.AppendLine("rtrim(a.Station)+'_'+rtrim(sta.Name), isnull(c.DefectCode,h.DefectCodeID) AS Defect, c.Cause, c.MajorPart, c.Remark,c.Obligation,c.OldPart, c.OldPartSno, c.NewPart, c.NewPartSno ");
			           sb.AppendLine(",deco.EngDescr AS FailureDescription,deinf.EngDescr AS PartCategory,dein.EngDescr AS Catergy, ");
                       sb.AppendLine("pro.CUSTSN,bios.InfoValue AS BIOS,imge.InfoValue AS IMAGE,mbct.InfoValue AS MBCT ,mbri.Location AS ComponentDesignator, c.Editor, c.Udt");
                       ...
                       sb.AppendLine(" LEFT JOIN DefectInfo deinf (NOLOCK) ON c.MajorPart=deinf.Code AND deinf.Type='MajorPart' ");
                       sb.AppendLine("LEFT JOIN DefectInfo dein(NOLOCK) ON c.Obligation=dein.Code AND dein.Type='Obligation' ");
                       ...
                       sb.AppendLine("LEFT JOIN dbo.ModelBOM (NOLOCK) modelbom ON modelbom.Material=mo.Model  AND LEFT(modelbom.Component,5) IN ('ZM2DC','ZM2MS') ");

                       //if (string.IsNullOrEmpty(ModelCategory))//Docking
                      // {
                      //     sb.AppendLine("left JOIN dbo.OSWIN (NOLOCK) os ON os.Zmode=RIGHT(rtrim(modelbom.Component),12) ");
                      // }
                       //else
                      // {
                      //     sb.AppendLine("inner JOIN dbo.OSWIN (NOLOCK) os ON os.Zmode=RIGHT(rtrim(modelbom.Component),12) ");
                       //}
                       sb.AppendLine("LEFT JOIN dbo.ProductInfo(NOLOCK) bios ON bios.ProductID=pro.ProductID AND bios.InfoType='BIOS' ");
                       sb.AppendLine("LEFT JOIN dbo.ProductInfo(NOLOCK) imge ON imge.ProductID=pro.ProductID AND imge.InfoType='IMG' ");
                       sb.AppendLine("LEFT JOIN dbo.PCBInfo (NOLOCK) mbct ON mbct.PCBNo=c.OldPartSno AND mbct.InfoType='MBCT' ");
                       sb.AppendLine("LEFT JOIN dbo.PCBRepair(NOLOCK) mbre ON mbre.PCBNo=c.OldPartSno AND CONVERT(CHAR(20),c.Udt,120)=CONVERT(CHAR(20),mbre.Cdt,120) ");
                       sb.AppendLine("LEFT JOIN  dbo.PCBRepair_DefectInfo  (NOLOCK) mbri ON mbre.ID=mbri.PCARepairID ");  
        ...
        public DataTable GetQueryResult_IQDC(string Connection, DateTime FromRepairDate, DateTime ToRepairDate,
             List<string> Station, List<string> Defect, List<string> Cause, List<string> PdLine, string ModelCategory)
        {
            string methodName = MethodBase.GetCurrentMethod().Name;
            ...
            try
            {
                ...
                sb.AppendLine("Inner JOIN ProductRepair_DefectInfo_IQDC c WITH(NOLOCK) ON c.ProductRepairID = b.ID ");
                if (Defect.Count > 0)
                {
                    sb.Append(string.Format("AND c.DefectCode IN ('{0}') ", string.Join("','", Defect.ToArray())));
                }
