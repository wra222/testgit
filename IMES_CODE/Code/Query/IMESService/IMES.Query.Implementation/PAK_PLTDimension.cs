        ...
        public DataTable GetQueryResult(string Connection, string input, string type, DateTime shipDate)
        {
            ...
            try
            {
                ...
                string SQLText = @"select    p.DeliveryNo,
                          ...
                          case when d2.InfoValue='T001' then 'AIR' when d2.InfoValue='T002' then 'Sea' ELSE d2.InfoValue END AS ShipWay,
                           p.PalletNo as PLTNO,
                          e.Weight,
                           a.Len as [Len],
                           a.High as [Height],
              ...
              from Delivery_Pallet p  WITH(NOLOCK) 
                     ...
                     left join DeliveryInfo d on  d.DeliveryNo=p.DeliveryNo and d.InfoType='Country'
                     left join DeliveryInfo d2 on  d2.DeliveryNo=p.DeliveryNo and d2.InfoType='ShipWay'
                     left join Delivery b on  p.DeliveryNo=b.DeliveryNo
                where ";
                switch (type)
                { 
                    case "Pallet" :
                        subSQL = "  p.PalletNo=@input";
                        break;
                    case "DN":
                        subSQL = "  p.DeliveryNo like @input";
                        break;
                    case "Model":
                        if (input == "")
                        { subSQL = "  b.ShipDate=@shipDate";}
                        else
                        {
                            subSQL = "  b.ShipDate=@shipDate and b.Model=@input"; 
